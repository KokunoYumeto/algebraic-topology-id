...
Status: active production. Source admission and composite route selection are closed. Units 001–021 translate Roberts `Notes.tex:134–4500` contiguously; every admitted unit has independent P1/P2/P3-zero review, stable IDs, solved mastery, append-only backend records, and deterministic cumulative readers. Unit 21 is the next GitHub publication boundary. Unit 22 is already source-frozen and independently clean at `Notes.tex:4501–4938`, but remains outside the Unit 21 index/commit until its own backend and cumulative build are admitted; Unit 23 was not started. The existing public Zenodo lineage is current through Unit 20 (`10.5281/zenodo.22071667`, concept `10.5281/zenodo.22061489`), so this one-unit boundary will not create a spam DOI update. The same-field Indonesian terminology QA is complete, its Unit 20 propagation is closed, and the edition records `OpenAI Codex gpt-5.6-sol, Ultra` without diminishing source or human credit. After all 30 Roberts lectures, the selected Fomberg lines 31–4185 and exact original closure follow; see `CURRICULUM_ROUTE_AND_FOMBERG_HANDOFF.md`.

## Current verified Units 001–021 local boundary (2026-08-23)
...
- Publication state before the next transaction: Unit 20 is public and anonymously byte-verified on GitHub Pages and Zenodo; Unit 21 is locally ready but not yet claimed public. Immediate action is one narrow Unit 21 commit/push, successful Pages deployment, anonymous raw/reader hash readback, and a sanitized receipt. Zenodo remains at Unit 20 because a single-unit update is not a substantial DOI boundary.

## Safely frozen Unit 22 working boundary (not yet admitted)
...
- Unit 22 ledger rows are `O012-TERM-0293..0300` and `O012-ADV-0298..0311`; the rejected terminology variants do not occur. These working-tree rows and Unit 22 files must remain unstaged during the Unit 21 commit, then become the exact prefix for Unit 22 backend/build admission. Unit 23 has not started.
