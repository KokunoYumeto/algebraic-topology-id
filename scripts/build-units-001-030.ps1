...
$baselineBuilder = Join-Path $PSScriptRoot 'build-units-001-029.ps1'
$authority = Join-Path $lane 'authority\upstream\AlgebraicTopology2019-b947ad2e9f9e301bfe24590a9db653bc54fa1a53\Notes.tex'
$unit030 = Join-Path $lane 'source\id-ID\units\unit-030-lecture-030.md'
$unit030Qa = Join-Path $lane 'qa\UNIT_030_QA.json'
$unit030Review = Join-Path $lane 'qa\UNIT_030_INDEPENDENT_REVIEW.md'
$unit030Audit = Join-Path $lane 'qa\UNIT_030_SOURCE_AUDIT.md'
...
$htmlDir = Join-Path $lane 'output\html\units-001-030'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-030-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-030-id.pdf'
$manifestPath = Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_030.csv'

# Frozen Units 001-029 boundary. These inputs are verified but never invoked
# or rewritten by this cumulative builder.
$baselineFrozen = @{
    $baselineBuilder = @(22399, 'fd40c592a28f3b804d2269a69df94d3c8af757369eacf7d4b7875409818419cd')
    (Join-Path $lane 'output\html\units-001-029\index.html') = @(4751974, '67e5dd915cbbdf9b1961368efb1538b9ce0da8a4596d7ade11a4af051a189167')
    (Join-Path $lane 'output\pdf\topologi-aljabar-unit-001-029-id.pdf') = @(2211479, '0a140fbfb48960fa178b3409bd6cca03c2819fedec4c84e12486f662dac9a317')
    (Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_029.csv') = @(249, '5fba43e016d94bbc80ffe501c2512c482d55f914c68f96be5783b097100d5be7')
    (Join-Path $lane 'qa\UNITS_001_029_BUILD_RECEIPT.json') = @(10762, 'f2c84ec523daded9e546d7fdfae14eda191f5ff35f9f40718e20438bacfe3667')
    (Join-Path $lane 'qa\UNITS_001_029_VISUAL_QA.md') = @(5485, '7dd48c6c4d18f61934357ba36e180f6f9e5660f1ba711d7da895477c4cd1c6fc')
    (Join-Path $lane 'qa\UNITS_001_029_RENDER_INVENTORY.csv') = @(2233, '10c0c9ea2780a171ac629c7e76ed9ce4e94810beff8e0c93db3098b398df76c4')
...
foreach ($path in $baselineFrozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Units 001-029 baseline input: $path" }
    $want = $baselineFrozen[$path]
    $item = Get-Item -LiteralPath $path
    $digest = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($item.Length -ne $want[0] -or $digest -ne $want[1]) { throw "Frozen Units 001-029 baseline mismatch: $path" }
}
...
$authoritySpanText = ($authorityLines[133..6367] -join "`n") + "`n"
...
if ($authoritySpanBytes.Length -ne 328324 -or $authoritySpanDigest -ne 'a2c8d73d93c827d163ada44a1f79bda75f256df231db70624f2ac79bcfdf5f8c') {
    throw 'Frozen Notes.tex:134-6368 cumulative span mismatch.'
}
if ($authorityLines[6367] -notmatch '^\\end\{document\}$' -or $authorityLines.Count -ne 6369 -or $authorityLines[6368] -ne '') {
    throw 'Notes.tex line 6368 is not the terminal end-of-document boundary.'
}

$sources = @((Join-Path $lane 'source\id-ID\reader-unit-001.md')) + @(
    2..30 | ForEach-Object { Join-Path $lane ("source\id-ID\units\unit-{0:000}-lecture-{0:000}.md" -f $_) }
)
$sourceExpected = @(
    ...
    @(27687, 805, 'cfb8fa5c49593a187bed5df1d4173cc952100b18e5faa009cb8d57036c5726c4'),
    @(23008, 729, '88da8cf71d0f81328bdd65b0dea7d54c48655ed8836e230eaed821796b61b08d')
)
...
# Frozen Unit 030 reader and complete admission-evidence quartet.
$unit030Frozen = @{
    $unit030 = @(23008, '88da8cf71d0f81328bdd65b0dea7d54c48655ed8836e230eaed821796b61b08d')
    $unit030Audit = @(7214, '177c4306e5db636e0294e85278904c186099d069f209474a472d2615b0d5a4cf')
    $unit030Review = @(8406, '58db70bbd6538961e8bfc0c809d00b7b539115147b2826dc46d97e5b77ba712e')
    $unit030Qa = @(8378, 'bef6fe6704084ac02386bb477b7b0082e02921d3d722955e1366e7d0b9247753')
}
foreach ($path in $unit030Frozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Unit 030 gate input: $path" }
    $want = $unit030Frozen[$path]
    if ((Get-Item -LiteralPath $path).Length -ne $want[0] -or (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $want[1]) {
        throw "Unit 030 gate identity mismatch: $path"
    }
}
$reviewText = [IO.File]::ReadAllText($unit030Review)
if ($reviewText -notmatch '(?m)^- P1: 0\s*$' -or $reviewText -notmatch '(?m)^- P2: 0\s*$' -or
    $reviewText -notmatch '(?m)^- P3: 0\s*$' -or -not $reviewText.Contains('23,008 bytes, 729 LF lines') -or
    -not $reviewText.Contains('268 MathML nodes')) {
    throw 'Unit 030 independent review is not the zero-finding final snapshot.'
}
$qaText = [IO.File]::ReadAllText($unit030Qa)
if ($qaText -notmatch '"status"\s*:\s*"PASS"' -or $qaText -notmatch '"model_provenance"\s*:\s*"OpenAI Codex gpt-5\.6-sol, Ultra"' -or
    -not $qaText.Contains('88da8cf71d0f81328bdd65b0dea7d54c48655ed8836e230eaed821796b61b08d') -or
    -not $qaText.Contains('58db70bbd6538961e8bfc0c809d00b7b539115147b2826dc46d97e5b77ba712e')) {
    throw 'Unit 030 QA is not a passing, source-and-review-bound, model-attributed receipt.'
}
$unit030Text = [IO.File]::ReadAllText($unit030)
foreach ($marker in @('TODO', 'TBD', 'FILL_AFTER', 'C:\Users\', 'C:/Users/', 'github_pat_', 'ghp_', 'sk-proj_', 'access_token')) {
    if ($unit030Text.Contains($marker)) { throw "Private or placeholder marker in Unit 030: $marker" }
}
if (([regex]::Matches($unit030Text, '(?m)^[ \t]*::: \{')).Count -ne 40 -or
    ([regex]::Matches($unit030Text, '(?m)^[ \t]*:::\s*$')).Count -ne 40) { throw 'Unit 030 fenced-div census or balance mismatch.' }
if (([regex]::Matches($unit030Text, '\$\$')).Count % 2 -ne 0) { throw 'Unit 030 has unbalanced display math.' }
$unit030Ids = [regex]::Matches($unit030Text, '#(o012-[a-z0-9-]+)(?=[}\s])') | ForEach-Object { $_.Groups[1].Value }
if ($unit030Ids.Count -ne 47 -or @($unit030Ids | Sort-Object -Unique).Count -ne 47) { throw 'Unit 030 stable-ID census mismatch.' }
$blockKinds = 'aside|boundary|corollary|definition|equation|example|exercise|fact|figure|hint|lemma|proof|proposition|remark|solution|source-audit|source-margin|theorem'
$counts = @{}
foreach ($match in [regex]::Matches($unit030Text, "(?m)^[ \t]*::: \{\.(?<kind>$blockKinds)\s+#o012-")) {
...
if ($signature -ne 'aside=4,boundary=1,corollary=1,definition=2,exercise=6,figure=1,hint=6,lemma=1,proof=4,proposition=1,remark=1,solution=6,source-audit=3,theorem=3') {
    throw "Unit 030 semantic-block mismatch: $signature"
}
if (-not $unit030Text.Contains('Batas akhir sumber Roberts') -or
    -not $unit030Text.Contains('EOF setelah Notes.tex baris 6368')) {
    throw 'Unit 030 terminal source boundary marker is missing.'
}
...
$assembled = Join-Path $checkDir 'reader-units-001-030.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-30"
subtitle: "Homotopi, Ruang Penutup, Grup Fundamental, Kohomologi Simpleksial dan Singular, Mayer--Vietoris, Eksisi, Kompleks CW, Aksioma Eilenberg--Steenrod, Teorema Titik Tetap Brouwer, Teorema Dasar Aljabar, serta Medan Vektor Tangen"
author:
...
$htmlAssembled = Join-Path $checkDir 'reader-units-001-030-html.md'
...
$common = @(
    ...
    '--metadata=lang:id-ID', '--metadata=pagetitle:Topologi Aljabar - Unit 1-30',
...
$htmlA = Join-Path $checkDir 'units-001-030-a.html'
$htmlB = Join-Path $checkDir 'units-001-030-b.html'
...
$missingUnit030Ids = @($unit030Ids | Where-Object { -not $domIdSet.Contains($_) })
if ($missingUnit030Ids.Count -ne 0) { throw "Unit 030 IDs missing from HTML: $($missingUnit030Ids -join ', ')" }
...
$pdfAssembled = Join-Path $checkDir 'reader-units-001-030-pdf.md'
$inDisplayMath = $false
...
$pdfA = Join-Path $checkDir 'units-001-030-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-030-b.pdf'
$pdfHeader = Join-Path $checkDir 'reader-units-001-030-header.tex'
