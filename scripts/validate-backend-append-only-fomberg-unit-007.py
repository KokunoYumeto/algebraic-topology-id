#!/usr/bin/env python3
"""Read-only validator for the exact Fomberg Unit 007 append-only suffix."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 007 backend validator FAIL: {message}")


def load_producer():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u007_producer_for_validator", PRODUCER_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 007 producer")
...
def load_exact_partition(c, expected_suffixes: dict[str, bytes], counts: dict[str, int]):
    ...
    for name in c.FILES:
        ...
        require(
            ...
            f"{name}: immutable Unit 006 prefix partition mismatch",
        )
...
def main() -> int:
    ...
    print("Fomberg Unit 007 append-only backend validation: PASS")
