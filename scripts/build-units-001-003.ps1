...
$htmlDir = Join-Path $lane 'output\html\units-001-003'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-003-build'
...
$assembled = Join-Path $checkDir 'reader-units-001-003.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-3"
subtitle: "Ruang Topologis, Keterhubungan, Homotopi, dan Funktor"
author:
...
$payload = $header.Replace("`r`n", "`n") +
    ...
    (Get-MarkdownBody $unit3).TrimEnd("`n") + "`n"
...
$common = @(
    ...
    '--number-sections',
    '--metadata=lang:id-ID',
    '--metadata=pagetitle:Topologi Aljabar - Unit 1-3',
    '--strip-comments'
)
...
$pdfA = Join-Path $checkDir 'units-001-003-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-003-b.pdf'
$pdfArgs = @(
    '--pdf-engine=pdflatex',
...
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-003-id.pdf'
...
[IO.File]::WriteAllText((Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_003.csv'), $manifest, [Text.UTF8Encoding]::new($false))
