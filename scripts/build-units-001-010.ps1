...
$htmlDir = Join-Path $lane 'output\html\units-001-010'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-010-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-010-id.pdf'
$manifestPath = Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_010.csv'
...
$assembled = Join-Path $checkDir 'reader-units-001-010.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-10"
subtitle: "Ruang Topologis, Homotopi, Ruang Penutup, Grupoid Fundamental, Monodromi, dan Baji"
author:
...
$common = @(
    ...
    '--number-sections',
    '--metadata=lang:id-ID',
    '--metadata=pagetitle:Topologi Aljabar - Unit 1-10',
    '--strip-comments'
)

$htmlA = Join-Path $checkDir 'units-001-010-a.html'
$htmlB = Join-Path $checkDir 'units-001-010-b.html'
...
$pdfA = Join-Path $checkDir 'units-001-010-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-010-b.pdf'
$pdfHeader = Join-Path $checkDir 'reader-units-001-010-header.tex'
...
Remove-Item -LiteralPath $htmlA, $htmlB, $pdfA, $pdfB, $assembled, $semanticCss, $pdfHeader -Force
