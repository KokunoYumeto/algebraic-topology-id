...
$baselineBuilder = Join-Path $PSScriptRoot 'build-units-001-028.ps1'
$authority = Join-Path $lane 'authority\upstream\AlgebraicTopology2019-b947ad2e9f9e301bfe24590a9db653bc54fa1a53\Notes.tex'
$unit029 = Join-Path $lane 'source\id-ID\units\unit-029-lecture-029.md'
$unit029Qa = Join-Path $lane 'qa\UNIT_029_QA.json'
$unit029Review = Join-Path $lane 'qa\UNIT_029_INDEPENDENT_REVIEW.md'
$unit029Audit = Join-Path $lane 'qa\UNIT_029_SOURCE_AUDIT.md'
...
$htmlDir = Join-Path $lane 'output\html\units-001-029'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-029-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-029-id.pdf'
$manifestPath = Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_029.csv'

# Frozen Units 001-028 boundary. These inputs are verified but never invoked
# or rewritten by this cumulative builder.
$baselineFrozen = @{
    $baselineBuilder = @(22203, '3482b02eb1cbb3f3e9d49309689e82099f78839b25b4f21c5edeb51adce077fb')
    (Join-Path $lane 'output\html\units-001-028\index.html') = @(4624723, '30f4821960a310bfb6f457f05838b6d8355e2a6df5c35b3a3dcdd9c1afa0a13e')
    (Join-Path $lane 'output\pdf\topologi-aljabar-unit-001-028-id.pdf') = @(2159838, '9f3e1f7fcad396d3e654362ab232bdcd2497346cec6f0aacf7122e6450381657')
    (Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_028.csv') = @(249, 'a662f7dae7cb673eb8892dce255acd23d21dc17238a3f5c9528a0ee5087d94a8')
    (Join-Path $lane 'qa\UNITS_001_028_BUILD_RECEIPT.json') = @(10747, '9b2d7b7af8110092e155cfa95aeef9f353fd144b3dc213f67086dca45a78bece')
    (Join-Path $lane 'qa\UNITS_001_028_VISUAL_QA.md') = @(5457, 'bf85357e9d3120177ecaa8ee2643960217b6206e767eb8e5fff767101f25eb86')
    (Join-Path $lane 'qa\UNITS_001_028_RENDER_INVENTORY.csv') = @(2241, 'e2370f505a64dc15e7ba3913992227d24212f65e820aed8e3e980cf08d1eba4b')
...
foreach ($path in $baselineFrozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Units 001-028 baseline input: $path" }
    $want = $baselineFrozen[$path]
    $item = Get-Item -LiteralPath $path
    $digest = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($item.Length -ne $want[0] -or $digest -ne $want[1]) { throw "Frozen Units 001-028 baseline mismatch: $path" }
}
...
$authoritySpanText = ($authorityLines[133..6269] -join "`n") + "`n"
...
if ($authoritySpanBytes.Length -ne 320034 -or $authoritySpanDigest -ne '498b8a46bbd40f3abc1f6066adc00107eb23121d017eae0e2bbe0b9ba280b987') {
    throw 'Frozen Notes.tex:134-6270 cumulative span mismatch.'
}
if ($authorityLines[6270] -notmatch '\\lecturenum\{30\}') { throw 'Notes.tex line 6271 is not the Unit 30 boundary.' }

$sources = @((Join-Path $lane 'source\id-ID\reader-unit-001.md')) + @(
    2..29 | ForEach-Object { Join-Path $lane ("source\id-ID\units\unit-{0:000}-lecture-{0:000}.md" -f $_) }
)
$sourceExpected = @(
...
# Frozen Unit 029 reader and complete admission-evidence quartet.
$unit029Frozen = @{
    $unit029 = @(27687, 'cfb8fa5c49593a187bed5df1d4173cc952100b18e5faa009cb8d57036c5726c4')
    $unit029Audit = @(5738, '6b3e96ca5a7d24a4f8182c46f02e99194c9538ca534b4b550ea23b666ad89afb')
    $unit029Review = @(8535, '873c21354acc83050e13f7236b361f29523b91d4907d9b0046ee061f7723547c')
    $unit029Qa = @(11192, '66f5709282d890e24a20756b69d287305497011c8f6e39d7108a30d5b4b1ffd9')
}
foreach ($path in $unit029Frozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Unit 029 gate input: $path" }
    $want = $unit029Frozen[$path]
    if ((Get-Item -LiteralPath $path).Length -ne $want[0] -or (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $want[1]) {
        throw "Unit 029 gate identity mismatch: $path"
    }
}
$reviewText = [IO.File]::ReadAllText($unit029Review)
if (-not $reviewText.Contains('P1: 0, P2: 0, P3: 0') -or -not $reviewText.Contains('27.687 byte') -or
    -not $reviewText.Contains('258 simpul MathML')) {
    throw 'Unit 029 independent review is not the zero-finding final snapshot.'
}
$qaText = [IO.File]::ReadAllText($unit029Qa)
if ($qaText -notmatch '"status"\s*:\s*"PASS"' -or $qaText -notmatch '"model_provenance"\s*:\s*"OpenAI Codex gpt-5\.6-sol, Ultra"' -or
    -not $qaText.Contains('cfb8fa5c49593a187bed5df1d4173cc952100b18e5faa009cb8d57036c5726c4')) {
    throw 'Unit 029 QA is not a passing, source-bound, model-attributed receipt.'
}
$unit029Text = [IO.File]::ReadAllText($unit029)
foreach ($marker in @('TODO', 'TBD', 'FILL_AFTER', 'C:\Users\', 'C:/Users/', 'github_pat_', 'ghp_', 'sk-proj_', 'access_token')) {
    if ($unit029Text.Contains($marker)) { throw "Private or placeholder marker in Unit 029: $marker" }
}
if (([regex]::Matches($unit029Text, '(?m)^[ \t]*::: \{')).Count -ne 40 -or
    ([regex]::Matches($unit029Text, '(?m)^[ \t]*:::\s*$')).Count -ne 40) { throw 'Unit 029 fenced-div census or balance mismatch.' }
if (([regex]::Matches($unit029Text, '\$\$')).Count % 2 -ne 0) { throw 'Unit 029 has unbalanced display math.' }
$unit029Ids = [regex]::Matches($unit029Text, '#(o012-[a-z0-9-]+)(?=[}\s])') | ForEach-Object { $_.Groups[1].Value }
if ($unit029Ids.Count -ne 49 -or @($unit029Ids | Sort-Object -Unique).Count -ne 49) { throw 'Unit 029 stable-ID census mismatch.' }
...
$counts = @{}
foreach ($match in [regex]::Matches($unit029Text, "(?m)^[ \t]*::: \{\.(?<kind>$blockKinds)\s+#o012-")) {
...
if ($signature -ne 'boundary=1,corollary=1,definition=2,example=3,exercise=6,fact=1,figure=1,hint=6,lemma=1,proof=4,solution=6,source-audit=6,theorem=2') {
    throw "Unit 029 semantic-block mismatch: $signature"
}
if (-not $unit029Text.Contains('data-source-label="eq:comparison_iso_simplicial_singular"') -or
    -not $unit029Text.Contains('Baris 6271')) {
    throw 'Unit 029 comparison-isomorphism alias or terminal cursor marker is missing.'
}
...
$assembled = Join-Path $checkDir 'reader-units-001-029.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-29"
subtitle: "Homotopi, Ruang Penutup, Grup Fundamental, Kohomologi Simpleksial dan Singular, Kohomologi Relatif dan Tereduksi, Mayer--Vietoris, Kohomologi Sfera, Eksisi, Baji Sfera, Teorema Perbandingan, Kompleks CW, serta Aksioma Eilenberg--Steenrod"
author:
...
$htmlAssembled = Join-Path $checkDir 'reader-units-001-029-html.md'
...
$common = @(
    ...
    '--metadata=lang:id-ID', '--metadata=pagetitle:Topologi Aljabar - Unit 1-29',
...
$htmlA = Join-Path $checkDir 'units-001-029-a.html'
$htmlB = Join-Path $checkDir 'units-001-029-b.html'
...
$missingUnit029Ids = @($unit029Ids | Where-Object { -not $domIdSet.Contains($_) })
if ($missingUnit029Ids.Count -ne 0) { throw "Unit 029 IDs missing from HTML: $($missingUnit029Ids -join ', ')" }
...
$pdfAssembled = Join-Path $checkDir 'reader-units-001-029-pdf.md'
$inDisplayMath = $false
...
$pdfA = Join-Path $checkDir 'units-001-029-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-029-b.pdf'
$pdfHeader = Join-Path $checkDir 'reader-units-001-029-header.tex'
