#!/usr/bin/env python3
"""Preflight, live validation, and exact replay for the Lab 2 backend append."""
...
QA = ROOT / "qa"
OUTPUTS = {
    "plan": QA / "BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_PLAN.json",
    "semantic": QA / "BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_RECEIPT.json",
    "replay": QA / "BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_REPLAY_RECEIPT.json",
    "manifest": QA / "BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_FILE_MANIFEST.csv",
    "cumulative": QA / "BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_CUMULATIVE_RECEIPT.json",
}


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Lab 2 backend validator FAIL: {message}")
...
def load_producer():
    path = ROOT / "scripts/extend-backend-computation-lab-002.py"
    spec = importlib.util.spec_from_file_location("o012_lab02_backend_producer", path)
    require(spec is not None and spec.loader is not None, "cannot load Lab 2 producer")
...
def partition_live(producer, expected_suffix: dict[str, bytes], require_suffix: bool) -> tuple[dict[str, bytes], dict[str, bytes], dict[str, bytes]]:
    ...
    for name in producer.FILES:
        path = producer.BACKEND / name
        require(path.is_file(), f"live backend file missing: {name}")
        raw = path.read_bytes()
...
def build_state(producer) -> dict[str, Any]:
    producer.verify_baseline_receipt()
...
def preflight(producer) -> dict[str, Any]:
    ...
    receipt = {
        ...
        "baseline_receipt": state["plan"]["baseline_receipt"],
        "delta": {
...
def validate_live(producer) -> dict[str, Any]:
    ...
    scratch = Path(tempfile.mkdtemp(prefix="lab02-", dir=scratch_parent))
    try:
        for name in producer.FILES:
    ...
    cumulative = {
        ...
        "cumulative": {"records": final[0], "bytes": final[1], "bundle_sha256": final[2], "computation_laboratories_complete": 2, "computation_laboratories_required": 4},
        "files": file_rows,
        "semantic_checks": semantic,
