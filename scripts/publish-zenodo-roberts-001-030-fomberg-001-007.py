#!/usr/bin/env python3
"""Publish the sealed Roberts 001-030 + Fomberg 001-007 checkpoint.

The transaction is locked to Zenodo concept DOI 10.5281/zenodo.22061489 and
public predecessor 22102865 (version 0.30.6).  It creates one new version,
never a new concept.  It uploads the exact reader-first nine-file payload,
publishes it, and anonymously reads back every file while verifying byte count
and SHA-256.  The credential is read only at transaction time by the inherited
publisher and is never written to output or receipts.
"""
...
SPEC = importlib.util.spec_from_file_location("o012_fomberg_006_publisher", BASE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved Fomberg 001-006 publisher")
...
class _ZenodoTLSAdapter(HTTPAdapter):
    """Use an explicit verified TLS context for the Zenodo transaction.

    The host's certificate and the standard-library HTTPS client verify cleanly,
    but this workstation's default ``requests`` adapter currently misapplies
    hostname validation.  Supplying the context and expected host explicitly
    keeps certificate-chain and hostname verification enabled.
    """

    def init_poolmanager(self, connections, maxsize, block=False, **pool_kwargs):
        pool_kwargs["ssl_context"] = ssl.create_default_context()
        pool_kwargs["assert_hostname"] = "zenodo.org"
        return super().init_poolmanager(
            connections, maxsize, block=block, **pool_kwargs
        )

    def send(self, request, **kwargs):
        # A TLS hostname failure occurs before any HTTP request bytes are sent,
        # so discarding that connection and retrying cannot duplicate a Zenodo
        # mutation.  Never retry any other transport or HTTP failure here.
        last_error = None
        for attempt in range(12):
            try:
                return super().send(request, **kwargs)
            except base.requests.exceptions.SSLError as error:
                message = str(error)
                if "hostname" not in message and "Hostname mismatch" not in message:
                    raise
                last_error = error
...
class _ZenodoSession(base.requests.Session):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.mount("https://zenodo.org/", _ZenodoTLSAdapter())


_ZENODO_ANONYMOUS_SESSION = _ZenodoSession()
base.requests.Session = _ZenodoSession
base.requests.get = _ZENODO_ANONYMOUS_SESSION.get

base.RELEASE = (
    base.LANE / "release" / "zenodo-roberts-001-030-fomberg-001-007"
)
...
base.SEED_RECORD = 22102865
base.CONCEPT_DOI = "10.5281/zenodo.22061489"
base.MODEL_NOTE = "OpenAI Codex gpt-5.6-sol, Ultra"
engine.PREVIOUS_RECORD_ID = 22102865
engine.PREVIOUS_DOI = "10.5281/zenodo.22102865"
base.RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-007-v0.30.7"
base.TITLE = (
...
base.FILE_NAMES = [
    base.PDF_NAME,
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_007_READER.html",
    (
...
def assert_organization_rule(metadata: dict) -> None:
    ...
    if occurrences != 1 or semantic_contributors != EXPECTED_CONTRIBUTORS:
        raise RuntimeError("organization must appear exactly once as contributor")
...
def verify_local() -> tuple[dict, dict]:
    ...
    if not (
        ...
        and fomberg.get("course_route_unit_ids")
        == ["D60-R08", "D60-R09", "D60-R10", "D60-R11", "D60-R12", "D60-R12", "D60-R12"]
        and fomberg.get("component_boundaries")
        == {
            "O012-FOM-001": "31-614",
    ...
    if (
        ...
        raise RuntimeError("publication controls remain intentionally unsealed")
    if (
        plan.get("release_id") != base.RELEASE_ID
    ...
    if (
        binding.get("final_identity_source")
...
def verify_public(
    ...
    for marker in (
        ...
        "O012-FOM-001–007",
        "O012-FOM-001 sampai O012-FOM-007",
        "baris 31–4185",
        "baris 4186",
        "CC BY 4.0",
        "CC BY-SA 4.0",
...
def make_receipt(
    ...
    return {
        ...
        "scope": (
            ...
            "Fomberg algebraic_topology.tex lines 31-4185, Sections 1.1-1.13, "
            "O012-FOM-001/002/003/004/005/006/007 complete; next line 4186; "
