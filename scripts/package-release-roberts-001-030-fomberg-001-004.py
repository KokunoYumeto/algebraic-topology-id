#!/usr/bin/env python3
"""Prepare the Roberts 001-030 + Fomberg 001-004 Zenodo payload.
...
BASE_PATH = SCRIPT.with_name(
    "package-release-roberts-001-030-fomberg-001-003.py"
)
SPEC = importlib.util.spec_from_file_location("o012_fomberg_003_packager", BASE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved Fomberg 001-003 packager")
previous = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(previous)
base = previous.prior

LANE = SCRIPT.parents[1]
RELEASE = LANE / "release" / "zenodo-roberts-001-030-fomberg-001-004"
...
TITLE = (
    ...
    "Jembatan Homologi §1.1–1.10"
)
VERSION = "0.30.4"
RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-004-v0.30.4"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22088708
PREVIOUS_DOI = "10.5281/zenodo.22088708"
...
# Replace by apply_patch only after every required input exists, all final
# receipts pass, and frozen-inputs.json has been independently checked.
FROZEN_LEDGER_SHA256 = "7320001bc1f6c2391208739a167fd724ae08b4fa926cb917ee3248bd5ce02207"

PDF_INPUT = "output/pdf/topologi-aljabar-roberts-001-030-fomberg-001-004-id.pdf"
HTML_INPUT = "output/html/roberts-001-030-fomberg-001-004/index.html"
ARTIFACT_MANIFEST = "output/ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_004.csv"
BUILD_RECEIPT = "qa/ROBERTS_001_030_FOMBERG_001_004_BUILD_RECEIPT.json"
VISUAL_QA = "qa/ROBERTS_001_030_FOMBERG_001_004_VISUAL_QA.md"
RENDER_INVENTORY = "qa/ROBERTS_001_030_FOMBERG_001_004_RENDER_INVENTORY.csv"
BACKEND_CUMULATIVE_JSON = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_CUMULATIVE_RECEIPT.json"
)
BACKEND_CUMULATIVE_MD = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_CUMULATIVE_RECEIPT.md"
)
FOMBERG_QA_004 = "qa/FOMBERG_UNIT_004_QA.json"
FOMBERG_SOURCE_AUDIT_004 = "qa/FOMBERG_UNIT_004_SOURCE_AUDIT.json"
FOMBERG_REVIEW_004 = "qa/fomberg-unit-004/INDEPENDENT_REVIEW_FINAL.json"
FOMBERG_READER_004 = (
    "source/id-ID/fomberg/units/"
    "fomberg-unit-004-excision-mayer-vietoris-naturality-comparison.md"
)
FOMBERG_ASSETS_004 = tuple(
    f"source/id-ID/fomberg/assets/unit-004/{name}"
    for name in (
...
PDF_NAME = (
    ...
    "FOMBERG_001_004_READER.pdf"
)
HTML_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_"
    "FOMBERG_001_004_READER.html"
)
SOURCE_ZIP_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_004_"
...
def source_entries() -> dict[str, Path]:
    """Compact resumable source/backend inventory, including Unit004 once."""
    entries = _previous_source_entries()
    entries.update(
        {
            FOMBERG_READER_004: base.lane_path(FOMBERG_READER_004),
            **{
                relative: base.lane_path(relative)
                for relative in FOMBERG_ASSETS_004
            },
            "scripts/build-roberts-001-030-fomberg-001-004.ps1": (
                LANE / "scripts" / "build-roberts-001-030-fomberg-001-004.ps1"
            ),
            "scripts/fomberg-unit-004-common.py": (
                LANE / "scripts" / "fomberg-unit-004-common.py"
            ),
            "scripts/qa-fomberg-unit-004.py": (
                LANE / "scripts" / "qa-fomberg-unit-004.py"
            ),
            "scripts/extend-backend-fomberg-unit-004.py": (
                LANE / "scripts" / "extend-backend-fomberg-unit-004.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-004.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-004.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-004-cumulative.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-004-cumulative.py"
            ),
            "scripts/package-release-roberts-001-030-fomberg-001-004.py": SCRIPT,
            "scripts/publish-zenodo-roberts-001-030-fomberg-001-004.py": (
                LANE
                / "scripts"
                / "publish-zenodo-roberts-001-030-fomberg-001-004.py"
            ),
        }
    )
    return entries
...
def source_path_inventory() -> set[str]:
    ...
    paths.remove("scripts/package-release-roberts-001-030-fomberg-001-004.py")
    return paths
...
def qa_path_inventory() -> set[str]:
    ...
    paths.update(
        {
            FOMBERG_QA_004,
            FOMBERG_SOURCE_AUDIT_004,
            FOMBERG_REVIEW_004,
            "qa/fomberg-unit-004/TERMINOLOGY_ROWS_DRAFT.csv",
            "qa/fomberg-unit-004/ADVERSE_ROWS_DRAFT.csv",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_FILE_MANIFEST.csv",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_RECEIPT.json",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_RECEIPT.md",
            BACKEND_CUMULATIVE_JSON,
            BACKEND_CUMULATIVE_MD,
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_REPLAY_RECEIPT.json",
            BUILD_RECEIPT,
            VISUAL_QA,
            RENDER_INVENTORY,
        }
...
def release_control_inventory() -> set[str]:
    prefix = "release/zenodo-roberts-001-030-fomberg-001-004/"
    return {
        prefix + name
        for name in (
            "metadata.json",
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[str]]:
    ...
    if re.search(
        ...
        r"(?:0*1\s*[-–]\s*0*3[1-4]|0*3[1-4]\s*/\s*0*3[1-4])",
        joined,
    ):
        raise RuntimeError("Fomberg was incorrectly renumbered as Roberts 31-34")
    for marker in (
        "30/30",
        "O012-FOM-001",
        "O012-FOM-002",
    ...
    if (
        ...
        or backend_binding.get("expected_records") <= 5747
    ...
    if not isinstance(readers["pdf"].get("pages"), int) or readers["pdf"]["pages"] <= 397:
    ...
    if (
        ...
        != "roberts_complete_fomberg_units_001_004_complete_composite_course_partial"
    ):
    ...
    if (
        ...
        != ["O012-FOM-001", "O012-FOM-002", "O012-FOM-003", "O012-FOM-004"]
        or numbering.get("course_route_unit_ids")
        != ["D60-R08", "D60-R09", "D60-R10", "D60-R11"]
        or numbering.get("fomberg_is_not_roberts_units_031_034") is not True
    ):
...
def verify_build_receipt(artifact_rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if (
        ...
        != "O012-RBT-001-030-FOM-001-004-COMPOSITE-BUILD"
    ):
    ...
    if (
        ...
        or fomberg.get("selected_source_span") != "31-2846"
        or fomberg.get("selected_span_lines") != 2816
        or fomberg.get("next_source_line") != 2847
...
def verify_fomberg_qa() -> dict[str, Any]:
    qa = json.loads(base.lane_path(FOMBERG_QA_004).read_text(encoding="utf-8"))
    if (
        qa.get("status") != "PASS"
        or qa.get("qa_id") != "O012-FOMBERG-UNIT-004-STATIC-QA"
        or qa.get("model_provenance") != MODEL_NOTE
    ):
        raise RuntimeError("Fomberg Unit004 static QA did not pass")
    source = qa.get("source", {})
    span = source.get("selected_span", {})
    if (
        source.get("commit") != FOMBERG_COMMIT
        or source.get("tree") != FOMBERG_TREE
        or source.get("next_line") != 2847
        or source.get("next_line_text") != r"\subsection{Degree maps}"
        or span.get("line_start") != 1923
        or span.get("line_end") != 2846
        or span.get("lf_lines") != 924
        or span.get("sha256")
        != "ddde995b54154623ccc565117aee63cce8361d2ada1c3c9f2852775b1aaac638"
    ):
        raise RuntimeError("Fomberg Unit004 source boundary mismatch")
    reader = qa.get("reader", {})
    if not base.identity_matches(reader.get("identity"), base.lane_path(FOMBERG_READER_004)):
        raise RuntimeError("Fomberg QA does not bind translated Unit004")
    if (
        reader.get("stable_ids") != 117
        or reader.get("stable_ids_unique") != 117
        or reader.get("mastery", {}).get("exercise_hint_solution_triples") != 7
        or reader.get("assets", {}).get("raster_redraws") != 3
        or len(reader.get("assets", {}).get("files", [])) != 6
        or reader.get("proof_repairs")
        != [
            "FOM-PR-05",
            "FOM-PR-06",
            "FOM-PR-07",
        ...
        raise RuntimeError("Fomberg Unit004 structure/mastery/proof closure drift")
    if any(value != "PASS" for value in qa.get("gates", {}).values()):
        raise RuntimeError("Fomberg Unit004 QA gate is incomplete")
    if qa.get("independent_review", {}).get("severity_census") != {
        "P1": 0,
        "P2": 0,
        "P3": 0,
    }:
        raise RuntimeError("Fomberg Unit004 independent review is not zero-severity")
    for row in reader.get("assets", {}).get("files", []):
        if row.get("path") not in FOMBERG_ASSETS_004 or not base.identity_matches(
            row, base.lane_path(row["path"])
        ):
            raise RuntimeError("Fomberg Unit004 asset identity drift")
    if {row.get("path") for row in reader["assets"]["files"]} != set(FOMBERG_ASSETS_004):
        raise RuntimeError("Fomberg Unit004 QA does not bind all six assets exactly once")
    if not base.identity_matches(qa.get("source_audit"), base.lane_path(FOMBERG_SOURCE_AUDIT_004)):
        raise RuntimeError("Fomberg Unit004 source-audit identity drift")
    if not base.identity_matches(
        qa.get("independent_review", {}).get("identity"),
        base.lane_path(FOMBERG_REVIEW_004),
    ):
        raise RuntimeError("Fomberg Unit004 independent-review identity drift")
    return qa
...
def verify_backend_receipt(backend_facts: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        != "O012-BACKEND-THROUGH-FOMBERG-UNIT-004-CUMULATIVE-SEMANTIC"
    ):
        raise RuntimeError("Fomberg Unit004 cumulative backend receipt failed")
...
def write_package_receipt(
    ...
    payload = {
        ...
        "scope": (
            ...
            "O012-FOM-004 (Sections 1.1-1.10, lines 31-2846) complete; "
            "next Fomberg line 2847; composite course partial"
        ),
        ...
        manifest.update(
            {
                ...
                "reader_qa": {
                    ...
                    "fomberg_stable_ids_total": 424,
                    "fomberg_unit_004_stable_ids": fomberg_qa["reader"]["stable_ids"],
                    "mastery_triples_total": 25,
                    "fomberg_unit_004_mastery_triples": fomberg_qa["reader"]["mastery"]
                    ["exercise_hint_solution_triples"],
                    "fomberg_unit_004_redraw_assets": len(
                        fomberg_qa["reader"]["assets"]["files"]
                    ),
        ...
        json.dumps(
            {
                ...
                "release_directory": RELEASE.relative_to(LANE).as_posix(),
                "scope": (
                    "Roberts 30/30 + Fomberg O012-FOM-001/002/003/004 "
                    "through line 2846; next line 2847; composite course partial"
                ),
                "files": [
                    {
                        "filename": name,
