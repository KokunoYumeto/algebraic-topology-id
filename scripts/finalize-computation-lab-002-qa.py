#!/usr/bin/env python3
"""Seal execution and independent-review closure for O012/D60 Lab 2."""
...
STATIC = ROOT / "qa/computation-lab-002/STATIC_QA.json"
CODE_REVIEW = ROOT / "qa/computation-lab-002/INDEPENDENT_CODE_REVIEW.json"
MATH_REVIEW = ROOT / "qa/computation-lab-002/INDEPENDENT_MATH_REVIEW.json"
LANGUAGE_REVIEW = ROOT / "qa/computation-lab-002/INDEPENDENT_SOURCE_LANGUAGE_REVIEW.json"
EXECUTION = ROOT / "qa/computation-lab-002/EXECUTION_RECEIPT.json"
COMBINED = ROOT / "qa/COMPUTATION_LAB_002_QA.json"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Lab 2 QA finalizer FAIL: {message}")
...
def verify_bound(review: dict[str, Any], current: dict[str, dict[str, Any]]) -> None:
    bound = {item["path"]: item for item in review.get("bound_artifacts", [])}
    require(bound, "review has no bound artifacts")
    for relative, item in bound.items():
        require(relative in current, f"review binds an undeclared input: {relative}")
...
def main() -> int:
    learner_paths = (SOURCE, PROGRAM, TESTS, EXPECTED)
    static_inputs = (*learner_paths, TERMINOLOGY, PREDECESSOR)
    review_paths = (CODE_REVIEW, MATH_REVIEW, LANGUAGE_REVIEW)
    all_inputs = (*static_inputs, STATIC, *review_paths)
    current = {item["path"]: item for item in (identity(path) for path in all_inputs)}

    static = json.loads(disciplined(STATIC).decode("utf-8"))
    require(static.get("status") == "PASS", "static QA is not PASS")
    require(static.get("laboratory_id") == "D60-LAB02", "static QA laboratory mismatch")
    require(static.get("course_route_unit_ids") == ["D60-R08"], "static route mapping drift")
    require(static.get("severity_counts") == {"P1": 0, "P2": 0, "P3": 0}, "static findings remain")
    require(static.get("structure", {}).get("stable_ids") == 25, "stable-ID census drift")
    require(static.get("structure", {}).get("tasks") == 6, "task census drift")
    for item in static.get("inputs", []):
        relative = item.get("path")
        require(relative in current, f"static QA binds unexpected input: {relative}")
        observed = current[relative]
        line_count = item.get("lines", item.get("lf_lines"))
        require(
            (item.get("bytes"), line_count, item.get("sha256"))
            == (observed["bytes"], observed["lf_lines"], observed["sha256"]),
    ...
    for path, kind in expected_review_kinds.items():
        receipt = json.loads(disciplined(path).decode("utf-8"))
        ...
        require(receipt.get("laboratory_id") == "D60-LAB02", f"{kind} laboratory mismatch")
        require(receipt.get("independent_from_production") is True, f"{kind} review is not independent")
    ...
    learner_identities = [current[path.relative_to(ROOT).as_posix()] for path in learner_paths]
    execution_receipt = {
        "schema_version": "1.0",
        "status": "PASS",
        ...
        "learner_artifacts": learner_identities,
        "model_provenance": MODEL,
        "severity_census": {"P1": 0, "P2": 0, "P3": 0},
    }
    write_json(EXECUTION, execution_receipt)
    ...
    require(not any(marker in joined for marker in private_markers), "credential or private-path marker detected")
    ...
    combined = {
        ...
        "checks": {
            ...
            "route_scope_D60_R08_only": "PASS",
            "terminology_0493_0502": "PASS",
    ...
    print(json.dumps({
        "status": "PASS",
        "laboratory_id": "D60-LAB02",
        "execution_receipt": identity(EXECUTION),
        "combined_receipt": identity(COMBINED),
