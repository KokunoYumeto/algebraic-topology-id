#!/usr/bin/env python3
"""Publish the prepared Units 001-024 payload as the next Zenodo version.
...
FILE_NAMES = [
    PDF_NAME,
    "TOPOLOGI_ALJABAR_ID_UNITS_001_024_READER.html",
    "TOPOLOGI_ALJABAR_ID_UNITS_001_024_EDITABLE_SOURCE_BACKEND.zip",
    "TOPOLOGI_ALJABAR_ID_UNITS_001_024_QA_PROVENANCE.zip",
    "README_RELEASE.md",
    "RELEASE_RIGHTS.md",
...
def verify_local() -> tuple[dict, dict]:
    ...
    if "Kuliah 25–30" not in public_text or "belum" not in public_text:
        raise RuntimeError("partial-scope language is missing")
    if "Notes.tex" not in public_text or "134-5369" not in public_text.replace("–", "-"):
...
def make_receipt(public: dict, rows: list[dict], metadata_payload: dict, local: dict) -> dict:
    ...
    return {
        ...
        "scope": "Roberts Notes.tex lines 134-5369, lectures 1-24 of 30; lectures 25-30, Fomberg bridge, and original proof/mastery/lab/capstone closure pending",
        "record_id": record_id,
        "doi": public.get("doi"),
