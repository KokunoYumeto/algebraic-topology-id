#!/usr/bin/env python3
"""Read-only validator for the exact Fomberg Unit 005 append-only suffix."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 005 backend validator FAIL: {message}")


def load_producer():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u005_producer_for_validator", PRODUCER_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 005 producer")
...
def load_exact_partition(c, expected_suffixes: dict[str, bytes]):
    ...
    for name in c.FILES:
        ...
        require(
            ...
            f"{name}: immutable Unit 004 prefix partition mismatch",
        )
        ...
        file_results[name] = {
            ...
            "prefix_sha256": prefix_sha, "records_added": c.DELTA[name],
    ...
    require(final_count == c.PREFIX_TOTAL[0] + sum(c.DELTA.values()) == 6309, "cumulative record count mismatch")
...
def main() -> int:
    ...
    require(sum(len(records) for records in additions.values()) == 196, "common module did not reconstruct exactly 196 records")
    ...
    for name in c.FILES:
        ...
        require((BACKEND / name).read_bytes()[c.PREFIX[name][1]:] == refreshed_suffix, f"{name}: live suffix/evidence binding changed during validation")
    print("Fomberg Unit 005 append-only backend validation: PASS")
