#!/usr/bin/env python3
"""Read-only validator for the exact Fomberg Unit 003 append-only suffix."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 003 backend validator FAIL: {message}")


def load_producer():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u003_producer_for_validator", PRODUCER_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 003 producer")
...
def load_exact_partition(c, expected_suffixes: dict[str, bytes]):
    ...
    for name in c.FILES:
        ...
        require(
            ...
            f"{name}: immutable Unit 002 prefix partition mismatch",
        )
    ...
    require(final_count == c.PREFIX_TOTAL[0] + sum(c.DELTA.values()) == 5747, "cumulative record count mismatch")
...
def main() -> int:
    ...
    require(sum(len(records) for records in additions.values()) == 405, "common module did not reconstruct exactly 405 records")
    ...
    print("Fomberg Unit 003 append-only backend validation: PASS")
