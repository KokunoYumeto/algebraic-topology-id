#!/usr/bin/env python3
"""Replay Unit 005 and write deterministic semantic/cumulative backend receipts."""
...
PRODUCER_PATH = LANE / "scripts/extend-backend-fomberg-unit-005.py"
VALIDATOR_PATH = LANE / "scripts/validate-backend-append-only-fomberg-unit-005.py"
OUTPUTS = {
    "manifest": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_FILE_MANIFEST.csv",
    "semantic_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_RECEIPT.json",
    "semantic_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_RECEIPT.md",
    "cumulative_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_CUMULATIVE_RECEIPT.json",
    "cumulative_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_CUMULATIVE_RECEIPT.md",
    "replay_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_005_REPLAY_RECEIPT.json",
}
ROBERTS_BOUNDARY = (
    4761, 5213679,
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 005 cumulative validator FAIL: {message}")
...
def replay_producer(producer, c) -> tuple[int, int, str]:
    with tempfile.TemporaryDirectory(prefix="o012-fom-u005-backend-replay-") as temporary:
        ...
        require("Fomberg Unit 005 semantic backend extension: PASS" in captured.getvalue(), "producer replay PASS marker absent")
        bundle = hashlib.sha256(); records = total_bytes = 0
        for name in c.FILES:
...
def write_outputs(c, identities, rows, final, script_results) -> None:
    ...
    for row in rows:
        ...
        writer.writerow({
            ...
            "records_added": c.DELTA[name], "final_records": row["records"],
    ...
    bound_inputs = {}
    for key, path in (
        ("common", LANE / "scripts/fomberg-unit-005-common.py"),
    ...
    semantic_md = (
        "# Fomberg Unit 005 append-only backend receipt\n\n"
        "Status: **PASS**\n\n"
        f"- Immutable Unit 004 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / {c.PREFIX_TOTAL[2]}.\n"
        f"- Unit 005 semantic suffix: {sum(c.DELTA.values())} records, including 52 unit and 52 segment records.\n"
        ...
        "- Reader: 52 stable IDs; two source TikZ-CD occurrences; one semantic figure block; nine resolved links; mastery: six solved triples.\n"
        "- FOM-PR-12 is complete, including choice independence and local-to-global degree. Cursor: source line 3123; source EOF: false.\n"
    ).encode("utf-8")
    OUTPUTS["semantic_md"].write_bytes(semantic_md)
    semantic = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-005-SEMANTIC-APPEND-ONLY",
        ...
        "closure": {
            "stable_ids": 52, "unit_records": 52, "segment_records": 52,
            "source_diagrams": 2, "semantic_figure_blocks": 1,
            "redraw_files": 0, "mastery_triples": 6,
            "proof_repairs": ["FOM-PR-12"], "fragment_links": 9,
            "distinct_fragment_targets": 8, "next_source_line": 3123,
    ...
    cumulative_md = (
        "# Cumulative backend receipt through Fomberg Unit 005\n\n"
        "Status: **PASS**\n\n"
        f"- Preserved Fomberg Unit 004 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / {c.PREFIX_TOTAL[2]}.\n"
        f"- Fomberg Unit 005 append-only suffix: {sum(c.DELTA.values())} records.\n"
        f"- Cumulative backend: {final[0]:,} records / {final[1]:,} bytes / {final[2]}.\n"
        "- Fomberg Units 001–005: 476 stable reader IDs mirrored one-to-one as unit and segment records; 79 source diagrams; 70 semantic figure blocks; 31 solved mastery triples.\n"
        "- Closed proof repairs: FOM-PR-01 through FOM-PR-12. Cursor: source line 3123; source EOF: false.\n"
        "- Deterministic producer replay and semantic append-only validation: **PASS**.\n"
    ).encode("utf-8")
    OUTPUTS["cumulative_md"].write_bytes(cumulative_md)
    cumulative = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-THROUGH-FOMBERG-UNIT-005-CUMULATIVE-SEMANTIC",
        "status": "PASS",
        "scope": "Roberts Units 001-030 immutable prefix plus Fomberg Units 001-005 append-only semantic suffixes",
        ...
        "nested_immutability": {
            ...
            "fomberg_unit_002_boundary": {
                "records": FOMBERG_UNIT_002_BOUNDARY[0], "bytes": FOMBERG_UNIT_002_BOUNDARY[1],
                "bundle_sha256": FOMBERG_UNIT_002_BOUNDARY[2], "preserved_byte_for_byte": True,
            },
            "fomberg_unit_003_boundary": {
                "records": FOMBERG_UNIT_003_BOUNDARY[0], "bytes": FOMBERG_UNIT_003_BOUNDARY[1],
                "bundle_sha256": FOMBERG_UNIT_003_BOUNDARY[2], "preserved_byte_for_byte": True,
            },
            ...
            "fomberg_unit_005_suffix": {"records": sum(c.DELTA.values()), "records_by_file": c.DELTA},
        },
        "current": {
            "total_records": final[0], "total_bytes": final[1],
            "bundle_sha256": final[2], "backend_files": backend_files,
            "stable_ids": 476, "unit_records": 476, "segment_records": 476,
            "source_diagrams": 79, "semantic_figure_blocks": 70,
            "redraw_files": 18, "mastery_triples": 31,
            "proof_repairs_closed": [f"FOM-PR-{number:02d}" for number in range(1, 13)],
            "next_source_line": 3123, "terminal_source_eof": False,
            "review_counts": {"P1": 0, "P2": 0, "P3": 0},
        },
    ...
    replay = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-005-DETERMINISTIC-REPLAY",
        "status": "PASS",
...
def main() -> int:
    ...
    producer = load(PRODUCER_PATH, "o012_fomberg_u005_producer_cumulative")
    c = producer.load_common(); identities = c.sealed_identities()
    result = subprocess.run(
        [sys.executable, "-B", str(VALIDATOR_PATH)], cwd=LANE,
        capture_output=True, text=True, encoding="utf-8",
    )
    require(result.returncode == 0, "semantic validator failed:\n" + result.stdout + result.stderr)
    require("Fomberg Unit 005 append-only backend validation: PASS" in result.stdout, "semantic validator PASS marker absent")
    rows, final = backend_snapshot(c)
    require(final[0] == 6309, "unexpected final record total")
    ...
    print("Cumulative backend validation through Fomberg Unit 005: PASS")
    print(f"unit004_prefix_records={c.PREFIX_TOTAL[0]}")
    print(f"unit005_records_added={sum(c.DELTA.values())}")
