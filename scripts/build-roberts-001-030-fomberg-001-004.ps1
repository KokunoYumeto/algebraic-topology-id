...
$htmlDir = Join-Path $lane 'output\html\roberts-001-030-fomberg-001-004'
$pdfDir = Join-Path $lane 'output\pdf'
$scratch = Join-Path $lane 'tmp\pdfs\roberts-001-030-fomberg-001-004-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-roberts-001-030-fomberg-001-004-id.pdf'
$manifest = Join-Path $lane 'output\ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_004.csv'
$receiptTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_004_BUILD_RECEIPT.json'
$visualQaTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_004_VISUAL_QA.md'
$renderInventoryTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_004_RENDER_INVENTORY.csv'
...
if ($fombergAuthorityLines[2846] -ne '\subsection{Degree maps}') {
    throw 'Fomberg next-source cursor line 2847 is not the frozen Degree maps subsection.'
}

$fombergCombinedSliceBytes = $utf8.GetBytes(
    ($fombergAuthorityLines[30..2845] -join $lf) + $lf
)
if ($fombergCombinedSliceBytes.Length -ne 107572 -or
    (Get-Sha256 $fombergCombinedSliceBytes) -ne '3aace2071a36b91494dbb5090e4a2eb2e709bdb067d2163239d789a07c6191f3') {
    throw 'Ordered Fomberg lines 31-2846 combined identity mismatch.'
}
...
$fombergIds = @($fombergIds001 + $fombergIds002 + $fombergIds003 + $fombergIds004)
if ($fombergIds.Count -ne 424 -or
    @($fombergIds | Sort-Object -Unique).Count -ne 424) {
...
if (($masteryTriples | Measure-Object -Sum).Sum -ne 25) {
...
$header = @'
---
title: "Topologi Aljabar - Roberts 30/30 dan Fomberg 1.1-1.10"
subtitle: "Komponen Roberts lengkap; jembatan Fomberg melalui pembandingan homologi; jalur komposit masih parsial"
author:
...
Checkpoint ini memuat komponen Roberts lengkap 30/30 serta empat komponen
...
baris sumber 1923-2846, dan dipetakan ke D60-R11. Jadi jembatan Fomberg kini
mencakup Bagian 1.1-1.10 dan baris sumber 31-2846. Jalur komposit masih
parsial: bagian berikutnya dimulai pada baris 2847 dan lapisan penguasaan
lintas-rute belum selesai. Keempat komponen Fomberg bukan kuliah Roberts
...
foreach ($name in @('fombergBody001', 'fombergBody002', 'fombergBody003', 'fombergBody004')) {
...
if ([regex]::IsMatch($payload, '(?im)^#{1,6}\s+(?:Unit|Kuliah)\s+(?:31|32|33|34)\b') -or
    [regex]::IsMatch($payload, '(?i)#[-a-z0-9]*unit-03[1234](?:[-}\s]|$)')) {
    throw 'Transient composite payload renumbers Fomberg as Roberts Unit 31, 32, 33, or 34.'
}
...
$common = @(
    ...
    '--number-sections',
    '--metadata=lang:id-ID',
    '--metadata=pagetitle:Topologi Aljabar - Roberts 30/30 dan Fomberg 1.1-1.10',
...
if ($domIds.Count -le 1982) {
    throw 'Composite DOM did not grow beyond the frozen Fomberg 001-003 boundary.'
}
...
if ($mathmlNodes -le 13466 -or
    $semanticFigures -lt 134 -or
...
if ($htmlImageTags.Count -ne 9 -or $htmlEmbeddedPngs.Count -ne 9) {
    throw 'HTML does not contain exactly nine embedded Unit 003-004 PNG images.'
}
...
if ($pdfPages -le 397) {
    throw 'Composite PDF did not grow beyond the frozen Fomberg 001-003 boundary.'
}
...
if ($pdfUnit001Position -lt 0 -or
    ...
    $pdfUnit004Position -le $pdfUnit003Position) {
...
if (-not $scratchFull.StartsWith(
    ...
    [IO.Path]::GetFileName($scratchFull) -ne
        'roberts-001-030-fomberg-001-004-build') {
...
[pscustomobject]@{
    status = 'PASS'
    scope = 'Roberts 30/30 complete; Fomberg Sections 1.1-1.10 through source line 2846; composite partial'
    html = $html
    ...
    fomberg_mastery_triples_total = 25
