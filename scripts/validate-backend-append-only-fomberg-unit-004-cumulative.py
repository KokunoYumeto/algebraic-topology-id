#!/usr/bin/env python3
"""Replay Unit 004 and write deterministic semantic/cumulative backend receipts.

This script cannot pass before the exact Unit 004 suffix has been appended and
...
PRODUCER_PATH = LANE / "scripts/extend-backend-fomberg-unit-004.py"
VALIDATOR_PATH = LANE / "scripts/validate-backend-append-only-fomberg-unit-004.py"
OUTPUTS = {
    "manifest": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_FILE_MANIFEST.csv",
    "semantic_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_RECEIPT.json",
    "semantic_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_RECEIPT.md",
    "cumulative_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_CUMULATIVE_RECEIPT.json",
    "cumulative_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_CUMULATIVE_RECEIPT.md",
    "replay_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_004_REPLAY_RECEIPT.json",
}
ROBERTS_BOUNDARY = (
    4761, 5213679,
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 004 cumulative validator FAIL: {message}")
...
def replay_producer(producer, c) -> tuple[int, int, str]:
    with tempfile.TemporaryDirectory(prefix="o012-fom-u004-backend-replay-") as temporary:
        ...
        require("Fomberg Unit 004 semantic backend extension: PASS" in captured.getvalue(), "producer replay PASS marker absent")
...
def write_outputs(c, identities, rows, final, script_results) -> None:
    ...
    bound_inputs = {}
    for key, path in (
        ("common", LANE / "scripts/fomberg-unit-004-common.py"),
    ...
    semantic_md = (
        "# Fomberg Unit 004 append-only backend receipt\n\n"
        "Status: **PASS**\n\n"
        f"- Immutable Unit 003 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / `{c.PREFIX_TOTAL[2]}`.\n"
        f"- Unit 004 semantic suffix: {sum(c.DELTA.values())} records, including 117 unit and 117 segment records.\n"
        ...
        "- Reader: 117 stable IDs; source diagrams: 20; semantic figure blocks: 19; linked redraw files: 6; mastery: seven solved triples.\n"
        "- FOM-PR-05 through FOM-PR-11 are complete. Cursor: source line 2847; source EOF: false.\n"
        "- The older Unit 003 forward-obligation ledger row remains explicitly preserved; this append does not silently rewrite historical control state.\n"
    ).encode("utf-8")
    OUTPUTS["semantic_md"].write_bytes(semantic_md)
    semantic = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-004-SEMANTIC-APPEND-ONLY",
        "status": "PASS",
        ...
        "closure": {
            "stable_ids": 117, "unit_records": 117, "segment_records": 117,
            "source_diagrams": 20, "semantic_figure_blocks": 19,
            "redraw_files": 6, "mastery_triples": 7,
            "proof_repairs": [f"FOM-PR-{number:02d}" for number in range(5, 12)],
    ...
    cumulative_md = (
        "# Cumulative backend receipt through Fomberg Unit 004\n\n"
        "Status: **PASS**\n\n"
        f"- Preserved Fomberg Unit 003 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / `{c.PREFIX_TOTAL[2]}`.\n"
        f"- Fomberg Unit 004 append-only suffix: {sum(c.DELTA.values())} records.\n"
        f"- Cumulative backend: {final[0]:,} records / {final[1]:,} bytes / `{final[2]}`.\n"
        "- Fomberg Units 001–004: 424 stable reader IDs mirrored one-to-one as unit and segment records; 77 source diagrams; 69 semantic figure blocks; 25 solved mastery triples.\n"
        "- Closed proof repairs: FOM-PR-01 through FOM-PR-11. Cursor: source line 2847; source EOF: false.\n"
        "- Deterministic producer replay and semantic append-only validation: **PASS**.\n"
    ).encode("utf-8")
    OUTPUTS["cumulative_md"].write_bytes(cumulative_md)
    cumulative = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-THROUGH-FOMBERG-UNIT-004-CUMULATIVE-SEMANTIC",
        "status": "PASS",
        "scope": "Roberts Units 001-030 immutable prefix plus Fomberg Units 001-004 append-only semantic suffixes",
        "model_provenance": c.MODEL,
        "nested_immutability": {
            ...
            "fomberg_unit_004_suffix": {"records": sum(c.DELTA.values()), "records_by_file": c.DELTA},
        },
        "current": {
            "total_records": final[0], "total_bytes": final[1], "bundle_sha256": final[2],
            "backend_files": backend_files, "stable_ids": 424,
            "unit_records": 424, "segment_records": 424,
            "source_diagrams": 77, "semantic_figure_blocks": 69,
            "redraw_files": 18, "mastery_triples": 25,
            "proof_repairs_closed": [f"FOM-PR-{number:02d}" for number in range(1, 12)],
            "next_source_line": 2847, "terminal_source_eof": False,
    ...
    replay = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-004-DETERMINISTIC-REPLAY",
        "status": "PASS",
...
def main() -> int:
    ...
    producer = load(PRODUCER_PATH, "o012_fomberg_u004_producer_cumulative")
    ...
    require("Fomberg Unit 004 append-only backend validation: PASS" in result.stdout, "semantic validator PASS marker absent")
    rows, final = backend_snapshot(c)
    require(final[0] == 6113, "unexpected final record total")
    ...
    print("Cumulative backend validation through Fomberg Unit 004: PASS")
    print(f"unit003_prefix_records={c.PREFIX_TOTAL[0]}")
    print(f"unit004_records_added={sum(c.DELTA.values())}")
