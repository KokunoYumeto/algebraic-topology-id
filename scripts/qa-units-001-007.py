#!/usr/bin/env python3
"""Fail-closed QA for the cumulative O012 Units 001-007 reader."""
...
EXPECTED_ID_COUNTS = (29, 41, 39, 33, 30, 28, 24)
EXPECTED_BLOCKS = (
...
HTML = LANE / "output/html/units-001-007/index.html"
PDF = LANE / "output/pdf/topologi-aljabar-unit-001-007-id.pdf"
MANIFEST = LANE / "output/ARTIFACT_MANIFEST_UNITS_001_007.csv"
VISUAL = LANE / "qa/UNITS_001_007_VISUAL_QA.md"
TEXT_WITNESS = LANE / "qa/units-001-007-extracted.txt"
RECEIPT = LANE / "qa/UNITS_001_007_QA.json"
...
REVIEWS = (
    LANE / "qa/UNIT_006_INDEPENDENT_REVIEW.md",
    LANE / "qa/UNIT_007_INDEPENDENT_REVIEW.md",
)

POST_BUILD_EXPECTED = {
    HTML: (899803, "55135048eafe0f097c45936add885e008392eefdf475270fea37adf6a2a7b7bb"),
    PDF: (702470, "3764b75ecfb9200e25a165db1f0f97a680384378e2a9a22e129aab57dd860d93"),
    MANIFEST: (247, "7b279f0413892f0ddedce636b3a272884bb7bfa01410bf33a6ce34c0c34db2f9"),
    VISUAL: (3259, "63a4b4545213a7aec1c556a3852b818ba2f207b10cac7e80c62330709604176f"),
    TEXT_WITNESS: (190424, "f6839e7eb7f25c8518ec3fc2e2372b82b1f1387b48402899ff8bc40ce153c8dc"),
    REVIEWS[0]: (1783, "5dd3868192a85e3e60562f42ec7d7b792e0e58811719ecc97207ed2bdc5de4bf"),
    REVIEWS[1]: (1761, "87c5129cd7d367893860b150c72948de1d196d7cbefe04d53f7a4efecf921f87"),
}
...
EXPECTED_HTML_IDS = 315
EXPECTED_STABLE_IDS = 224
EXPECTED_FRAGMENT_HREFS = 89
EXPECTED_MATHML = 2344
EXPECTED_PDF_PAGES = 66
UNIT6_ADVERSE = {f"O012-ADV-{number:04d}" for number in range(71, 83)}
UNIT7_ADVERSE = {f"O012-ADV-{number:04d}" for number in range(83, 95)}
UNIT6_REFLOW = {"O012-ADV-0082"}
UNIT7_REFLOW = {"O012-ADV-0087", "O012-ADV-0088"}
UNIT7_ALIAS = {"O012-ADV-0094"}
ARTIFACT_PATHS = {
...
def block_counts(text: str) -> dict[str, int]:
    values = Counter(
        re.findall(
            r"^::: \{\.(corollary|definition|example|exercise|lemma|note|proof|proposition|question|remark|theorem)\s+#o012-",
            text,
            flags=re.MULTILINE,
        )
    )
...
def verify_sources(upstream: list[str]) -> tuple[list[str], dict[str, dict[str, int]]]:
    ...
    spans = ((1305, 1515, 6), (1516, 1770, 7))
    expected_envs = (
        {"definition": 2, "example": 3, "lemma": 2, "proof": 2, "theorem": 2},
        {"corollary": 1, "definition": 1, "example": 3, "lemma": 1, "proof": 1, "prop": 2},
    )
    ...
    unit6, unit7 = texts[5], texts[6]
    for token in (
        r"p(t)=e^{it}", r"2\pi\mathbb{Z}", "hukum eksponensial",
        r"H(0,t)=\gamma(t)", r"H(1,t)=\eta(t)", r"H(s,0)=x", r"H(s,1)=y",
        "Istilah SLSC tanpa syarat tambahan", "setengah bola yang konveks",
        "DMRthesis_final.pdf", "Bukti mandiri Teorema 6.2",
    ):
        require(token in unit6, f"Missing Unit 6 correction/mastery invariant: {token}")
    for token in (
        r"Z_{\gamma(0)}", r"h(s,t)=(1-s)\phi(t)+s\psi(t)",
        r"I\times\Omega_xX\times\Omega_xX\times\Omega_xX", "aksi kanan",
        "representasi kiri", "pemetaan jepit", r"[(S^1,1),(X,x)]_*",
        'data-source-label="eg:piS^1_infinite"',
    ):
        require(token in unit7, f"Missing Unit 7 correction/mastery invariant: {token}")
    require(unit7.count('data-source-label="eg:piS^1_infinite"') == 1, "Unit 7 source alias mismatch")

    for lecture, text in ((6, unit6), (7, unit7)):
        ...
        require("David Michael Roberts" in text and "*Algebraic Topology*" in text, f"Unit {lecture} attribution missing")
        require("Creative Commons Attribution 4.0 International" in text, f"Unit {lecture} license link missing")
        require("tidak menyiratkan dukungan atau pengesahan" in text, f"Unit {lecture} non-endorsement missing")

    return all_ids, {f"unit_{number:03d}": blocks[number - 1] for number in range(1, 8)}


def verify_controls() -> dict[str, object]:
    with ADVERSE_LEDGER.open("r", encoding="utf-8", newline="") as stream:
        rows = list(csv.DictReader(stream))
    require(len(rows) >= 94 and all(None not in row and len(row) == 7 for row in rows), "Adverse ledger CSV shape mismatch")
    indexed = {row["event_id"]: row for row in rows}
    require(len(indexed) == len(rows), "Duplicate adverse-ledger event ID")
    required = UNIT6_ADVERSE | UNIT7_ADVERSE
    require(required <= indexed.keys(), f"Missing Unit 6/7 adverse events: {sorted(required-indexed.keys())}")
    ...
    require({event_id for event_id in UNIT6_ADVERSE if indexed[event_id]["status"] == "accessibility_reflow"} == UNIT6_REFLOW, "Unit 6 reflow inventory mismatch")
    require({event_id for event_id in UNIT7_ADVERSE if indexed[event_id]["status"] == "accessibility_reflow"} == UNIT7_REFLOW, "Unit 7 reflow inventory mismatch")
    require({event_id for event_id in UNIT7_ADVERSE if indexed[event_id]["status"] == "identifier_preservation"} == UNIT7_ALIAS, "Unit 7 alias inventory mismatch")
    ...
    require(adverse_subset_sha == "f1ec9603163615c991d508f6f8474b50478c015783754b598a4d23c12e83ac65", "Unit 6/7 adverse subset hash mismatch")
    with TERMINOLOGY.open("r", encoding="utf-8", newline="") as stream:
        terms = list(csv.DictReader(stream))
    require(len(terms) >= 133 and all(None not in row and len(row) == 6 for row in terms), "Terminology CSV shape mismatch")
    term_ids = [row["term_id"] for row in terms]
    require(len(term_ids) == len(set(term_ids)), "Duplicate terminology ID")
    required_terms = {f"O012-TERM-{number:04d}" for number in range(115, 134)}
    require(required_terms <= set(term_ids), "Unit 6/7 terminology range incomplete")
    ...
    require(terms_subset_sha == "1023cf7b682cf35e66292608f4bbc985a15445f6bab1fa6df8bcb0e3780d1d00", "Unit 6/7 terminology subset hash mismatch")
    return {
        "unit_006_adverse_ids": sorted(UNIT6_ADVERSE),
        "unit_007_adverse_ids": sorted(UNIT7_ADVERSE),
        "unit_006_accessibility_reflows": sorted(UNIT6_REFLOW),
        "unit_007_accessibility_reflows": sorted(UNIT7_REFLOW),
        "unit_007_identifier_alias": sorted(UNIT7_ALIAS),
        "terminology_ids": [f"O012-TERM-{number:04d}" for number in range(115, 134)],
...
def verify_reviews() -> dict[str, str]:
    out: dict[str, str] = {}
    for lecture, path, source_expected, span in zip((6, 7), REVIEWS, SOURCE_EXPECTED[5:], ("1305-1515", "1516-1770"), strict=True):
        ...
        require(
            f"{EXPECTED_ID_COUNTS[lecture-1]} stable IDs" in review,
    ...
    require("Verdict: **PASS**" in visual and "all 66 pages" in visual, "Visual all-page review missing")
    ...
    require("32 were wider" in visual and "all 32 exposed local horizontal scrolling" in visual, "Formula-local scrolling evidence missing")
...
def verify_html(source_ids: list[str]) -> dict[str, int]:
    ...
    require("".join(root.xpath("//title/text()")) == "Topologi Aljabar - Unit 1-7", "HTML title mismatch")
    ...
    alias = root.xpath('//*[@data-source-label="eg:piS^1_infinite"]')
    require(len(alias) == 1 and alias[0].get("id") == "o012-rbt-l07-exa-003", "Source-label alias was not preserved exactly")
    styles = "\n".join(root.xpath("//style/text()"))
    for rule in ("max-width: 58rem", "margin: 0 auto", 'math[display="block"]', "overflow-x: auto", ".question", ".theorem", ".corollary", ".remark"):
...
def verify_pdf() -> tuple[dict[str, object], bytes]:
    ...
    require(metadata.title == "Topologi Aljabar - Unit 1-7", "PDF title mismatch")
    ...
    for phrase in (
        "Kuliah 1", "Kuliah 2", "Kuliah 3", "Kuliah 4", "Kuliah 5", "Kuliah 6", "Kuliah 7",
        "Topologi kompak-terbuka pada ruang lintasan", "Grup fundamental dan serat ruang penutup",
        "Solusi Pemeriksaan 6.4", "Solusi Pemeriksaan 7.4",
    ):
...
def main() -> int:
    ...
    receipt = {
        ...
        "role_id": "O012",
        "course_id": "D60",
        "unit_ids": [f"o012-rbt-l{number:02d}" for number in range(1, 8)],
        "source_authority": {
            ...
            "line_end": 1770,
            "next_line": 1771,
            "next_boundary": "Lecture 8 marker",
        },
        "reader_sources": [
            {
        ...
        "witnesses": {
            **reviews,
            ...
            "prior_boundary_units_001_005_qa_sha256": HISTORICAL_EXPECTED["qa/UNITS_001_005_QA.json"][1],
        },
        "responsive_browser": {
            ...
            "mobile_content_css_px": 375,
            "mobile_document_overflow_css_px": 0,
            "mobile_wide_display_math": 32,
            "mobile_locally_scrollable_wide_display_math": 32,
        ...
        "gates": {
            ...
            "unit_006_007_corrections_reflows_aliases_verified": True,
            ...
            "visual_review": "pass_manual_all_66_pages_and_browser_desktop_mobile",
