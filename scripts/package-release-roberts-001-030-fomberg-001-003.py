#!/usr/bin/env python3
"""Prepare the Roberts 001-030 + Fomberg 001-003 Zenodo payload.
...
O012-FOM-001, O012-FOM-002, and O012-FOM-003 remain distinct edition
components mapped to D60-R08, D60-R09, and D60-R10; none is renumbered as a
Roberts lecture.
"""
...
RELEASE = LANE / "release" / "zenodo-roberts-001-030-fomberg-001-003"
...
TITLE = (
    ...
    "Jembatan Homologi §1.1–1.6"
)
VERSION = "0.30.3"
RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-003-v0.30.3"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22087423
PREVIOUS_DOI = "10.5281/zenodo.22087423"
...
FROZEN_LEDGER_SHA256 = "71380dc5ecbad43007437831a74a49bdbd9e2cd83a61e797874647330b219fda"

PDF_INPUT = (
    "output/pdf/"
    "topologi-aljabar-roberts-001-030-fomberg-001-003-id.pdf"
)
HTML_INPUT = (
    "output/html/roberts-001-030-fomberg-001-003/index.html"
)
ARTIFACT_MANIFEST = (
    "output/ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_003.csv"
)
BUILD_RECEIPT = "qa/ROBERTS_001_030_FOMBERG_001_003_BUILD_RECEIPT.json"
VISUAL_QA = "qa/ROBERTS_001_030_FOMBERG_001_003_VISUAL_QA.md"
RENDER_INVENTORY = (
    "qa/ROBERTS_001_030_FOMBERG_001_003_RENDER_INVENTORY.csv"
)
BACKEND_CUMULATIVE_JSON = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_003_CUMULATIVE_RECEIPT.json"
)
BACKEND_CUMULATIVE_MD = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_003_CUMULATIVE_RECEIPT.md"
)
FOMBERG_QA_003 = "qa/FOMBERG_UNIT_003_QA.json"
FOMBERG_READER_001 = (
...
FOMBERG_ASSETS_003 = tuple(
    f"source/id-ID/fomberg/assets/unit-003/{name}"
    for name in (
        "brouwer-radial-retraction.png",
        "brouwer-radial-retraction.svg",
        "cone-circle.png",
        "cone-circle.svg",
        "relative-boundary.png",
        "relative-boundary.svg",
        "relative-chains-formal.png",
        "relative-chains-formal.svg",
        "relative-cycle.png",
        "relative-cycle.svg",
        "suspension-circle.png",
        "suspension-circle.svg",
    )
)

PDF_NAME = (
    "00_TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_"
    "FOMBERG_001_003_READER.pdf"
)
HTML_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_"
    "FOMBERG_001_003_READER.html"
)
SOURCE_ZIP_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_003_"
    ...
    entries.update(
        {
            ...
            FOMBERG_READER_003: prior.lane_path(FOMBERG_READER_003),
            **{
                relative: prior.lane_path(relative)
                for relative in FOMBERG_ASSETS_003
            },
            "scripts/build-roberts-001-030-fomberg-001-003.ps1": (
                LANE / "scripts" / "build-roberts-001-030-fomberg-001-003.ps1"
            ),
            "scripts/package-release-roberts-001-030-fomberg-001-003.py": SCRIPT,
            "scripts/publish-zenodo-roberts-001-030-fomberg-001-003.py": (
                LANE
                / "scripts"
                / "publish-zenodo-roberts-001-030-fomberg-001-003.py"
            ),
            ...
            "scripts/fomberg-unit-003-common.py": (
                LANE / "scripts" / "fomberg-unit-003-common.py"
            ),
            "scripts/qa-fomberg-unit-003.py": (
                LANE / "scripts" / "qa-fomberg-unit-003.py"
            ),
            "scripts/extend-backend-fomberg-unit-003.py": (
                LANE / "scripts" / "extend-backend-fomberg-unit-003.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-003.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-003.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-003-cumulative.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-003-cumulative.py"
            ),
        }
    )
    return entries
...
def source_path_inventory() -> set[str]:
    ...
    paths.remove("scripts/package-release-roberts-001-030-fomberg-001-003.py")
    return paths
    ...
    paths.update(
        {
            ...
            "qa/fomberg-unit-003/INDEPENDENT_REVIEW_EXACT_FINAL.json",
            "qa/fomberg-unit-003/INDEPENDENT_REVIEW_RELATIVE_FINAL.json",
            "qa/fomberg-unit-003/INDEPENDENT_REVIEW_INTEGRATED_FINAL.json",
...
def release_control_inventory() -> set[str]:
    prefix = "release/zenodo-roberts-001-030-fomberg-001-003/"
    return {
        prefix + name
        for name in (
            "metadata.json",
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[str]]:
    ...
    if re.search(
        ...
        r"(?:0*1\s*[-–]\s*0*(?:31|32|33)|"
        r"0*(?:31|32|33)\s*/\s*0*(?:31|32|33))",
        joined,
    ):
        raise RuntimeError("Fomberg was incorrectly renumbered as Roberts 31/32/33")
    for marker in (
        "30/30",
        "O012-FOM-001",
        "O012-FOM-002",
    ...
    if (
        ...
        raise RuntimeError(
            "publication controls are intentionally unsealed; bind final "
            "artifact identities before packaging"
        )
    if (
    ...
    if (
        backend_binding.get("expected_records") != 5747
        or backend_binding.get("final_identity_source") != BACKEND_CUMULATIVE_JSON
        or backend_binding.get("hardcoded_final_identity") is not False
    ):
        raise RuntimeError("publication plan backend binding policy drift")
    expected_readers = {
        "html": {
            "bytes": 7190228,
            "path": HTML_INPUT,
    ...
    if plan.get("final_reader_artifacts") != expected_readers:
        raise RuntimeError("publication plan final reader identity drift")
    if (
    ...
    if (
        ...
        != "roberts_complete_fomberg_units_001_003_complete_composite_course_partial"
    ):
    ...
    if (
        ...
        != ["O012-FOM-001", "O012-FOM-002", "O012-FOM-003"]
        or numbering.get("course_route_unit_ids")
        != ["D60-R08", "D60-R09", "D60-R10"]
        or numbering.get("fomberg_is_not_roberts_units_031_033") is not True
    ):
...
def verify_build_receipt(
    ...
    if (
        ...
        != "O012-RBT-001-030-FOM-001-003-COMPOSITE-BUILD"
    ):
    ...
    if (
        ...
        or fomberg.get("selected_source_span") != "31-1922"
        or fomberg.get("selected_span_lines") != 1892
        or fomberg.get("next_source_line") != 1923
...
def verify_fomberg_qa() -> dict[str, Any]:
    qa = json.loads(prior.lane_path(FOMBERG_QA_003).read_text(encoding="utf-8"))
    if (
        qa.get("status") != "PASS"
        or qa.get("qa_id") != "O012-FOMBERG-UNIT-003-STATIC-QA"
    ):
        raise RuntimeError("Fomberg Unit 003 static QA did not pass")
    ...
    if (
        ...
        or authority.get("next_source_line") != 1923
        or span.get("line_start") != 1291
        or span.get("line_end") != 1922
        or authority.get("terminal_source_eof") is not False
    ):
        raise RuntimeError("Fomberg Unit 003 source boundary mismatch")
    if not prior.identity_matches(
        qa.get("reader"), prior.lane_path(FOMBERG_READER_003)
    ):
        raise RuntimeError("Fomberg QA does not bind the translated reader")
    checks = qa.get("checks", {})
    required = (
        "reader_identity_65540_bytes_1773_lf_lines",
        "reader_sha256_2571f62b",
        "source_full_and_three_span_identities",
        "cursor_line_1923",
        "unique_125_stable_ids",
        "balanced_119_fenced_divs",
        "twenty_nine_source_diagrams_twenty_six_figures",
        "six_png_svg_redraw_pairs_exact_and_accessible",
        "fom_pr_04_choice_additivity_and_exactness_closed",
        "six_exercise_hint_solution_triples",
        "three_final_reviews_zero_and_pass",
        "review_count_124_superseded_by_independent_125",
        "pandoc_parse_available_and_pass",
        "inputs_not_modified",
        "backend_write_deferred",
    )
    if any(checks.get(name) is not True for name in required):
        raise RuntimeError("Fomberg Unit 003 static QA gate is incomplete")
    if (
        qa.get("structure", {}).get("stable_id_count") != 125
        or qa.get("structure", {}).get("redraw_asset_files") != 12
        or qa.get("mastery", {}).get("triples") != 6
        or qa.get("model_provenance") != MODEL_NOTE
    ):
        raise RuntimeError("Fomberg Unit 003 QA structure/provenance drift")
    return qa
...
def verify_backend_receipt(backend_facts: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        != "O012-BACKEND-THROUGH-FOMBERG-UNIT-003-CUMULATIVE-SEMANTIC"
    ):
        raise RuntimeError("Fomberg Unit 003 cumulative backend receipt failed")
    ...
    if snapshot != {
        ...
        raise RuntimeError("publication plan backend snapshot/receipt drift")
    if (
        current.get("next_source_line") != 1923
        or current.get("terminal_source_eof") is not False
        or current.get("stable_ids") != 307
        or current.get("mastery_triples") != 18
        or current.get("redraw_files") != 12
        or current.get("proof_repairs_closed")
        != ["FOM-PR-01", "FOM-PR-02", "FOM-PR-03", "FOM-PR-04"]
    ):
    ...
    unit002_prefix = receipt.get("nested_immutability", {}).get(
        "fomberg_unit_002_boundary", {}
    )
    if (
        unit002_prefix.get("preserved_byte_for_byte") is not True
        or unit002_prefix.get("records") != 5342
        or unit002_prefix.get("bundle_sha256")
        != "83d98f1b271c5e62334a072354f1be1c4a1535ed26c8a403223e89773bb1eba1"
    ):
        raise RuntimeError("Fomberg Unit 002 backend boundary was not preserved")
    if not prior.identity_matches(
...
def write_package_receipt(
    ...
    payload = {
        ...
        "scope": (
            "Roberts 30/30 complete; Fomberg O012-FOM-001, "
            "O012-FOM-002, and O012-FOM-003 (Sections 1.1-1.6, "
            "lines 31-1922) complete; next Fomberg line 1923; "
            "composite course partial"
        ),
        ...
        manifest.update(
            {
                ...
                "reader_qa": {
                    ...
                    "fomberg_stable_ids_total": 307,
                    "fomberg_unit_003_stable_ids": fomberg_qa["structure"][
                        "stable_id_count"
                    ],
                    "mastery_triples_total": 18,
                    "fomberg_unit_003_mastery_triples": fomberg_qa["mastery"][
                        "triples"
                    ],
                    "fomberg_unit_003_redraw_assets": fomberg_qa["structure"][
                        "redraw_asset_files"
                    ],
        ...
        json.dumps(
            {
                ...
                "release_directory": RELEASE.relative_to(LANE).as_posix(),
                "scope": (
                    "Roberts 30/30 + Fomberg O012-FOM-001/002/003 through "
                    "line 1922; next line 1923; composite course partial"
                ),
                "files": [
                    {
                        "filename": name,
