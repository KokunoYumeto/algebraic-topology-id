#!/usr/bin/env python3
"""Publish the sealed Roberts 001-030 + Fomberg 001 Zenodo checkpoint.

The transaction is concept-lineage locked, resumable, reader-first, and
fail-closed.  It reuses the proven new-version transaction engine from the
Roberts 001-030 release while replacing every release-specific validation
rule.  Credentials are read only after local and anonymous-lineage gates and
are never written.  Completion requires anonymous byte/SHA-256 readback of all
nine public files.
"""
...
BASE_PATH = SCRIPT.with_name("publish-zenodo-units-001-030.py")
SPEC = importlib.util.spec_from_file_location("o012_roberts_030_publisher", BASE_PATH)
...
base.RELEASE = base.LANE / "release" / "zenodo-roberts-001-030-fomberg-001"
...
base.SEED_RECORD = 22077025
previous.PREVIOUS_RECORD_ID = 22077025
previous.PREVIOUS_DOI = "10.5281/zenodo.22077025"
base.RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-v0.30.1"
base.TITLE = (
    "Topologi Aljabar: Edisi Bahasa Indonesia — Roberts 1–30 dan "
    "Jembatan Homologi §1.1–1.2"
)
base.VERSION = "0.30.1"
base.PDF_NAME = "00_TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_READER.pdf"
base.FILE_NAMES = [
    base.PDF_NAME,
...
def public_text_is_safe(text: str) -> None:
    ...
    if any(re.search(pattern, text) for pattern in patterns):
        raise RuntimeError("private path or credential marker in public Zenodo material")
...
def assert_organization_rule(metadata: dict) -> None:
    ...
    if re.search(r"(?i)\bTTP\b|Translation and Transcription Project", prose):
        raise RuntimeError("organization marker leaked into title/description/notes/prose")
...
def verify_local() -> tuple[dict, dict]:
    ...
    for path in public_prose_paths:
        ...
        if re.search(r"(?i)\bTTP\b|Translation and Transcription Project", text):
            raise RuntimeError(f"organization marker repeated in public prose: {path.name}")
    ...
    if "published" in manifest:
        raise RuntimeError("immutable upload manifest contains a stale publication claim")
    ...
    if not (
        ...
        and fomberg.get("line_end") == 614
        and fomberg.get("component_id") == "O012-FOM-001"
        and fomberg.get("next_source_line") == 615
    ...
    if (
        ...
        raise RuntimeError("package preparation receipt scope/inventory drift")
    ...
    if not all(
        ...
        raise RuntimeError("package receipt does not record all preparation gates")
...
def verify_public(
    ...
    if normalized_people(
        ...
        raise RuntimeError("public Zenodo contributor drift")
    ...
    for marker in (
        ...
        if marker not in description:
            raise RuntimeError(f"public Zenodo description omits scope marker: {marker}")
...
def make_receipt(public: dict, rows: list[dict], metadata_payload: dict, local: dict) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "Fomberg algebraic_topology.tex lines 31-614, Sections 1.1-1.2, "
            "O012-FOM-001 complete; next line 615; composite course partial"
        ),
        "record_id": record_id,
        "doi": public.get("doi"),
