#!/usr/bin/env python3
"""Seal and package the v0.31.4 computation-Labs-1–2 successor (local only).

This fail-closed successor reuses the proved v0.31.3 deterministic archive and
...
TEMPLATE_PATH = SCRIPT.with_name(
    "package-release-roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01.py"
)
SPEC = importlib.util.spec_from_file_location("o012_v0313_packager_template", TEMPLATE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved v0.31.3 packager")
...
TITLE = (
    ...
    "Petunjuk Rute 1–6, dan Laboratorium Komputasi 1–2"
)
VERSION = "0.31.4"
RELEASE_ID = f"o012-composite-id-{SLUG}-v{VERSION}"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22142210
PREVIOUS_DOI = "10.5281/zenodo.22142210"
...
FROZEN_LEDGER_SHA256 = "c1a96e5292267225b284d21c219a1990c2342a91e22a583b4f75786d4310e54b"
...
LAB_QA = "qa/COMPUTATION_LAB_002_QA.json"
LAB_STATIC_QA = "qa/computation-lab-002/STATIC_QA.json"
LAB_MATH_REVIEW = "qa/computation-lab-002/INDEPENDENT_MATH_REVIEW.json"
LAB_LANGUAGE_REVIEW = "qa/computation-lab-002/INDEPENDENT_SOURCE_LANGUAGE_REVIEW.json"
LAB_CODE_REVIEW = "qa/computation-lab-002/INDEPENDENT_CODE_REVIEW.json"
LAB_EXECUTION_RECEIPT = "qa/computation-lab-002/EXECUTION_RECEIPT.json"
BACKEND_CUMULATIVE = "qa/BACKEND_APPEND_ONLY_COMPUTATION_LAB_002_CUMULATIVE_RECEIPT.json"

LAB_SOURCE = "source/id-ID/labs/computation-lab-002-chain-matrices-smith-normal-form.md"
LAB_PROGRAM = "source/id-ID/labs/o012_d60_lab02_smith_normal_form.py"
LAB_TESTS = "source/id-ID/labs/test_o012_d60_lab02_smith_normal_form.py"
LAB_EXPECTED = "source/id-ID/labs/expected-output-lab02.txt"
...
MANIFEST_STATUS = (
    ...
    "computation_labs_001_002_complete_composite_course_partial"
)
...
def source_entries() -> dict[str, Path]:
    ...
    additions = {
        ...
        LAB_TESTS,
        LAB_EXPECTED,
        "scripts/merge-computation-lab-002.py",
        "scripts/qa-computation-lab-002.py",
        "scripts/finalize-computation-lab-002-qa.py",
        "scripts/extend-backend-computation-lab-002.py",
        "scripts/validate-backend-append-only-computation-lab-002.py",
...
def verify_lab() -> dict[str, Any]:
    ...
    if not (
        qa.get("status") == "PASS"
        and qa.get("laboratory_id") == "D60-LAB02"
        and qa.get("edition_unit_id") == "O012-ORIG-LAB02"
        and qa.get("course_route_unit_ids") == ["D60-R08"]
        ...
        raise RuntimeError("computation Lab 2 source/execution/review closure mismatch")
    ...
    for relative in expected:
        if relative not in indexed or not base.identity_matches(indexed[relative], lane_path(relative)):
            raise RuntimeError(f"Lab 2 QA has a stale source identity: {relative}")
    for relative in (LAB_STATIC_QA, LAB_CODE_REVIEW, LAB_MATH_REVIEW, LAB_LANGUAGE_REVIEW, LAB_EXECUTION_RECEIPT):
        ...
        if not str(review.get("status", "")).startswith("PASS"):
            raise RuntimeError(f"Lab 2 review/execution receipt failed: {relative}")
    return qa
...
def verify_backend() -> dict[str, Any]:
    ...
    if not (
        ...
        and receipt.get("laboratory_id") == "D60-LAB02"
        and receipt.get("edition_unit_id") == "O012-ORIG-LAB02"
        and prefix.get("preserved_exactly") is True
        and prefix.get("records") == 7404
        and prefix.get("bytes") == 8975700
        and prefix.get("bundle_sha256")
        == "4740eb2ff83b4f9df3c0d90c2426ff77e652b23cad0bbe7763c54ebdefa60b4b"
        and final.get("records") == 7546
        and final.get("bytes") == 9122755
        ...
        and replay.get("final", {}).get("records") == 7546
        and replay.get("temporary_replay_removed") is True
    ):
        raise RuntimeError("Lab 2 cumulative backend boundary mismatch")
    return receipt
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        ...
        raise RuntimeError("v0.31.4 metadata identity/rights/creator drift")
    ...
    if not (
        ...
        raise RuntimeError("publication plan escaped the existing concept or v0.31.3 predecessor")
    ...
    if not (
        ...
        and binding.get("expected_records") == 7546
        and snapshot.get("total_records") == 7546
        and snapshot.get("total_bytes") == 9122755
        and snapshot.get("bundle_sha256")
        == "ac3a0377861ed2b728f9c7473579fdd4febe43e454a92f3ea06451e13d46c8f8"
    ):
        raise RuntimeError("publication plan does not bind the Lab 2 backend")
...
def verify_final_gates(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if build.get("status") != "PASS":
        raise RuntimeError("final Lab 2 build receipt did not pass")
    pdf = build.get("pdf", {})
    html = build.get("html", {})
    if not base.identity_matches(pdf, lane_path(PDF_INPUT)) or not base.identity_matches(html, lane_path(HTML_INPUT)):
        raise RuntimeError("build receipt does not bind the final Lab 2 readers")
    if pdf.get("pages") != 529 or pdf.get("page_size") != "A4":
        raise RuntimeError("Lab 2 PDF identity/page boundary drift")
    if plan.get("final_reader_artifacts", {}).get("pdf", {}).get("pages") != 529:
    ...
    if not (
        ...
        and backend.get("cumulative_records") == 7546
        and backend.get("cumulative_bytes") == 9122755
        and backend.get("cumulative_bundle_sha256")
        == "ac3a0377861ed2b728f9c7473579fdd4febe43e454a92f3ea06451e13d46c8f8"
        and backend.get("laboratories_complete") == 2
        and backend.get("laboratories_required") == 4
    ):
        raise RuntimeError("final build does not bind the Lab 2 source/backend closure")
    if not (
        ...
        raise RuntimeError("Lab 2 browser QA did not pass")
    if "Status: **PASS**" not in visual_text or pdf["sha256"] not in visual_text:
        raise RuntimeError("Lab 2 visual QA does not bind the final PDF")
    for relative, row in (
    ...
    if build.get("remaining", {}).get("computation_labs") != ["D60-LAB03", "D60-LAB04"]:
        raise RuntimeError("Lab 2 build has dishonest remaining-laboratory state")
        ...
        manifest.update(
            {
                ...
                "laboratories": {
                    "completed": ["D60-LAB01", "D60-LAB02"],
                    "edition_unit_ids": ["O012-ORIG-LAB01", "O012-ORIG-LAB02"],
                    "required": 4,
                    "completed_count": 2,
                    "remaining": ["D60-LAB03", "D60-LAB04"],
                    "tasks": 12,
                    "tests": 12,
                    "stable_ids": 49,
                ...
                "course_closure": {
                    ...
                    "computation_laboratories_complete": 2,
                    "computation_laboratories_required": 4,
                    "computation_labs_remaining": 2,
    ...
    base.write_json(
        PACKAGE_RECEIPT,
        {
            ...
            "scope": (
                ...
                "computation Labs 1–2 complete; Labs 3–4, proof-metadata closure, and capstone pending"
            ),
        ...
        json.dumps(
            {
                ...
                "post_build_bindings_required": [
                    "final Lab 2 build receipt and exact PDF/HTML identities/pages",
                    "visual, render-inventory, and browser QA receipts",
                    "Lab 2 source/execution/review and exact backend replay receipts",
                    "v0.31.4 release controls and checksum template",
