#!/usr/bin/env python3
"""Deterministically append computation Lab 3 to the frozen 529-page reader."""
...
PRIOR_BYTES = 9_507_127
PRIOR_SHA256 = "1bad03f9ba031ba91967a0a0ac2af6d15a0f768882cd541fe26dcbe26c4edd0b"
PRIOR_PAGES = 529
PRIOR_OUTLINE = 451
PRIOR_NAMED = 3_105
PYPDF_VERSION = "6.12.2"
SCRATCH_BASENAME = "roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01-lab02-lab03-build"
...
def source_contract(path: Path) -> tuple[list[tuple[int, str, str]], list[str]]:
    ...
    headings = [
        ...
        for match in re.finditer(
            r"(?m)^(#{1,2})\s+(.+?)\s+\{#(o012-d60-lab03(?:-[a-z0-9]+)*)\}\s*$",
            text,
        )
    ]
    ids = re.findall(r"#(o012-d60-lab03(?:-[a-z0-9]+)*)\b", text)
    require(len(ids) == len(set(ids)) == 25, "Lab 3 stable-ID inventory is not 25 unique IDs")
    require(len(headings) == 18, "Lab 3 heading inventory is not 18")
    require(headings[0][0] == 1 and sum(level == 1 for level, _title, _ident in headings) == 2, "Lab 3 heading hierarchy lacks its two top-level surfaces")
    require(all(level in (1, 2) for level, _title, _ident in headings), "Lab 3 heading level is outside 1-2")
...
def bookmark_title(source_title: str) -> str:
    """Match Pandoc's bounded plain-text bookmark form for Lab 3 headings."""
...
def merge(prior_path: Path, append_path: Path, output_path: Path, source: Path, scratch_root: Path) -> dict[str, Any]:
    ...
    require(all(name in appendix_named_pages and appendix_named_pages[name] is not None for name in stable_ids), "appendix lacks one or more Lab 3 destinations")
    ...
    require(set(stable_ids).isdisjoint(prior_named_pages), "Lab 3 stable ID collides with predecessor")
    ...
    writer.add_metadata({
        "/Title": "Topologi Aljabar — Roberts 30/30, Fomberg 1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–3",
        "/Author": "David Michael Roberts; Yeheli Fomberg; edisi Bahasa Indonesia dengan pendamping penguasaan",
        "/Subject": "Checkpoint komposit parsial dengan 108 soal bersolusi dan Laboratorium Komputasi 1–3",
    ...
    require(all(merged_named_pages.get(name) == PRIOR_PAGES + int(appendix_named_pages[name]) for name in stable_ids), "Lab 3 named destinations changed")
