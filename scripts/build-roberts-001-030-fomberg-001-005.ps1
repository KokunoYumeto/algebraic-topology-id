...
$htmlDir = Join-Path $lane 'output\html\roberts-001-030-fomberg-001-005'
$pdfDir = Join-Path $lane 'output\pdf'
$scratch = Join-Path $lane 'tmp\pdfs\roberts-001-030-fomberg-001-005-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-roberts-001-030-fomberg-001-005-id.pdf'
$manifest = Join-Path $lane 'output\ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_005.csv'
$receiptTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_005_BUILD_RECEIPT.json'
$visualQaTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_005_VISUAL_QA.md'
$renderInventoryTarget = Join-Path $lane 'qa\ROBERTS_001_030_FOMBERG_001_005_RENDER_INVENTORY.csv'
...
Assert-Frozen $terminologyLedger 56920 '4f3ec0ee76769ac297b3ed820c1e46645277d87c4eb30cbde5358f2cca72b68f'
Assert-Frozen $adverseLedger 194914 'ebf0157674a1db25690480defe4046e2e9a3e1d5322368af2b5a8f3e94597388'
...
if ($fombergAuthorityLines[3122] -ne '\subsection{Cellular complexes}') {
    throw 'Fomberg next-source cursor line 3123 is not the frozen Cellular complexes subsection.'
}

$fombergCombinedSliceBytes = $utf8.GetBytes(
    ($fombergAuthorityLines[30..3121] -join $lf) + $lf
)
if ($fombergCombinedSliceBytes.Length -ne 119775 -or
    (Get-Sha256 $fombergCombinedSliceBytes) -ne '8fea4ae30f1fb8c347505b43a8455d3bdaa1abba86802c1232a3bbe137f1fcbd') {
    throw 'Ordered Fomberg lines 31-3122 combined identity mismatch.'
}
...
if ($fombergIds.Count -ne 476 -or
    @($fombergIds | Sort-Object -Unique).Count -ne 476) {
    throw 'Cross-unit Fomberg stable IDs are incomplete or collide.'
}
$unit005OrderedIdBytes = $utf8.GetBytes(
    ($fombergIds005 -join $lf) + $lf
)
if ((Get-Sha256 $unit005OrderedIdBytes) -ne
    'e85b618bcffc24fd8a802834a352ee73b0b76ced4eef751d3edf3e98e3c5f503') {
    throw 'Unit 005 ordered stable-ID sequence identity mismatch.'
}
...
if (($masteryTriples | Measure-Object -Sum).Sum -ne 31) {
...
subtitle: "Komponen Roberts lengkap; jembatan Fomberg melalui derajat lokal; jalur komposit masih parsial"
author:
...
Checkpoint ini memuat komponen Roberts lengkap 30/30 serta lima komponen
...
sebagai pembandingan derajat opsional serta lapisan derajat lokal aditif. Jadi
jembatan Fomberg kini mencakup Bagian 1.1-1.11 dan baris sumber 31-3122. Jalur
komposit masih parsial: bagian berikutnya dimulai pada baris 3123 dan lapisan
penguasaan lintas-rute belum selesai. Kelima komponen Fomberg bukan kuliah Roberts
tambahan dan tidak mengubah
...
if ([regex]::IsMatch($payload, '(?im)^#{1,6}\s+(?:Unit|Kuliah)\s+(?:31|32|33|34|35)\b') -or
    [regex]::IsMatch($payload, '(?i)#[-a-z0-9]*unit-03[12345](?:[-}\s]|$)')) {
    throw 'Transient composite payload renumbers Fomberg as Roberts Unit 31, 32, 33, 34, or 35.'
}
...
$common = @(
    ...
    '--number-sections',
    '--metadata=lang:id-ID',
    '--metadata=pagetitle:Topologi Aljabar - Roberts 30/30 dan Fomberg 1.1-1.11',
...
if ($domIds.Count -le 2108) {
    throw 'Composite DOM did not grow beyond the frozen Fomberg 001-004 boundary.'
}
...
if ($mathmlNodes -le 14396 -or
    $semanticFigures -lt 135 -or
...
if ($htmlImageTags.Count -ne 9 -or $htmlEmbeddedPngs.Count -ne 9) {
    throw 'HTML does not preserve exactly nine embedded Unit 003-004 PNG images.'
}
...
if ($pdfPages -le 424) {
    throw 'Composite PDF did not grow beyond the frozen Fomberg 001-004 boundary.'
}
...
if ($pdfImageRows.Count -ne 18) {
    throw "PDF must contain exactly nine primary images and nine paired transparency masks; found $($pdfImageRows.Count) rows."
}
$pdfImageInventory = @(
...
if ($pdfPrimaryImages.Count -ne 9 -or $pdfSoftMasks.Count -ne 9) {
    throw "PDF image inventory requires nine primary image rows and nine smask rows; found $($pdfPrimaryImages.Count) and $($pdfSoftMasks.Count)."
}
...
if ($pdfUnit001Position -lt 0 -or
    ...
    $pdfUnit005Position -le $pdfUnit004Position) {
...
if (-not $scratchFull.StartsWith(
    ...
    [IO.Path]::GetFileName($scratchFull) -ne
        'roberts-001-030-fomberg-001-005-build') {
...
[pscustomobject]@{
    status = 'PASS'
    scope = 'Roberts 30/30 complete; Fomberg Sections 1.1-1.11 through source line 3122; composite partial'
    html = $html
    ...
    fomberg_unit_005_ordered_ids_sha256 = 'e85b618bcffc24fd8a802834a352ee73b0b76ced4eef751d3edf3e98e3c5f503'
    fomberg_unit_005_fenced_divs = 46
    fomberg_ids_total = $fombergIds.Count
    fomberg_mastery_triples_total = 31
    fomberg_selected_source_span = '31-3122'
    fomberg_selected_source_bytes = $fombergCombinedSliceBytes.Length
    fomberg_selected_source_sha256 = '8fea4ae30f1fb8c347505b43a8455d3bdaa1abba86802c1232a3bbe137f1fcbd'
    fomberg_next_source_line = 3123
    fomberg_unit_005_reader_sha256 = $fombergReader005Sha256
    fomberg_unit_005_source_audit_sha256 = '2c8280c954bdad90995c8d209b94e9355962e3c724ffedbd1ad84675828b2135'
    fomberg_unit_005_static_qa_sha256 = '874d9ef02875d4fbc28458e56b2c2894be8c990a9fd1c333a0327ccd2d3c4964'
    fomberg_unit_005_independent_review_sha256 = 'bcf628eb480234d217c235727c2082289f0cc09d8062d822c1e33964c072a6e0'
    terminology_ledger_sha256 = '4f3ec0ee76769ac297b3ed820c1e46645277d87c4eb30cbde5358f2cca72b68f'
    adverse_ledger_sha256 = 'ebf0157674a1db25690480defe4046e2e9a3e1d5322368af2b5a8f3e94597388'
    ...
    fomberg_unit_005_semantic_figures = 1
    fomberg_unit_005_raster_images = 0
