...
$htmlDir = Join-Path $lane 'output\html\units-001-004'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-004-build'

# FILL_AFTER_SOURCE_FREEZE: replace these two values only after Unit 4 has
# passed its mathematical/editorial review. The builder deliberately refuses
# to consume a moving source file.
$unit4ExpectedBytes = 24582
$unit4ExpectedSha256 = '826fcb368275cdad02f72a5cec951fc8466ba68b09ca0139d72c81a4c5591fea'
if ($unit4ExpectedBytes -le 0 -or $unit4ExpectedSha256 -notmatch '^[0-9a-f]{64}$') {
    throw 'Fill the exact Unit 4 byte count and SHA-256 before building.'
}
...
$frozen = @{
    ...
    $unit4 = @($unit4ExpectedBytes, $unit4ExpectedSha256)
...
$assembled = Join-Path $checkDir 'reader-units-001-004.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-4"
subtitle: "Ruang Topologis, Keterhubungan, Homotopi, Funktor, dan Ruang Penutup"
author:
...
$payload = $header.Replace("`r`n", "`n") +
    ...
    (Get-MarkdownBody $unit4).TrimEnd("`n") + "`n"
...
$common = @(
    ...
    '--number-sections',
    '--metadata=lang:id-ID',
    '--metadata=pagetitle:Topologi Aljabar - Unit 1-4',
    '--strip-comments'
)
...
$pdfA = Join-Path $checkDir 'units-001-004-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-004-b.pdf'
$pdfArgs = @(
    '--pdf-engine=pdflatex',
...
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-004-id.pdf'
...
[IO.File]::WriteAllText((Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_004.csv'), $manifest, [Text.UTF8Encoding]::new($false))
