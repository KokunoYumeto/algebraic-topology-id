...
RELEASE = LANE / "release" / "zenodo-roberts-001-030-fomberg-001-007-ca01"
...
VERSION = "0.31.0"
RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-007-ca01-v0.31.0"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22104150
PREVIOUS_DOI = "10.5281/zenodo.22104150"
...
FROZEN_LEDGER_SHA256 = "94a76f217db6ee925d3923dbce9a51033721636d7274dc61a4c17326cd9efe83"

PDF_INPUT = "output/pdf/topologi-aljabar-roberts-001-030-fomberg-001-007-ca01-id.pdf"
HTML_INPUT = "output/html/roberts-001-030-fomberg-001-007-ca01/index.html"
ARTIFACT_MANIFEST = "output/ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_007_CA01.csv"
BUILD_RECEIPT = "qa/ROBERTS_001_030_FOMBERG_001_007_CA01_BUILD_RECEIPT.json"
VISUAL_QA = "qa/ROBERTS_001_030_FOMBERG_001_007_CA01_VISUAL_QA.md"
RENDER_INVENTORY = "qa/ROBERTS_001_030_FOMBERG_001_007_CA01_RENDER_INVENTORY.csv"
BROWSER_QA = "qa/ROBERTS_001_030_FOMBERG_001_007_CA01_BROWSER_QA.json"
BACKEND_CUMULATIVE = "qa/BACKEND_APPEND_ONLY_CUMULATIVE_ASSESSMENT_001_CUMULATIVE_RECEIPT.json"
...
PDF_NAME = "00_TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_007_CA01_READER.pdf"
HTML_NAME = "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_007_CA01_READER.html"
SOURCE_ZIP_NAME = "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_007_CA01_EDITABLE_SOURCE_BACKEND.zip"
QA_ZIP_NAME = "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_007_CA01_QA_PROVENANCE.zip"
...
def assert_safe_bytes(
    ...
    *,
    ...
    producer = "scripts/extend-backend-cumulative-assessment-001.py"
    if normalized.endswith(producer) or normalized.endswith(f"{producer} as {producer}"):
        marker = b"authoriza" + b"tion: bearer"
        if data.lower().count(marker) != 1:
            raise RuntimeError("CA01 producer credential-test marker census drift")
    ...
    entries.update(
        {
            ...
            "scripts/package-release-roberts-001-030-fomberg-001-007-ca01.py": SCRIPT,
            "scripts/publish-zenodo-roberts-001-030-fomberg-001-007-ca01.py": LANE / "scripts/publish-zenodo-roberts-001-030-fomberg-001-007-ca01.py",
        }
    )
    return entries
...
def source_path_inventory() -> set[str]:
    ...
    paths.remove("scripts/package-release-roberts-001-030-fomberg-001-007-ca01.py")
    return paths
    ...
    paths.update(
        {
            ...
            ROUTE_CENSUS,
            PROOF_CENSUS,
            "qa/BACKEND_APPEND_ONLY_CUMULATIVE_ASSESSMENT_001_FILE_MANIFEST.csv",
            "qa/BACKEND_APPEND_ONLY_CUMULATIVE_ASSESSMENT_001_PLAN.json",
            "qa/BACKEND_APPEND_ONLY_CUMULATIVE_ASSESSMENT_001_RECEIPT.json",
            "qa/BACKEND_APPEND_ONLY_CUMULATIVE_ASSESSMENT_001_REPLAY_RECEIPT.json",
            BACKEND_CUMULATIVE,
            BUILD_RECEIPT,
            VISUAL_QA,
            RENDER_INVENTORY,
...
def release_control_inventory() -> set[str]:
    prefix = "release/zenodo-roberts-001-030-fomberg-001-007-ca01/"
    return {
        prefix + name
        for name in (
            "metadata.json",
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[str]]:
    ...
    for marker in (
        "30/30", "O012-FOM-001–007", "baris 31–4185", "baris 4186",
        "D60-CA01", "8/8", "D60-CA02", "D60-CA03", "empat laboratorium",
    ...
    if (
        ...
        or backend.get("expected_records") <= 6742
    ...
    if readers["pdf"].get("pages") != 477:
        raise RuntimeError("publication plan must bind the 477-page CA01 reader")

    template = json.loads((RELEASE / "release-manifest.template.json").read_text(encoding="utf-8"))
    expected_status = "roberts_complete_fomberg_units_001_007_complete_ca01_complete_composite_course_partial"
    if (
    ...
    if (
        ...
        or numbering.get("original_edition_unit_ids") != ["O012-ORIG-CA01"]
...
def verify_backend_receipt(backend_facts: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        receipt.get("status") != "PASS" or receipt.get("assessment_id") != "D60-CA01"
        or receipt.get("edition_unit_id") != "O012-ORIG-CA01"
        ...
        raise RuntimeError("CA01 cumulative backend receipt/live backend mismatch")
    prefix = receipt.get("immutable_prefix", {})
    if (
        prefix.get("preserved_exactly") is not True or prefix.get("records") != 6742
        or prefix.get("bytes") != 8213649
        or prefix.get("bundle_sha256") != "523b570517eb54720c50007aacc5d4eea525ea252b9ca1f6f45b027182354765"
    ):
        raise RuntimeError("published Unit007 backend prefix was not preserved")
    ...
    if (
        ...
        or semantic.get("original_source_demarcation") != "PASS"
        or semantic.get("rights_closure") != "PASS"
        or semantic.get("route_mapping") != "PASS"
        or semantic.get("segment_kind_counts") != {"assessment": 1, "exercise": 8, "hint": 8, "solution": 8}
    ):
        raise RuntimeError("CA01 backend semantic/replay gates are incomplete")
...
def verify_censuses() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        or deficits.get("ordinary_missing_triples") != 36
    ):
        raise RuntimeError("route mastery census does not prove CA01 or remaining scope")
...
def verify_build_receipt(artifact_rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("qa_id") != "O012-RBT-001-030-FOM-001-007-CA01-COMPOSITE-BUILD"
        or receipt.get("model_provenance") != MODEL_NOTE
    ):
        raise RuntimeError("final CA01 build receipt identity/status mismatch")
    ...
    if artifacts.get("pdf", {}).get("pages") != 477 or artifacts.get("pdf", {}).get("tagged") is not False:
        raise RuntimeError("final CA01 PDF boundary/limitation drift")
    ca01 = receipt.get("ca01", {})
    if (
        ca01.get("assessment_id") != "D60-CA01" or ca01.get("edition_unit_id") != "O012-ORIG-CA01"
        or ca01.get("census", {}).get("exercises") != 8
        or ca01.get("census", {}).get("hints") != 8
        or ca01.get("census", {}).get("complete_solutions") != 8
        or not base.identity_matches(ca01.get("source"), base.lane_path(CA01_SOURCE))
    ):
        raise RuntimeError("final build receipt CA01 scope drift")
    ...
    if (
        ...
        or html.get("ca01_stable_ids") != 34
        or html.get("ca01_exercise_hint_solution_triples") != 8
        or html.get("centered_reflow_css_markers") is not True
    ):
        raise RuntimeError("final HTML gate is incomplete")
    pdf = receipt.get("pdf_checks", {})
    if (
        pdf.get("status") not in {"PASS", "PASS_STRUCTURAL"}
        or pdf.get("pages") != 477 or pdf.get("appended_ca01_pages") != 5
        or pdf.get("all_pages_a4") is not True
        or pdf.get("all_fonts_embedded_subset_tounicode") is not True
        or pdf.get("predecessor_outline_destinations_preserved_as_exact_prefix") is not True
        or pdf.get("all_outline_destinations_resolve") is not True
        or pdf.get("all_named_destinations_resolve") is not True
        or not isinstance(pdf.get("merged_named_destinations"), int)
        or pdf.get("merged_named_destinations") <= 2863
    ):
    ...
    for key in (
        ...
        "ca01_pdf_builds_byte_identical", "merged_pdf_builds_byte_identical",
...
def verify_browser_qa() -> dict[str, Any]:
    ...
    if (
        receipt.get("qa_id") != "O012-RBT-001-030-FOM-001-007-CA01-BROWSER-QA"
        ...
        or receipt.get("mobile", {}).get("ca01_non_math_block_escapes") != 0
    ):
        raise RuntimeError("independent desktop/mobile browser QA is incomplete")
    semantic = receipt.get("semantic_and_navigation", {})
    if (
        semantic.get("live_dom_ids") != semantic.get("unique_live_dom_ids")
        or semantic.get("unresolved_fragment_links") != 0
        or semantic.get("ca01_exercises") != 8 or semantic.get("ca01_hints") != 8
        or semantic.get("ca01_complete_solutions") != 8
...
def verify_visual_qa() -> dict[str, Any]:
    ...
    required_markers = (
        "Status: **PASS**", "477 pages", f"SHA-256 `{base.sha(pdf_path)}`",
        "all 389 outline entries resolve", "all 2,873 named destinations are present",
    ...
    if (
        len(rows) != 6 or [int(row["physical_page"]) for row in rows] != list(range(472, 478))
        or any(row.get("visual_status") != "PASS" for row in rows)
        or any(int(row.get("width_px", 0)) != 1241 or int(row.get("height_px", 0)) != 1754 for row in rows)
...
def write_package_receipt(frozen: dict[str, dict[str, Any]], source_zip: dict[str, Any], qa_zip: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "release_id": RELEASE_ID,
        "scope": "Roberts 30/30 complete; selected Fomberg 1.1-1.13 complete; D60-CA01 complete 8/8; composite course partial",
        ...
        "verification": {
            ...
            "original_ca01_license": "CC BY-SA 4.0", "new_concept_created": False,
        ...
        manifest.update(
            {
                ...
                "reader_qa": {
                    "status": "PASS", "pdf_pages": 477, "pdf_tagged": False,
                    "pdf_all_fonts_embedded_subset_tounicode": True,
                    "html_unique_dom_ids": build_receipt["html_checks"]["dom_ids"],
                ...
                "backend": {**backend_facts, "cumulative_receipt": base.identity(base.lane_path(BACKEND_CUMULATIVE)), "assessment_id": backend_receipt["assessment_id"]},
