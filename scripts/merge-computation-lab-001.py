#!/usr/bin/env python3
"""Deterministically append computation Lab 1 to the frozen 501-page reader."""
...
PRIOR_BYTES = 8_915_996
PRIOR_SHA256 = "74ed9b5bf0f79a98693369dc7beba3e84ac81c711cc96b9951ae950ae9632a16"
PRIOR_PAGES = 501
PRIOR_OUTLINE = 416
PRIOR_NAMED = 3_056
PYPDF_VERSION = "6.12.2"
SCRATCH_BASENAME = "roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01-build"
...
def source_contract(path: Path) -> tuple[list[tuple[int, str, str]], list[str]]:
    ...
    headings = [
        ...
        for match in re.finditer(
            r"(?m)^(#{1,2})\s+(.+?)\s+\{#(o012-d60-lab01(?:-[a-z0-9]+)*)\}\s*$",
            text,
        )
    ]
    ids = re.findall(r"#(o012-d60-lab01(?:-[a-z0-9]+)*)\b", text)
    require(len(ids) == len(set(ids)) == 24, "Lab 1 stable-ID inventory is not 24 unique IDs")
    require(len(headings) == 17, "Lab 1 heading inventory is not 17")
    require(headings[0][0] == 1 and sum(level == 1 for level, _title, _ident in headings) == 2, "Lab 1 heading hierarchy lacks its two top-level surfaces")
    require(all(level in (1, 2) for level, _title, _ident in headings), "Lab 1 heading level is outside 1-2")
...
def merge(prior_path: Path, append_path: Path, output_path: Path, source: Path, scratch_root: Path) -> dict[str, Any]:
    ...
    require(len(appendix_outline) == len(headings), "appendix outline does not match 17 source headings")
    require([title for _level, title, _ident in headings] == [title for title, _page in appendix_outline], "appendix outline titles differ from source")
    ...
    require(all(name in appendix_named_pages and appendix_named_pages[name] is not None for name in stable_ids), "appendix lacks one or more Lab 1 destinations")
    ...
    require(set(stable_ids).isdisjoint(prior_named_pages), "Lab 1 stable ID collides with predecessor")
    ...
    writer.add_metadata({
        "/Title": "Topologi Aljabar — Roberts 30/30, Fomberg 1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1",
        "/Author": "David Michael Roberts; Yeheli Fomberg; edisi Bahasa Indonesia dengan pendamping penguasaan",
        "/Subject": "Checkpoint komposit parsial dengan 108 soal bersolusi dan Laboratorium Komputasi 1",
    ...
    require(all(merged_named_pages.get(name) == PRIOR_PAGES + int(appendix_named_pages[name]) for name in stable_ids), "Lab 1 named destinations changed")
