#!/usr/bin/env python3
"""Replay Unit 006 and write deterministic semantic/cumulative receipts."""
...
VALIDATOR_PATH = LANE / "scripts/validate-backend-append-only-fomberg-unit-006.py"
OUTPUTS = {
    "manifest": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_FILE_MANIFEST.csv",
    "semantic_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_RECEIPT.json",
    "semantic_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_RECEIPT.md",
    "cumulative_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_CUMULATIVE_RECEIPT.json",
    "cumulative_md": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_CUMULATIVE_RECEIPT.md",
    "replay_json": LANE / "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_REPLAY_RECEIPT.json",
}
ROBERTS_BOUNDARY = (
    4761, 5213679,
    "51b8c7f611560b9b5e88e97b3a943b54e0eb687c703f0cd34c1a3d164e4fb920",
)
PRIOR_FOMBERG_STABLE_IDS = 476
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 006 cumulative validator FAIL: {message}")
...
def replay_producer(producer, c) -> tuple[int, int, str]:
    with tempfile.TemporaryDirectory(prefix="o012-fom-u006-backend-replay-") as temporary:
        ...
        require("Fomberg Unit 006 semantic backend extension: PASS" in captured.getvalue(), "producer replay PASS marker absent")
        bundle = hashlib.sha256(); records = total_bytes = 0
        for name in c.FILES:
...
def write_outputs(c, identities, additions, rows, final, script_results) -> None:
    ...
    for key, path in (
        ("common", LANE / "scripts/fomberg-unit-006-common.py"),
    ...
    semantic_md = (
        "# Fomberg Unit 006 append-only backend receipt\n\n"
        "Status: **PASS**\n\n"
        f"- Immutable Unit 005 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / {c.PREFIX_TOTAL[2]}.\n"
        f"- Unit 006 semantic suffix: {sum(counts.values())} records, including {c.EXPECTED_STABLE_IDS} unit and {c.EXPECTED_STABLE_IDS} segment records.\n"
        ...
        f"- Reader: {c.EXPECTED_STABLE_IDS} stable IDs; eleven source diagram functions; seven paired accessible redraws; mastery: six solved triples.\n"
        "- No new proof-repair ID was required. Cursor: source line 3518; source EOF: false.\n"
    ).encode("utf-8")
    OUTPUTS["semantic_md"].write_bytes(semantic_md)
    semantic = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-006-SEMANTIC-APPEND-ONLY",
        ...
        "closure": {
            ...
            "source_diagram_functions": 11, "semantic_figure_blocks": 7,
            "redraw_files": 14, "mastery_triples": 6,
            "proof_repairs": [], "next_source_line": 3518,
    ...
    cumulative_md = (
        "# Cumulative backend receipt through Fomberg Unit 006\n\n"
        "Status: **PASS**\n\n"
        f"- Preserved Fomberg Unit 005 boundary: {c.PREFIX_TOTAL[0]:,} records / {c.PREFIX_TOTAL[1]:,} bytes / {c.PREFIX_TOTAL[2]}.\n"
        f"- Fomberg Unit 006 append-only suffix: {sum(counts.values())} records.\n"
        f"- Cumulative backend: {final[0]:,} records / {final[1]:,} bytes / {final[2]}.\n"
        f"- Fomberg Units 001–006: {PRIOR_FOMBERG_STABLE_IDS + c.EXPECTED_STABLE_IDS} stable reader IDs mirrored one-to-one as unit and segment records; 90 source diagram functions; 77 semantic figure blocks; 37 solved mastery triples.\n"
        "- Closed proof repairs remain FOM-PR-01 through FOM-PR-12. Cursor: source line 3518; source EOF: false.\n"
        "- Deterministic producer replay and semantic append-only validation: **PASS**.\n"
    ).encode("utf-8")
    OUTPUTS["cumulative_md"].write_bytes(cumulative_md)
    cumulative = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-THROUGH-FOMBERG-UNIT-006-CUMULATIVE-SEMANTIC",
        "status": "PASS",
        "scope": "Roberts Units 001-030 immutable prefix plus Fomberg Units 001-006 append-only semantic suffixes",
        ...
        "nested_immutability": {
            ...
            "fomberg_unit_006_suffix": {"records": sum(counts.values()), "records_by_file": counts},
        },
        "current": {
            "total_records": final[0], "total_bytes": final[1],
            "bundle_sha256": final[2], "backend_files": backend_files,
            ...
            "source_diagram_functions": 90, "semantic_figure_blocks": 77,
            "redraw_files": 32, "mastery_triples": 37,
            "proof_repairs_closed": [f"FOM-PR-{number:02d}" for number in range(1, 13)],
            "next_source_line": 3518, "terminal_source_eof": False,
            "review_counts": {"P1": 0, "P2": 0, "P3": 0},
        },
    ...
    replay = {
        "schema_version": "1.0.0",
        "receipt_id": "O012-BACKEND-FOMBERG-UNIT-006-DETERMINISTIC-REPLAY",
        "status": "PASS",
...
def main() -> int:
    ...
    producer = load(PRODUCER_PATH, "o012_fomberg_u006_producer_cumulative")
    ...
    require("Fomberg Unit 006 append-only backend validation: PASS" in result.stdout, "semantic validator PASS marker absent")
    ...
    print("Cumulative backend validation through Fomberg Unit 006: PASS")
    print(f"unit005_prefix_records={c.PREFIX_TOTAL[0]}")
    print(f"unit006_records_added={sum(counts.values())}")
