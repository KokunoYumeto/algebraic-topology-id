#!/usr/bin/env python3
"""Deterministically append computation Lab 4 to the frozen 545-page reader."""
...
PRIOR_BYTES = 9_836_725
PRIOR_SHA256 = "b26b670db97facc9f5ab389eed69cf1f8b03f70e6047eacbd2bfa68c849ccd0d"
PRIOR_PAGES = 545
PRIOR_OUTLINE = 469
PRIOR_NAMED = 3_130
PYPDF_VERSION = "6.12.2"
SCRATCH_BASENAME = "roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01-lab02-lab03-lab04-build"
...
def load_base():
    ...
    require(
        ...
        == (
            14_091,
            302,
            "88fc19a95f9066dfb69c67afd69eff4a066c9d6bdc9c7175cc28820f761a5746",
        ),
        "frozen PDF merge-helper identity drift",
    )
...
def source_contract(path: Path) -> tuple[list[tuple[int, str, str]], list[str]]:
    ...
    headings = [
        ...
        for match in re.finditer(
            r"(?m)^(#{1,2})\s+(.+?)\s+\{#(o012-d60-lab04(?:-[a-z0-9]+)*)\}\s*$",
            text,
        )
    ]
    ids = re.findall(r"#(o012-d60-lab04(?:-[a-z0-9]+)*)\b", text)
    require(len(ids) == len(set(ids)) == 25, "Lab 4 stable-ID inventory is not 25 unique IDs")
    require(len(headings) == 18, "Lab 4 heading inventory is not 18")
    require(
        headings[0][0] == 1 and sum(level == 1 for level, _title, _ident in headings) == 2,
        "Lab 4 heading hierarchy lacks its two top-level surfaces",
    )
    require(all(level in (1, 2) for level, _title, _ident in headings), "Lab 4 heading level is outside 1-2")
    require({ident for _level, _title, ident in headings}.issubset(ids), "heading ID absent from full stable-ID inventory")
    return headings, ids


def bookmark_title(source_title: str) -> str:
    """Match Pandoc's bounded plain-text bookmark form for Lab 4 headings."""
    require(source_title.count("$") % 2 == 0, "unbalanced inline-math delimiter in heading")
    title = source_title.replace("$", "")
    require("\\" not in title and "{" not in title and "}" not in title, "unsupported TeX remains in bookmark title")
    return title
...
def merge(prior_path: Path, append_path: Path, output_path: Path, source: Path, scratch_root: Path) -> dict[str, Any]:
    ...
    require(
        prior_path.stat().st_size == PRIOR_BYTES and digest(prior_path) == PRIOR_SHA256,
        "frozen predecessor PDF identity drift",
    )
    ...
    source_outline_titles = [bookmark_title(title) for _level, title, _ident in headings]
    ...
    require(
        all(page is not None and 0 <= page < len(appendix.pages) for _title, page in appendix_outline),
        "appendix outline has unresolved pages",
    )

    appendix_named = appendix.named_destinations
    appendix_named_pages = {
        name: appendix.get_destination_page_number(destination)
        for name, destination in appendix_named.items()
    }
    require(
        all(name in appendix_named_pages and appendix_named_pages[name] is not None for name in stable_ids),
        "appendix lacks one or more Lab 4 destinations",
    )
    prior_named_pages = {
        name: prior.get_destination_page_number(destination)
        for name, destination in prior.named_destinations.items()
    }
    require(set(stable_ids).isdisjoint(prior_named_pages), "Lab 4 stable ID collides with predecessor")

    external_targets = [
        target
        for target in base.link_targets(appendix, 0)
        if target not in stable_ids and target not in prior_named_pages
    ]
    ...
    current_parent = None
    for (level, title, _ident), (pdf_title, local_page) in zip(headings, appendix_outline, strict=True):
        require(bookmark_title(title) == pdf_title, "source/PDF bookmark title drift")
        page_number = PRIOR_PAGES + int(local_page)
        if level == 1:
            current_parent = writer.add_outline_item(pdf_title, page_number, is_open=True)
        else:
            require(current_parent is not None, "level-2 outline item precedes a level-1 parent")
            writer.add_outline_item(pdf_title, page_number, parent=current_parent)

    writer.add_metadata(
        {
            "/Title": "Topologi Aljabar — Roberts 30/30, Fomberg 1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–4",
            "/Author": "David Michael Roberts; Yeheli Fomberg; edisi Bahasa Indonesia dengan pendamping penguasaan",
            "/Subject": "Checkpoint komposit parsial dengan 108 soal bersolusi dan Laboratorium Komputasi 1–4",
    ...
    merged_named_pages = {
        name: merged.get_destination_page_number(destination)
        for name, destination in merged.named_destinations.items()
    }
    ...
    require(
        merged_outline[:PRIOR_OUTLINE] == prior_outline
        and len(merged_outline) == PRIOR_OUTLINE + len(headings),
        "predecessor outline is not exact prefix",
    )
    require(len(merged_named_pages) == PRIOR_NAMED + len(stable_ids), "merged named-destination census drift")
    require(
        all(merged_named_pages.get(name) == page for name, page in prior_named_pages.items()),
        "predecessor named destinations changed",
    )
    require(
        all(
            merged_named_pages.get(name) == PRIOR_PAGES + int(appendix_named_pages[name])
            for name in stable_ids
        ),
        "Lab 4 named destinations changed",
    )
...
def main() -> None:
    ...
    result = merge(
        *(Path(value).resolve() for value in (args.prior, args.append, args.output, args.source, args.scratch_root))
    )
