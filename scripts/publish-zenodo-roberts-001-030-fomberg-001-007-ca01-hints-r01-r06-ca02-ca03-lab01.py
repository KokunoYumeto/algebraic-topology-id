#!/usr/bin/env python3
"""Publish v0.31.3 in the existing Zenodo concept and verify every byte twice.

With no arguments this uses the proved transaction engine: it reads the token
only at runtime, creates one new version from public record 22135136, uploads
...
TEMPLATE_PATH = SCRIPT.with_name(
    "publish-zenodo-roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03.py"
)
SPEC = importlib.util.spec_from_file_location("o012_v0312_publisher_template", TEMPLATE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved v0.31.2 publisher")
...
base.SEED_RECORD = 22135136
base.CONCEPT_DOI = "10.5281/zenodo.22061489"
base.MODEL_NOTE = "OpenAI Codex gpt-5.6-sol, Ultra"
engine.PREVIOUS_RECORD_ID = 22135136
engine.PREVIOUS_DOI = "10.5281/zenodo.22135136"
base.RELEASE_ID = f"o012-composite-id-{SLUG}-v0.31.3"
base.TITLE = (
...
FINAL_BACKEND_RECEIPT = "qa/BACKEND_APPEND_ONLY_COMPUTATION_LAB_001_CUMULATIVE_RECEIPT.json"
MANIFEST_STATUS = (
...
def assert_metadata(metadata: dict[str, Any], *, exact_people: bool = True) -> None:
    ...
    for marker in ("108/108", "D60-LAB01", "Laboratorium Komputasi 1", base.MODEL_NOTE):
...
def verify_local() -> tuple[dict, dict]:
    ...
    if not (
        ...
        raise RuntimeError("publication plan escaped the exact v0.31.2 predecessor")
    ...
    if not (
        ...
        == 24
        and labs.get("completed") == ["D60-LAB01"]
        and labs.get("edition_unit_ids") == ["O012-ORIG-LAB01"]
        and labs.get("required") == 4
        and labs.get("completed_count") == 1
        and labs.get("remaining") == ["D60-LAB02", "D60-LAB03", "D60-LAB04"]
        and labs.get("tasks") == 6
        and labs.get("tests") == 6
        and labs.get("stable_ids") == 24
        ...
        == 108
        and closure.get("computation_laboratories_complete") == 1
        and closure.get("computation_laboratories_required") == 4
        and closure.get("computation_labs_remaining") == 3
        ...
        and backend.get("records") == 7404
        and backend.get("bytes") == 8975700
...
engine.verify_public = template.verify_public
engine.make_receipt = make_receipt
base.verify_local = verify_local
base.verify_public = template.verify_public
