#!/usr/bin/env python3
"""Read-only validator for the exact Fomberg Unit 004 append-only suffix."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 004 backend validator FAIL: {message}")


def load_producer():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u004_producer_for_validator", PRODUCER_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 004 producer")
...
def load_exact_partition(c, expected_suffixes: dict[str, bytes]):
    ...
    for name in c.FILES:
        ...
        require(
            (len(prefix.splitlines()), len(prefix), c.digest(prefix))
                == (expected_count, boundary, prefix_sha),
            f"{name}: immutable Unit 003 prefix partition mismatch",
        )
        require(suffix == expected_suffixes[name], f"{name}: live suffix differs from deterministic reconstruction")
        require(len(suffix.splitlines()) == c.DELTA[name], f"{name}: suffix count mismatch")
        ...
        file_results[name] = {
            "prefix_records": expected_count,
            "prefix_bytes": boundary,
    ...
    require(final_count == c.PREFIX_TOTAL[0] + sum(c.DELTA.values()) == 6113, "cumulative record count mismatch")
...
def main() -> int:
    ...
    require(sum(len(records) for records in additions.values()) == 366, "common module did not reconstruct exactly 366 records")
    prefix_records, all_records, files, final = load_exact_partition(c, expected_suffixes)
    _, reconstructed = producer.validate_merged(c, data, identities, prefix_records, additions)
    require(
        {record["id"]: record for record in reconstructed}
            == {record["id"]: record for record in all_records},
        "live records differ semantically from reconstructed records",
    )
    for name in c.FILES:
        suffix = (BACKEND / name).read_bytes()[c.PREFIX[name][1]:]
        parsed = parse_canonical_records(c, f"{name}:suffix-recheck", suffix)
        require(
            [record["id"] for record in parsed] == [record["id"] for record in additions[name]],
            f"{name}: suffix ID order mismatch",
        )
    # Evidence and controls must remain byte-identical for the whole pass.
    ...
    for name in c.FILES:
        ...
        require(
            (BACKEND / name).read_bytes()[c.PREFIX[name][1]:] == refreshed_suffix,
            f"{name}: live suffix/evidence binding changed during validation",
        )

    print("Fomberg Unit 004 append-only backend validation: PASS")
