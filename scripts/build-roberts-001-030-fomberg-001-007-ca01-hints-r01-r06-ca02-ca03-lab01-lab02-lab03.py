#!/usr/bin/env python3
"""Deterministically append the verified computation Lab 3 reader surface."""
...
PRIOR_HTML = ROOT / "output/html/roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01-lab02/index.html"
PRIOR_PDF = ROOT / "output/pdf/topologi-aljabar-roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03-lab01-lab02-id.pdf"
...
SOURCE_QA = ROOT / "qa/COMPUTATION_LAB_003_QA.json"
BACKEND_RECEIPT = ROOT / "qa/BACKEND_APPEND_ONLY_COMPUTATION_LAB_003_CUMULATIVE_RECEIPT.json"
MERGER = ROOT / "scripts/merge-computation-lab-003.py"
...
MANIFEST_OUT = ROOT / "output/ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_007_CA01_HINTS_R01_R06_CA02_CA03_LAB01_LAB02_LAB03.csv"
DRAFT_RECEIPT = ROOT / "qa/ROBERTS_001_030_FOMBERG_001_007_CA01_HINTS_R01_R06_CA02_CA03_LAB01_LAB02_LAB03_BUILD_DRAFT.json"
SCRATCH = ROOT / f"tmp/pdfs/{SLUG}-build"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"

PRIOR_HTML_ID = (15_615_104, "d0c6afddfa92759d475258bf08f20ea4019eccf72b7554128b2b938bd247b375")
PRIOR_PDF_ID = (9_507_127, "1bad03f9ba031ba91967a0a0ac2af6d15a0f768882cd541fe26dcbe26c4edd0b")
PRIOR_PAGES = 529
PRIOR_OUTLINE = 451
PRIOR_NAMED = 3_105
BACKEND_PREFIX_BYTES = 9_122_755
BACKEND_PREFIX_RECORDS = 7_546
BACKEND_PREFIX_SHA256 = "ac3a0377861ed2b728f9c7473579fdd4febe43e454a92f3ea06451e13d46c8f8"
SOURCE_QA_ID = (4_534, "a13bc301036c0d2cbfb6c92ab1423d2c2ca09a503bfa2a830a62832a0c4bf12f")
BACKEND_RECEIPT_ID = (10_047, "7ee69a9291368d407e38de4c63440d599b5cd13ec5fc288f5468084bc7774c80")
MERGER_ID = (10_292, "f9b9a2d11131153a15f28d1a763a8d5b17f4c6fca7c8eef93cfcea13feb8512c")
BACKEND_CUMULATIVE_ID = (7_694, 9_280_385, "cddd65499da547e0c4f01b8a880f68d1c3d314c078a9179528e4a28b2c5f65a2")
...
def write_atomic(destination: Path, source: Path) -> None:
    ...
    pending = destination.parent / f".{destination.name}.lab03.pending"
    backup = destination.parent / f".{destination.name}.lab03.rollback"
...
def expand_source() -> str:
    ...
    replacements = {
        "O012_LAB03_INCLUDE_PROGRAM": f"```python\n{program}\n```",
        "O012_LAB03_INCLUDE_TESTS": f"```python\n{tests}\n```",
        "O012_LAB03_INCLUDE_EXPECTED": f"```text\n{expected}\n```",
    }
    for marker, value in replacements.items():
        require(source.count(marker) == 1, f"include marker drift: {marker}")
        source = source.replace(marker, value)
    link_replacements = {
        "[`o012_d60_lab03_cellular_degree.py`](o012_d60_lab03_cellular_degree.py)": "`o012_d60_lab03_cellular_degree.py`",
        "[`test_o012_d60_lab03_cellular_degree.py`](test_o012_d60_lab03_cellular_degree.py)": "`test_o012_d60_lab03_cellular_degree.py`",
        "[`expected-output-lab03.txt`](expected-output-lab03.txt)": "`expected-output-lab03.txt`",
        "../units/unit-030-lecture-030.md#o012-rbt-l30-s04": "#o012-rbt-l30-s04",
        "../fomberg/units/fomberg-unit-005-degree-maps-local-degree.md#o012-fom-u005-s11a": "#o012-fom-u005-s11a",
        "../fomberg/units/fomberg-unit-005-degree-maps-local-degree.md#o012-fom-u005-local-degree": "#o012-fom-u005-local-degree",
        "../fomberg/units/fomberg-unit-007-cellular-homology.md#o012-fom-u007": "#o012-fom-u007",
    }
    for old, new in link_replacements.items():
        require(source.count(old) == 1, f"canonical file link drift: {old}")
        source = source.replace(old, new)
    require("O012_LAB03_INCLUDE_" not in source, "unresolved Lab 3 include marker")
    return source
...
def main() -> int:
    ...
    require((SOURCE_QA.stat().st_size, digest(SOURCE_QA)) == SOURCE_QA_ID, "frozen source QA identity drift")
    require((BACKEND_RECEIPT.stat().st_size, digest(BACKEND_RECEIPT)) == BACKEND_RECEIPT_ID, "frozen backend receipt identity drift")
    require((MERGER.stat().st_size, digest(MERGER)) == MERGER_ID, "frozen Lab 3 merger identity drift")
    qa = json.loads(disciplined(SOURCE_QA))
    require(qa.get("status") == "PASS" and qa.get("laboratory_id") == "D60-LAB03", "source QA gate failed")
    require(qa.get("receipt_kind") == "computation_laboratory_source_execution_review_closure", "source QA receipt kind drift")
    require(qa.get("edition_unit_id") == "O012-ORIG-LAB03" and qa.get("course_route_unit_ids") == ["D60-R12", "D60-R14"], "source QA route/edition scope drift")
    ...
    require(qa_checks.get("route_scope_D60_R12_R14") == "PASS", "source QA route gate failed")
    ...
    require(backend.get("laboratory_id") == "D60-LAB03" and backend.get("edition_unit_id") == "O012-ORIG-LAB03", "backend Lab 3 identity drift")
    ...
    require((cumulative_records, cumulative_bytes, cumulative_sha256) == BACKEND_CUMULATIVE_ID, "backend cumulative boundary identity drift")
    require(cumulative.get("computation_laboratories_complete") == 3 and cumulative.get("computation_laboratories_required") == 4, "backend laboratory completion census drift")
    ...
    require(replay.get("laboratory_id") == "D60-LAB03" and replay.get("receipt_kind") == "isolated_exact_backend_replay", "backend replay identity drift")
    ...
    require(semantic.get("stable_ids") == 25 and semantic.get("tasks") == 6 and semantic.get("hints") == 1, "backend Lab 3 semantic census drift")
    ...
    try:
        ...
        expanded_path = SCRATCH / "lab03-expanded.md"
        expanded_path.write_text(expanded, encoding="utf-8", newline="\n")
        fragment_a = SCRATCH / "lab03-a.html"
        fragment_b = SCRATCH / "lab03-b.html"
        html_args = [
        ...
        require(digest(fragment_a) == digest(fragment_b), "Lab 3 HTML fragments differ")
        ...
        for ident in code_ids:
            fragment = fragment.replace(f'id="{ident}"', f'id="lab03-{ident}"')
            fragment = fragment.replace(f'href="#{ident}"', f'href="#lab03-{ident}"')
        require(re.search(r'\bid="cb[0-9]+(?:-[0-9]+)?"', fragment) is None, "unprefixed Lab 3 code-line ID remains")
        ids = re.findall(r'\bid="(o012-d60-lab03(?:-[a-z0-9]+)*)"', fragment)
        ...
        old_title = "<title>Topologi Aljabar — Roberts 30/30, Fomberg 1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–2</title>"
        new_title = "<title>Topologi Aljabar — Roberts 30/30, Fomberg 1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–3</title>"
        old_heading = '<h1 class="title">Topologi Aljabar — Roberts 30/30, Fomberg' + nl + '1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–2</h1>'
        new_heading = '<h1 class="title">Topologi Aljabar — Roberts 30/30, Fomberg' + nl + '1.1–1.13, Asesmen Kumulatif 1–3, dan Laboratorium Komputasi 1–3</h1>'
        old_subtitle = '<p class="subtitle">Komponen Roberts lengkap; jembatan Fomberg melalui' + nl + 'homologi seluler; 108 soal bersolusi; Laboratorium Komputasi 1–2 lengkap dan dapat dijalankan luring; checkpoint komposit parsial</p>'
        new_subtitle = '<p class="subtitle">Komponen Roberts lengkap; jembatan Fomberg melalui' + nl + 'homologi seluler; 108 soal bersolusi; Laboratorium Komputasi 1–3 lengkap dan dapat dijalankan luring; checkpoint komposit parsial</p>'
        ...
        toc_insert = nl + '<li><a href="#o012-d60-lab03" id="toc-o012-d60-lab03">Laboratorium Komputasi 3 — Batas Seluler dan Derajat</a></li>'
        combined = combined[:nav_pos] + toc_insert + combined[nav_pos:]

        old_status = "Laboratorium Komputasi 1–2 kini lengkap. Jalur komposit masih parsial" + nl + "karena Laboratorium Komputasi 3–4, penutupan metadata bukti, dan capstone" + nl + "belum selesai; kursor sumber berikutnya tetap pada baris 4186."
        new_status = "Laboratorium Komputasi 1–3 kini lengkap. Jalur komposit masih parsial" + nl + "karena Laboratorium Komputasi 4, penutupan metadata bukti, dan capstone" + nl + "belum selesai; kursor sumber berikutnya tetap pada baris 4186."
        ...
        status_insert = nl + '<aside id="o012-d60-lab03-checkpoint-status" class="note" data-origin="edition-original">' + nl + '<p><strong>Tambahan checkpoint.</strong> Laboratorium Komputasi 3 menambahkan program Python berpustaka standar, enam uji deterministik, keluaran acuan, interpretasi, petunjuk, dan solusi lengkap untuk batas seluler, serat torus eksak, tanda derajat lokal, komposisi, serta kontrol singular. Lapisan asli edisi ini berlisensi CC BY-SA 4.0 dan tidak memakai bank masalah Fomberg.</p>' + nl + '</aside>'
        ...
        require('id="toc-o012-d60-lab03"' in combined and 'id="o012-d60-lab03"' in combined, "Lab 3 ToC/source missing")
        ...
        require("O012_LAB03_INCLUDE_" not in combined, "unresolved Lab 3 include marker in HTML")

        pdf_text = re.sub(r"\A---\n.*?\n---\n", "", expanded, count=1, flags=re.DOTALL)
        # pdfLaTeX listings cannot encode U+2013 inside \lstinline.  Normalize
        # only these five bounded source-locator code spans in the PDF staging
        # copy; the canonical source and semantic HTML retain their en dashes.
        for locator in (
            "algebraic_topology.tex:2849–2875",
            ":3037–3114",
            ":3612–3664",
            ":3740–3845",
            "Notes.tex:6327–6348",
        ...
        plain_lab_heading = "# Laboratorium Komputasi 3 — Batas Seluler dan Derajat {#o012-d60-lab03}"
        balanced_lab_heading = (
            "# Laboratorium Komputasi 3 — Batas Seluler \\newline "
            "dan Derajat {#o012-d60-lab03}"
        )
        pdf_text = replace_once(pdf_text, plain_lab_heading, balanced_lab_heading, "balanced PDF Lab 3 heading")
        fence_re = re.compile(r"(?m)^(::: \{\.(?:exercise|hint) )#(o012-d60-lab03(?:-[a-z0-9]+)*)([^\r\n]*\})$")
        require(len(fence_re.findall(pdf_text)) == 7, "PDF fenced stable-ID transform census drift")
        pdf_text = fence_re.sub(r"\1\3\n\n```{=latex}\n\\hypertarget{\2}{}\n\\par\\noindent\n```", pdf_text)
        heading_re = re.compile(r"(?m)^(#{1,2})\s+(.+?)\s+\{#(o012-d60-lab03(?:-[a-z0-9]+)*)\}\s*$")
        require(len(heading_re.findall(pdf_text)) == 18, "PDF heading stable-ID transform census drift")
        pdf_text = heading_re.sub(r"\1 \2\n\n```{=latex}\n\\hypertarget{\3}{}\n```", pdf_text)
        pdf_source = SCRATCH / "lab03-layout.md"
        pdf_source.write_text(pdf_text, encoding="utf-8", newline="\n")
        pdf_header = SCRATCH / "lab03-header.tex"
        ...
        work_pdf = SCRATCH / "lab03-work.pdf"
        append_a = SCRATCH / "lab03-a.pdf"
        append_b = SCRATCH / "lab03-b.pdf"
        pdf_args = [
            ...
            "--metadata=pagetitle:Laboratorium Komputasi 3", "--metadata=date:28 Agustus 2026",
        ...
        require(digest(append_a) == digest(append_b), "Lab 3 appendix PDF builds differ")
        ...
        require(digest(merged_a) == digest(merged_b), "merged Lab 3 PDFs differ")
        ...
        prior_text = SCRATCH / "prior-001-529.txt"
        merged_prefix_text = SCRATCH / "merged-001-529.txt"
        ...
        require("O012_LAB03_INCLUDE_" not in all_pdf_text, "unresolved Lab 3 include marker in PDF text")
        for required in ("Laboratorium Komputasi 3", "batas seluler", "torus_preimages", "derajat_multiplikatif", "CC BY-SA 4.0"):
