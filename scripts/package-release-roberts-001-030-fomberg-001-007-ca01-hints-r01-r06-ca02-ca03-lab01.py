#!/usr/bin/env python3
"""Seal and package the v0.31.3 computation-Lab-1 successor (local only).

This fail-closed successor reuses the proved v0.31.2 deterministic archive and
...
TEMPLATE_PATH = SCRIPT.with_name(
    "package-release-roberts-001-030-fomberg-001-007-ca01-hints-r01-r06-ca02-ca03.py"
)
SPEC = importlib.util.spec_from_file_location("o012_v0312_packager_template", TEMPLATE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved v0.31.2 packager")
...
INHERITED_VERIFY_ASSESSMENTS = template.verify_assessments
...
TITLE = (
    ...
    "Petunjuk Rute 1–6, dan Laboratorium Komputasi 1"
)
VERSION = "0.31.3"
RELEASE_ID = f"o012-composite-id-{SLUG}-v{VERSION}"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22135136
PREVIOUS_DOI = "10.5281/zenodo.22135136"
...
FROZEN_LEDGER_SHA256 = "853c30bdf2f48ecdf99ba2a59b8f3d6ed1608959a3b8c7434b65fad47ee03742"
...
LAB_QA = "qa/COMPUTATION_LAB_001_QA.json"
LAB_STATIC_QA = "qa/computation-lab-001/STATIC_QA.json"
LAB_MATH_REVIEW = "qa/computation-lab-001/INDEPENDENT_MATH_REVIEW.json"
LAB_LANGUAGE_REVIEW = "qa/computation-lab-001/INDEPENDENT_SOURCE_LANGUAGE_REVIEW.json"
LAB_EXECUTION_RECEIPT = "qa/computation-lab-001/EXECUTION_RECEIPT.json"
BACKEND_CUMULATIVE = "qa/BACKEND_APPEND_ONLY_COMPUTATION_LAB_001_CUMULATIVE_RECEIPT.json"

LAB_SOURCE = "source/id-ID/labs/computation-lab-001-monodromy-presentations.md"
LAB_PROGRAM = "source/id-ID/labs/o012_d60_lab01_monodromy.py"
LAB_TESTS = "source/id-ID/labs/test_o012_d60_lab01_monodromy.py"
LAB_EXPECTED = "source/id-ID/labs/expected-output-lab01.txt"
...
MANIFEST_STATUS = (
    ...
    "computation_lab_001_complete_composite_course_partial"
)
...
def source_entries() -> dict[str, Path]:
    ...
    additions = {
        ...
        LAB_TESTS,
        LAB_EXPECTED,
        "scripts/merge-computation-lab-001.py",
        "scripts/qa-computation-lab-001.py",
        "scripts/finalize-computation-lab-001-qa.py",
        "scripts/extend-backend-computation-lab-001.py",
        "scripts/validate-backend-append-only-computation-lab-001.py",
...
def verify_lab() -> dict[str, Any]:
    ...
    if not (
        qa.get("status") == "PASS"
        and qa.get("laboratory_id") == "D60-LAB01"
        and qa.get("edition_unit_id") == "O012-ORIG-LAB01"
        and qa.get("course_route_unit_ids") == ["D60-R04", "D60-R05", "D60-R06"]
        and checks.get("stable_ids_24_unique") == "PASS"
        ...
        raise RuntimeError("computation Lab 1 source/execution/review closure mismatch")
    ...
    for relative in expected:
        if relative not in indexed or not base.identity_matches(indexed[relative], lane_path(relative)):
            raise RuntimeError(f"Lab 1 QA has a stale source identity: {relative}")
    for relative in (LAB_STATIC_QA, LAB_MATH_REVIEW, LAB_LANGUAGE_REVIEW, LAB_EXECUTION_RECEIPT):
        review = load_json(relative)
        if not str(review.get("status", "")).startswith("PASS"):
            raise RuntimeError(f"Lab 1 review/execution receipt failed: {relative}")
    return qa
...
def verify_backend() -> dict[str, Any]:
    ...
    if not (
        ...
        and receipt.get("laboratory_id") == "D60-LAB01"
        and receipt.get("edition_unit_id") == "O012-ORIG-LAB01"
        and prefix.get("preserved_exactly") is True
        and prefix.get("records") == 7273
        and prefix.get("bytes") == 8840132
        and prefix.get("bundle_sha256")
        == "97edc6371a0bf670ebdaaa4fab8618ec138ae25c4bf54ca9172139934ba0b464"
        and final.get("records") == 7404
        and final.get("bytes") == 8975700
        ...
        and replay.get("final", {}).get("records") == 7404
        and replay.get("temporary_replay_removed") is True
    ):
        raise RuntimeError("Lab 1 cumulative backend boundary mismatch")
    return receipt
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        ...
        raise RuntimeError("v0.31.3 metadata identity/rights/creator drift")
    ...
    if not (
        ...
        raise RuntimeError("publication plan escaped the existing concept or v0.31.2 predecessor")
    ...
    if not (
        ...
        and binding.get("expected_records") == 7404
        and snapshot.get("total_records") == 7404
        and snapshot.get("total_bytes") == 8975700
        and snapshot.get("bundle_sha256")
        == "4740eb2ff83b4f9df3c0d90c2426ff77e652b23cad0bbe7763c54ebdefa60b4b"
    ):
        raise RuntimeError("publication plan does not bind the Lab 1 backend")
...
def verify_final_gates(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if build.get("status") != "PASS":
        raise RuntimeError("final Lab 1 build receipt did not pass")
    pdf = build.get("pdf", {})
    html = build.get("html", {})
    if not base.identity_matches(pdf, lane_path(PDF_INPUT)) or not base.identity_matches(html, lane_path(HTML_INPUT)):
        raise RuntimeError("build receipt does not bind the final Lab 1 readers")
    if pdf.get("pages") != 511 or pdf.get("page_size") != "A4":
        raise RuntimeError("Lab 1 PDF identity/page boundary drift")
    if plan.get("final_reader_artifacts", {}).get("pdf", {}).get("pages") != 511:
    ...
    if not (
        ...
        and lab.get("tasks") == 6
        and lab.get("tests") == 6
        and lab.get("stable_ids") == 24
        and backend.get("cumulative_records") == 7404
        and backend.get("cumulative_bytes") == 8975700
        and backend.get("cumulative_bundle_sha256")
        == "4740eb2ff83b4f9df3c0d90c2426ff77e652b23cad0bbe7763c54ebdefa60b4b"
        and backend.get("laboratories_complete") == 1
        and backend.get("laboratories_required") == 4
    ):
        raise RuntimeError("final build does not bind the Lab 1 source/backend closure")
    if not (
        ...
        raise RuntimeError("Lab 1 browser QA did not pass")
    if "Status: **PASS**" not in visual_text or pdf["sha256"] not in visual_text:
        raise RuntimeError("Lab 1 visual QA does not bind the final PDF")
    for relative, row in (
    ...
    if build.get("remaining", {}).get("computation_labs") != ["D60-LAB02", "D60-LAB03", "D60-LAB04"]:
        raise RuntimeError("Lab 1 build has dishonest remaining-laboratory state")
        ...
        manifest.update(
            {
                ...
                "laboratories": {
                    "completed": ["D60-LAB01"],
                    "edition_unit_ids": ["O012-ORIG-LAB01"],
                    "required": 4,
                    "completed_count": 1,
                    "remaining": ["D60-LAB02", "D60-LAB03", "D60-LAB04"],
                    "tasks": 6,
                    "tests": 6,
                    "stable_ids": 24,
                ...
                "course_closure": {
                    ...
                    "computation_laboratories_complete": 1,
                    "computation_laboratories_required": 4,
                    "computation_labs_remaining": 3,
                ...
                "reader_qa": {
                    ...
                    "laboratory_stable_ids_added": 24,
    ...
    base.write_json(
        PACKAGE_RECEIPT,
        {
            ...
            "scope": (
                ...
                "computation Lab 1 complete; Labs 2–4, proof-metadata closure, and capstone pending"
            ),
        ...
        json.dumps(
            {
                ...
                "post_build_bindings_required": [
                    "final Lab 1 build receipt and exact PDF/HTML identities/pages",
                    "visual, render-inventory, and browser QA receipts",
                    "Lab 1 source/execution/review and exact backend replay receipts",
                    "v0.31.3 release controls and checksum template",
