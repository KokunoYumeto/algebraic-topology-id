#!/usr/bin/env python3
"""Fail-closed append-only producer for the Fomberg Unit 006 backend suffix."""
...
COMMON_PATH = LANE / "scripts/fomberg-unit-006-common.py"


def load_common():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u006_common_producer", COMMON_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 006 common module")
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 006 backend producer FAIL: {message}")
...
def validate_merged(c, data, identities, prefix_records, additions):
    ...
    root_unit = by_id.get(c.ROOT); root_segment = by_id.get("segment:o012-fom-u006")
    ...
    require(
        ...
        ) == ("reader_unit", 36, 6, 12, "o012-fom-u006", "source_heading", "o012-fom-u006"),
    ...
    for number in range(1, 7):
        require(by_id[f"unit:o012-fom-u006-sol-{number:03d}"].get("solution_status") == "complete_checked_solution", f"mastery solution {number} status mismatch")
    ...
    require(len(assets) == 16, "asset census is not source + layer + seven PNG/SVG pairs")
    layer = assets["asset:o012-fom-u006-semantic-diagram-layer"]
    require(
        layer.get("source_diagram_count") == 11
        and layer.get("semantic_figure_block_count") == 7
        and layer.get("geometric_redraw_count") == 7
        and len(layer.get("semantic_unit_ids", [])) == 7,
        "semantic redraw layer closure mismatch",
    )
    for slug, figure_id in c.ASSET_SPECS:
        for ext in ("png", "svg"):
            asset = assets[f"asset:o012-fom-u006-{slug}-{ext}"]
    ...
    require(
        relation_counts == Counter({
            "adapts": 1, "contains": 2, "hints": 6, "illustrates": 7,
    ...
    require(
        b"C:\\Users" not in combined and b"token" not in combined.lower()
...
def main() -> int:
    ...
    print("Fomberg Unit 006 semantic backend extension: PASS")
