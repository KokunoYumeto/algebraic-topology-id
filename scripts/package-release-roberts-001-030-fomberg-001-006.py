#!/usr/bin/env python3
"""Prepare the Roberts 001-030 + Fomberg 001-006 Zenodo payload.
...
BASE_PATH = SCRIPT.with_name(
    "package-release-roberts-001-030-fomberg-001-005.py"
)
SPEC = importlib.util.spec_from_file_location("o012_fomberg_005_packager", BASE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the proved Fomberg 001-005 packager")
previous = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(previous)
base = previous.base

LANE = SCRIPT.parents[1]
RELEASE = LANE / "release" / "zenodo-roberts-001-030-fomberg-001-006"
...
TITLE = (
    ...
    "Jembatan Homologi, Derajat, dan Kompleks Seluler §1.1–1.12"
)
VERSION = "0.30.6"
RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-006-v0.30.6"
CONCEPT_DOI = "10.5281/zenodo.22061489"
PREVIOUS_RECORD_ID = 22098820
PREVIOUS_DOI = "10.5281/zenodo.22098820"
...
ARTIFACT_MANIFEST = "output/ARTIFACT_MANIFEST_ROBERTS_001_030_FOMBERG_001_006.csv"
BUILD_RECEIPT = "qa/ROBERTS_001_030_FOMBERG_001_006_BUILD_RECEIPT.json"
VISUAL_QA = "qa/ROBERTS_001_030_FOMBERG_001_006_VISUAL_QA.md"
RENDER_INVENTORY = "qa/ROBERTS_001_030_FOMBERG_001_006_RENDER_INVENTORY.csv"
BACKEND_CUMULATIVE_JSON = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_CUMULATIVE_RECEIPT.json"
)
BACKEND_CUMULATIVE_MD = (
    "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_CUMULATIVE_RECEIPT.md"
)
FOMBERG_QA_006 = "qa/FOMBERG_UNIT_006_QA.json"
FOMBERG_SOURCE_AUDIT_006 = "qa/FOMBERG_UNIT_006_SOURCE_AUDIT.json"
FOMBERG_REVIEW_006 = "qa/fomberg-unit-006/INDEPENDENT_REVIEW_FINAL.json"
FOMBERG_READER_006 = (
    "source/id-ID/fomberg/units/"
    "fomberg-unit-006-cellular-complexes.md"
)
FOMBERG_UNIT_006_SOURCE_SHA256 = (
    "c16d595b8f8c4c67ea5f0f58c1ad7de83ac94efae509d3a8d3bef28da2522f19"
)
FOMBERG_ASSETS_006: tuple[str, ...] = tuple(
    f"source/id-ID/fomberg/assets/unit-006/{stem}.{suffix}"
    for stem in (
        "attaching-characteristic-maps",
        "cw-skeleta",
        "hawaiian-earring",
        "petersen-graph",
        "projective-filtration",
        "sphere-cell-structures",
        "torus-cw",
    )
...
PDF_NAME = (
    ...
    "FOMBERG_001_006_READER.pdf"
)
HTML_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_"
    "FOMBERG_001_006_READER.html"
)
SOURCE_ZIP_NAME = (
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_006_"
...
def source_entries() -> dict[str, Path]:
    """Compact resumable source/backend inventory, including Unit006 once."""
    entries = _previous_source_entries()
    entries.update(
        {
            FOMBERG_READER_006: base.lane_path(FOMBERG_READER_006),
            **{
                relative: base.lane_path(relative)
                for relative in FOMBERG_ASSETS_006
            },
            "scripts/build-roberts-001-030-fomberg-001-006.ps1": (
                LANE / "scripts" / "build-roberts-001-030-fomberg-001-006.ps1"
            ),
            "scripts/fomberg-unit-006-common.py": (
                LANE / "scripts" / "fomberg-unit-006-common.py"
            ),
            "scripts/qa-fomberg-unit-006.py": (
                LANE / "scripts" / "qa-fomberg-unit-006.py"
            ),
            "scripts/extend-backend-fomberg-unit-006.py": (
                LANE / "scripts" / "extend-backend-fomberg-unit-006.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-006.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-006.py"
            ),
            "scripts/validate-backend-append-only-fomberg-unit-006-cumulative.py": (
                LANE
                / "scripts"
                / "validate-backend-append-only-fomberg-unit-006-cumulative.py"
            ),
            "scripts/finalize-build-roberts-001-030-fomberg-001-006.py": (
                LANE
                / "scripts"
            ...
            "scripts/publish-zenodo-roberts-001-030-fomberg-001-006.py": (
                LANE
                / "scripts"
                / "publish-zenodo-roberts-001-030-fomberg-001-006.py"
            ),
        }
    )
    return entries
...
def source_path_inventory() -> set[str]:
    ...
    paths.remove("scripts/package-release-roberts-001-030-fomberg-001-006.py")
    return paths
...
def qa_path_inventory() -> set[str]:
    ...
    paths.update(
        {
            FOMBERG_QA_006,
            FOMBERG_SOURCE_AUDIT_006,
            FOMBERG_REVIEW_006,
            "qa/fomberg-unit-006/TERMINOLOGY_ROWS_DRAFT.csv",
            "qa/fomberg-unit-006/ADVERSE_ROWS_DRAFT.csv",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_FILE_MANIFEST.csv",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_RECEIPT.json",
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_RECEIPT.md",
            BACKEND_CUMULATIVE_JSON,
            BACKEND_CUMULATIVE_MD,
            "qa/BACKEND_APPEND_ONLY_FOMBERG_UNIT_006_REPLAY_RECEIPT.json",
            BUILD_RECEIPT,
            VISUAL_QA,
            RENDER_INVENTORY,
        }
...
def release_control_inventory() -> set[str]:
    prefix = "release/zenodo-roberts-001-030-fomberg-001-006/"
    return {
        prefix + name
        for name in (
            "metadata.json",
...
def verify_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[str]]:
    ...
    if re.search(
        ...
        r"(?:0*1\s*[-–]\s*0*3[1-6]|0*3[1-6]\s*/\s*0*3[1-6])",
        joined,
    ):
        raise RuntimeError("Fomberg was incorrectly renumbered as Roberts 31-36")
    for marker in (
        "30/30",
        "O012-FOM-001",
        "O012-FOM-002",
    ...
    if (
        ...
        != "roberts_complete_fomberg_units_001_006_complete_composite_course_partial"
    ):
    ...
    if (
        ...
        != ["D60-R08", "D60-R09", "D60-R10", "D60-R11", "D60-R12", "D60-R12"]
        or numbering.get("fomberg_is_not_roberts_units_031_036") is not True
    ):
...
def verify_build_receipt(artifact_rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if (
        ...
        != "O012-RBT-001-030-FOM-001-006-COMPOSITE-BUILD"
    ):
    ...
    if (
        ...
        or fomberg.get("selected_source_span") != "31-3517"
        or fomberg.get("selected_span_lines") != 3487
        or fomberg.get("next_source_line") != 3518
...
def verify_fomberg_qa() -> dict[str, Any]:
    qa = json.loads(base.lane_path(FOMBERG_QA_006).read_text(encoding="utf-8"))
    if (
        qa.get("status") != "PASS"
        or qa.get("qa_id") != "O012-FOMBERG-UNIT-006-STATIC-QA"
        or qa.get("model_provenance") != MODEL_NOTE
    ):
        raise RuntimeError("Fomberg Unit006 static QA did not pass")
    source = qa.get("source", {})
    span = source.get("selected_span", {})
    if (
        source.get("commit") != FOMBERG_COMMIT
        or source.get("tree") != FOMBERG_TREE
        or source.get("next_line") != 3518
        or source.get("next_line_text") != r"\subsection{Cellular homology}"
        or span.get("line_start") != 3123
        or span.get("line_end") != 3517
        or span.get("lf_lines") != 395
        or span.get("sha256") != FOMBERG_UNIT_006_SOURCE_SHA256
    ):
        raise RuntimeError("Fomberg Unit006 source boundary mismatch")
    reader = qa.get("reader", {})
    if not base.identity_matches(reader.get("identity"), base.lane_path(FOMBERG_READER_006)):
        raise RuntimeError("Fomberg QA does not bind translated Unit006")
    if (
        reader.get("stable_ids") != 55
        or reader.get("stable_ids_unique") != 55
        or reader.get("mastery", {}).get("exercise_hint_solution_triples") != 6
        or reader.get("mastery", {}).get("complete_solutions") != 6
        or reader.get("assets", {}).get("semantic_figures") != 7
    ):
        raise RuntimeError("Fomberg Unit006 structure/mastery/proof closure drift")
    if any(value != "PASS" for value in qa.get("gates", {}).values()):
        raise RuntimeError("Fomberg Unit006 QA gate is incomplete")
    if qa.get("independent_review", {}).get("severity_census") != {
        "P1": 0,
        "P2": 0,
        "P3": 0,
    }:
        raise RuntimeError("Fomberg Unit006 independent review is not zero-severity")
    asset_rows = reader.get("assets", {}).get("files", [])
    if {row.get("path") for row in asset_rows} != set(FOMBERG_ASSETS_006):
        raise RuntimeError("Unit006 QA does not bind all seven PNG/SVG pairs")
    for row in asset_rows:
        if not base.identity_matches(row, base.lane_path(row["path"])):
            raise RuntimeError(f"Unit006 asset identity drift: {row['path']}")
    if not base.identity_matches(qa.get("source_audit"), base.lane_path(FOMBERG_SOURCE_AUDIT_006)):
        raise RuntimeError("Fomberg Unit006 source-audit identity drift")
    if not base.identity_matches(
        qa.get("independent_review", {}).get("identity"),
        base.lane_path(FOMBERG_REVIEW_006),
    ):
        raise RuntimeError("Fomberg Unit006 independent-review identity drift")
    return qa
...
def verify_backend_receipt(backend_facts: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("Fomberg Unit006 cumulative backend receipt failed")
    ...
    if (
        current.get("next_source_line") != 3518
        or current.get("terminal_source_eof") is not False
        or current.get("stable_ids") != 531
        or current.get("mastery_triples") != 37
        or current.get("redraw_files") != 32
        or current.get("total_records") != 6512
        or current.get("total_bytes") != 7855910
    ...
    if (
        ...
        or prefix.get("records") != 6309
        or prefix.get("bundle_sha256")
        != "c7b153cd217ac7dee87f4c399815c4d2bb51cff872c71665fe395efb3ffd95fa"
    ):
        raise RuntimeError("Fomberg Unit005 backend boundary was not preserved")
    if not base.identity_matches(
...
def write_package_receipt(
    ...
    payload = {
        ...
        "scope": (
            ...
            "O012-FOM-006 (Sections 1.1-1.12, lines 31-3517) complete; "
            "next Fomberg line 3518; composite course partial"
        ),
        ...
        manifest.update(
            {
                ...
                "reader_qa": {
                    ...
                    "fomberg_unit_006_stable_ids": fomberg_qa["reader"]["stable_ids"],
                    "mastery_triples_total": backend_receipt["current"]
                    ["mastery_triples"],
                    "fomberg_unit_006_mastery_triples": fomberg_qa["reader"]["mastery"]
                    ["exercise_hint_solution_triples"],
                    "fomberg_unit_006_semantic_figures": fomberg_qa["reader"]
                    ["assets"]["semantic_figures"],
                    "fomberg_unit_006_external_assets": len(FOMBERG_ASSETS_006),
        ...
        json.dumps(
            {
                ...
                "release_directory": RELEASE.relative_to(LANE).as_posix(),
                "scope": (
                    "Roberts 30/30 + Fomberg O012-FOM-001/002/003/004/005/006 "
                    "through line 3517; next line 3518; composite course partial"
                ),
                "files": [
                    {
                        "filename": name,
