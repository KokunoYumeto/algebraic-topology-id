#!/usr/bin/env python3
"""Publish the sealed Roberts 001-030 + Fomberg 001-003 checkpoint.

The transaction is locked to Zenodo concept DOI 10.5281/zenodo.22061489 and
public predecessor 22087423 (version 0.30.2).  It creates one new version,
...
base.RELEASE = (
    base.LANE / "release" / "zenodo-roberts-001-030-fomberg-001-003"
)
...
base.SEED_RECORD = 22087423
previous.PREVIOUS_RECORD_ID = 22087423
previous.PREVIOUS_DOI = "10.5281/zenodo.22087423"
base.RELEASE_ID = "o012-composite-id-roberts-001-030-fomberg-001-003-v0.30.3"
base.TITLE = (
...
base.FILE_NAMES = [
    base.PDF_NAME,
    "TOPOLOGI_ALJABAR_ID_ROBERTS_001_030_FOMBERG_001_003_READER.html",
    (
...
def verify_local() -> tuple[dict, dict]:
    ...
    if not (
        ...
        and fomberg.get("line_end") == 1922
        and fomberg.get("sections") == "1.1-1.6"
        ...
        and fomberg.get("next_source_line") == 1923
    ...
    if (
        ...
        raise RuntimeError(
            "publication controls are intentionally unsealed; publishing is blocked"
        )
    if (
    ...
    if (
        ...
        or backend_binding.get("final_identity_source")
...
def make_receipt(
    ...
    return {
        ...
        "scope": (
            ...
            "Fomberg algebraic_topology.tex lines 31-1922, Sections 1.1-1.6, "
            "O012-FOM-001/002/003 complete; next line 1923; "
