#!/usr/bin/env python3
"""Fail-closed append-only producer for the Fomberg Unit 003 backend suffix."""
...
COMMON_PATH = LANE / "scripts/fomberg-unit-003-common.py"


def load_common():
    spec = importlib.util.spec_from_file_location("o012_fomberg_u003_common_producer", COMMON_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load Fomberg Unit 003 common module")
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"Fomberg Unit 003 backend producer FAIL: {message}")
...
def validate_merged(
    ...
    require(len(suffix_records) == sum(c.DELTA.values()) == 405, "suffix is not exactly 405 records")
    ...
    require(
        ...
        == ("reader_unit", 33, 3, 10, "o012-fom-u003", "source_heading", "o012-fom-u003"),
    ...
    require(
        ...
        and {record["id"] for record in corrections}
            == {f"correction:o012-fom-u003-adv-{number:04d}" for number in range(457, 499)}
        and {record.get("adverse_ledger_id") for record in corrections}
            == {f"O012-ADV-{number:04d}" for number in range(457, 499)},
        "42-record adverse correction closure mismatch",
    )
    require(
        all(
    ...
    require(
        len(pending) == 1 and pending[0]["adverse_ledger_id"] == "O012-ADV-0460",
        "forward quotient-LES obligation is not isolated exactly once",
    )

    relation_counts = Counter(record["relation_type"] for record in additions["relations.jsonl"])
    require(
        relation_counts == Counter({
            "adapts": 1, "contains": 2, "hints": 6, "illustrates": 32,
            "precedes": 2, "proves": 1, "solves": 6, "xref": 7,
        }),
        f"57-relation closure mismatch: {dict(relation_counts)}",
    )
    require(
        by_id["unit:o012-fom-u003-proof-long-exact-repair"].get("repair_id") == "FOM-PR-04"
        and by_id["unit:o012-fom-u003-proof-long-exact-repair"].get("proof_status") == "complete_original_repair"
        and by_id["unit:o012-fom-u003-thm-long-exact"].get("repair_id") == "FOM-PR-04"
        and by_id["unit:o012-fom-u003-thm-long-exact"].get("proof_status") == "statement_with_complete_repaired_proof"
        and by_id["unit:o012-fom-u003-omission-pr04"].get("repair_id") == "FOM-PR-04"
        and by_id["unit:o012-fom-u003-omission-pr04"].get("proof_status") == "source_omission_named",
        "FOM-PR-04 statement/omission/repair binding mismatch",
    )
    for number in range(1, 7):
        require(
            by_id[f"unit:o012-fom-u003-sol-{number:03d}"].get("solution_status")
    ...
    require(len(assets) == 14, "asset record count mismatch")
    require(
        len([record for record in assets.values() if record.get("role") == "reader_linked_accessible_redraw"]) == 6
        and len([record for record in assets.values() if record.get("role") == "accessible_vector_source_companion"]) == 6,
        "six PNG/six SVG asset closure mismatch",
    )
    diagram = assets["asset:o012-fom-u003-semantic-diagram-layer"]
    require(
        diagram.get("source_diagram_count") == 29
        and diagram.get("semantic_figure_block_count") == 26
        and diagram.get("geometric_redraw_count") == 6
        and len(diagram.get("semantic_unit_ids", [])) == 26,
        "semantic diagram-layer closure mismatch",
    )

    artifacts = {record["id"]: record for record in additions["artifacts.jsonl"]}
    require(len(artifacts) == 6, "six-artifact evidence closure mismatch")
    ...
    require(
        {record.get("terminology_control_id") for record in additions["terms.jsonl"]}
            == {f"O012-TERM-{number:04d}" for number in range(401, 416)},
...
def main() -> int:
    ...
    require(total_records == c.PREFIX_TOTAL[0] + sum(c.DELTA.values()) == 5747, "cumulative record count mismatch")
    print("Fomberg Unit 003 semantic backend extension: PASS")
