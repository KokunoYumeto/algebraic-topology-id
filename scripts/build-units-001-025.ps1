...
$baselineBuilder = Join-Path $PSScriptRoot 'build-units-001-024.ps1'
$authority = Join-Path $lane 'authority\upstream\AlgebraicTopology2019-b947ad2e9f9e301bfe24590a9db653bc54fa1a53\Notes.tex'
$unit025 = Join-Path $lane 'source\id-ID\units\unit-025-lecture-025.md'
$unit025Qa = Join-Path $lane 'qa\UNIT_025_QA.json'
$unit025Review = Join-Path $lane 'qa\UNIT_025_INDEPENDENT_REVIEW.md'
$unit025Audit = Join-Path $lane 'qa\UNIT_025_SOURCE_AUDIT.md'
...
$htmlDir = Join-Path $lane 'output\html\units-001-025'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-025-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-025-id.pdf'
$manifestPath = Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_025.csv'

# Frozen Units 001-024 boundary. These inputs are verified but never invoked
# or rewritten by this cumulative builder.
$baselineFrozen = @{
    $baselineBuilder = @(21976, '8e62da597b783f56e0a9174a7822ee453de6de8be01fc5493cb2ab93c41a3c44')
    (Join-Path $lane 'output\html\units-001-024\index.html') = @(3927104, '28a84406de9e196070965920a7f7937177197977f9ddf118f0f8b07d464cbf0f')
    (Join-Path $lane 'output\pdf\topologi-aljabar-unit-001-024-id.pdf') = @(1907368, '5189b04f2f28d7e8192c16e8ef070e23bbf98085d150d1f2124d15c071ccf9b8')
    (Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_024.csv') = @(249, '23d2b33dd8eb08ba82bb020e3607abbf24925d79e774ca177be228656800a0ff')
    (Join-Path $lane 'qa\UNITS_001_024_BUILD_RECEIPT.json') = @(7560, 'a050b3d282d43033ccdd7565bc6ee301eee6c30014ef6d8b84c5ec490406129a')
    (Join-Path $lane 'qa\UNITS_001_024_VISUAL_QA.md') = @(4928, '7aff942b47ec489a56923879aee189fc3911e0eded232a511de66aff8ee01a27')
    (Join-Path $lane 'qa\UNITS_001_024_RENDER_INVENTORY.csv') = @(2659, '1d9554d98de7d4751fc7ee2d1b5a6cb45edb580e62226b030aea389dff9de683')
...
foreach ($path in $baselineFrozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Units 001-024 baseline input: $path" }
    $want = $baselineFrozen[$path]
    $item = Get-Item -LiteralPath $path
    $digest = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($item.Length -ne $want[0] -or $digest -ne $want[1]) { throw "Frozen Units 001-024 baseline mismatch: $path" }
}
...
$authoritySpanText = ($authorityLines[133..5610] -join "`n") + "`n"
...
if ($authoritySpanBytes.Length -ne 283555 -or $authoritySpanDigest -ne '2bb15364d4f03264797bfd93bfecb69861b55f4b5075d11441921bd677f3e2b1') {
    throw 'Frozen Notes.tex:134-5611 cumulative span mismatch.'
}
if ($authorityLines[5611] -notmatch '\\lecturenum\{26\}') { throw 'Notes.tex line 5612 is not the Unit 26 boundary.' }

$sources = @((Join-Path $lane 'source\id-ID\reader-unit-001.md')) + @(
    2..25 | ForEach-Object { Join-Path $lane ("source\id-ID\units\unit-{0:000}-lecture-{0:000}.md" -f $_) }
)
$sourceExpected = @(
...
# Frozen Unit 025 reader and complete admission-evidence quartet.
$unit025Frozen = @{
    $unit025 = @(36578, 'df72add4e57236b51ff7d2a0c99af4b65299365874163cb334be5d0988c0f769')
    $unit025Audit = @(6386, 'f252e9f15e0980ed2a2c15dfbd1c22fd6fd99990333e2de9a9372f695523e903')
    $unit025Review = @(6933, 'c7be12ea116b76ea2789b9e1d81cca973ea4d108e97f821eaeba4491ddcb7c08')
    $unit025Qa = @(6224, 'b884ca922191c1ec08a05788e9ae37a8bd05fb475f743f5340815f3a10225b15')
}
foreach ($path in $unit025Frozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Unit 025 gate input: $path" }
    $want = $unit025Frozen[$path]
    if ((Get-Item -LiteralPath $path).Length -ne $want[0] -or (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $want[1]) {
        throw "Unit 025 gate identity mismatch: $path"
    }
}
$reviewText = [IO.File]::ReadAllText($unit025Review)
if ($reviewText -notmatch '(?m)^- P1: 0\s*$' -or $reviewText -notmatch '(?m)^- P2: 0\s*$' -or $reviewText -notmatch '(?m)^- P3: 0\s*$' -or
    $reviewText -notmatch '36,578 bytes, 1,104 LF lines' -or $reviewText -notmatch '449 MathML nodes') {
    throw 'Unit 025 independent review is not the zero-finding final snapshot.'
}
$qaText = [IO.File]::ReadAllText($unit025Qa)
if ($qaText -notmatch '"status"\s*:\s*"PASS"' -or $qaText -notmatch '"model_provenance"\s*:\s*"OpenAI Codex gpt-5\.6-sol, Ultra"' -or
    -not $qaText.Contains('df72add4e57236b51ff7d2a0c99af4b65299365874163cb334be5d0988c0f769')) {
    throw 'Unit 025 QA is not a passing, source-bound, model-attributed receipt.'
}
$unit025Text = [IO.File]::ReadAllText($unit025)
foreach ($marker in @('TODO', 'TBD', 'FILL_AFTER', 'C:\Users\', 'C:/Users/', 'github_pat_', 'ghp_', 'sk-proj_', 'access_token')) {
    if ($unit025Text.Contains($marker)) { throw "Private or placeholder marker in Unit 025: $marker" }
}
if (([regex]::Matches($unit025Text, '(?m)^[ \t]*::: \{')).Count -ne 52 -or
    ([regex]::Matches($unit025Text, '(?m)^[ \t]*:::\s*$')).Count -ne 52) { throw 'Unit 025 fenced-div census or balance mismatch.' }
if (([regex]::Matches($unit025Text, '\$\$')).Count % 2 -ne 0) { throw 'Unit 025 has unbalanced display math.' }
$unit025Ids = [regex]::Matches($unit025Text, '#(o012-[a-z0-9-]+)(?=[}\s])') | ForEach-Object { $_.Groups[1].Value }
if ($unit025Ids.Count -ne 59 -or @($unit025Ids | Sort-Object -Unique).Count -ne 59) { throw 'Unit 025 stable-ID census mismatch.' }
$blockKinds = 'aside|boundary|definition|example|exercise|figure|hint|lemma|proof|proposition|remark|solution|source-audit'
$counts = @{}
foreach ($match in [regex]::Matches($unit025Text, "(?m)^[ \t]*::: \{\.(?<kind>$blockKinds)\s+#o012-")) {
...
if ($signature -ne 'aside=7,boundary=1,definition=3,example=6,exercise=6,figure=2,hint=6,lemma=2,proof=4,proposition=2,remark=1,solution=6,source-audit=6') {
    throw "Unit 025 semantic-block mismatch: $signature"
}
if (-not $unit025Text.Contains('data-source-label="eg:dim_minus_one_skeleton_rel_cochains"') -or
    -not $unit025Text.Contains('Baris 5612')) {
    throw 'Unit 025 source alias or terminal cursor marker is missing.'
}
...
$assembled = Join-Path $checkDir 'reader-units-001-025.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-25"
subtitle: "Homotopi, Ruang Penutup, Grup Fundamental, Kohomologi, Korantai Simpleksial, Kohomologi Relatif dan Tereduksi, Lema Lima, serta Karakteristik Euler"
author:
...
$htmlAssembled = Join-Path $checkDir 'reader-units-001-025-html.md'
...
$common = @(
    ...
    '--metadata=lang:id-ID', '--metadata=pagetitle:Topologi Aljabar - Unit 1-25',
...
$htmlA = Join-Path $checkDir 'units-001-025-a.html'
$htmlB = Join-Path $checkDir 'units-001-025-b.html'
...
$missingUnit025Ids = @($unit025Ids | Where-Object { -not $domIdSet.Contains($_) })
if ($missingUnit025Ids.Count -ne 0) { throw "Unit 025 IDs missing from HTML: $($missingUnit025Ids -join ', ')" }
...
$pdfAssembled = Join-Path $checkDir 'reader-units-001-025-pdf.md'
$inDisplayMath = $false
...
$pdfA = Join-Path $checkDir 'units-001-025-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-025-b.pdf'
$pdfHeader = Join-Path $checkDir 'reader-units-001-025-header.tex'
