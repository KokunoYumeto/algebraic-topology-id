...
$baselineBuilder = Join-Path $PSScriptRoot 'build-units-001-026.ps1'
$authority = Join-Path $lane 'authority\upstream\AlgebraicTopology2019-b947ad2e9f9e301bfe24590a9db653bc54fa1a53\Notes.tex'
$unit027 = Join-Path $lane 'source\id-ID\units\unit-027-lecture-027.md'
$unit027Qa = Join-Path $lane 'qa\UNIT_027_QA.json'
$unit027Review = Join-Path $lane 'qa\UNIT_027_INDEPENDENT_REVIEW.md'
$unit027Audit = Join-Path $lane 'qa\UNIT_027_SOURCE_AUDIT.md'
...
$htmlDir = Join-Path $lane 'output\html\units-001-027'
$pdfDir = Join-Path $lane 'output\pdf'
$checkDir = Join-Path $lane 'tmp\pdfs\units-001-027-build'
$html = Join-Path $htmlDir 'index.html'
$pdf = Join-Path $pdfDir 'topologi-aljabar-unit-001-027-id.pdf'
$manifestPath = Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_027.csv'

# Frozen Units 001-026 boundary. These inputs are verified but never invoked
# or rewritten by this cumulative builder.
$baselineFrozen = @{
    $baselineBuilder = @(22117, 'fe75e28e13418df0854355ef5ba37bfd93976c70382faa8dc497a9d65ce6015a')
    (Join-Path $lane 'output\html\units-001-026\index.html') = @(4306392, 'a9e4bea73cab5e90136822fbb94a11a1006f7e81ffcdf0a5740da52e335b4e3b')
    (Join-Path $lane 'output\pdf\topologi-aljabar-unit-001-026-id.pdf') = @(2042637, '27a5a6f2635328dbd3576a92900d110ae627f48da8c514c40f0d94f2d153f19d')
    (Join-Path $lane 'output\ARTIFACT_MANIFEST_UNITS_001_026.csv') = @(249, '2c4d00fae0543ce7e0b8f58896d9435e46364f1fed4bae2dc81dddc9a96cfd51')
    (Join-Path $lane 'qa\UNITS_001_026_BUILD_RECEIPT.json') = @(10284, '51d6329fe481f207509249a68eed5879df96d399fa39bbb8f23c29a56d4b1ccb')
    (Join-Path $lane 'qa\UNITS_001_026_VISUAL_QA.md') = @(4903, 'b768eb101761e87dba6bf936d0d0d6a2ddb1bc8d4a978fc66baeb78562253c0f')
    (Join-Path $lane 'qa\UNITS_001_026_RENDER_INVENTORY.csv') = @(2880, '90bc6d15c91ce36fd266ad752e374449ab54956278b4fc79b6735214b8200d5f')
...
foreach ($path in $baselineFrozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Units 001-026 baseline input: $path" }
    $want = $baselineFrozen[$path]
    $item = Get-Item -LiteralPath $path
    $digest = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($item.Length -ne $want[0] -or $digest -ne $want[1]) { throw "Frozen Units 001-026 baseline mismatch: $path" }
}
...
$authoritySpanText = ($authorityLines[133..5922] -join "`n") + "`n"
...
if ($authoritySpanBytes.Length -ne 300330 -or $authoritySpanDigest -ne '4fd010c0cff49009ab26761990be513078ed0107f2980880a8919734459be733') {
    throw 'Frozen Notes.tex:134-5923 cumulative span mismatch.'
}
if ($authorityLines[5923] -notmatch '\\lecturenum\{28\}') { throw 'Notes.tex line 5924 is not the Unit 28 boundary.' }

$sources = @((Join-Path $lane 'source\id-ID\reader-unit-001.md')) + @(
    2..27 | ForEach-Object { Join-Path $lane ("source\id-ID\units\unit-{0:000}-lecture-{0:000}.md" -f $_) }
)
$sourceExpected = @(
...
# Frozen Unit 027 reader and complete admission-evidence quartet.
$unit027Frozen = @{
    $unit027 = @(35879, 'a3238bbc429e4c3689bce3b3bb78c5514e0fae74f276c9efebe694730b2df2a0')
    $unit027Audit = @(7369, '67621ce38a69fe4e6afd24fa6572dfa6e9499a3e77db03e3943ce045dfa30138')
    $unit027Review = @(8560, 'd464310e42fea0c199b49a6a9670d97cf8704b3c37b7cc79023fece21cb734b5')
    $unit027Qa = @(8152, '1db997b0ddf1dee05583c8c8c482e68f59a9add7677b34fded034088f55e491f')
}
foreach ($path in $unit027Frozen.Keys) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing frozen Unit 027 gate input: $path" }
    $want = $unit027Frozen[$path]
    if ((Get-Item -LiteralPath $path).Length -ne $want[0] -or (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $want[1]) {
        throw "Unit 027 gate identity mismatch: $path"
    }
}
$reviewText = [IO.File]::ReadAllText($unit027Review)
if ($reviewText -notmatch '(?m)^- P1: 0\s*$' -or $reviewText -notmatch '(?m)^- P2: 0\s*$' -or $reviewText -notmatch '(?m)^- P3: 0\s*$' -or
    $reviewText -notmatch '35,879 bytes, 1,175 LF lines' -or $reviewText -notmatch '459 MathML nodes') {
    throw 'Unit 027 independent review is not the zero-finding final snapshot.'
}
$qaText = [IO.File]::ReadAllText($unit027Qa)
if ($qaText -notmatch '"status"\s*:\s*"PASS"' -or $qaText -notmatch '"model_provenance"\s*:\s*"OpenAI Codex gpt-5\.6-sol, Ultra"' -or
    -not $qaText.Contains('a3238bbc429e4c3689bce3b3bb78c5514e0fae74f276c9efebe694730b2df2a0')) {
    throw 'Unit 027 QA is not a passing, source-bound, model-attributed receipt.'
}
$unit027Text = [IO.File]::ReadAllText($unit027)
foreach ($marker in @('TODO', 'TBD', 'FILL_AFTER', 'C:\Users\', 'C:/Users/', 'github_pat_', 'ghp_', 'sk-proj_', 'access_token')) {
    if ($unit027Text.Contains($marker)) { throw "Private or placeholder marker in Unit 027: $marker" }
}
if (([regex]::Matches($unit027Text, '(?m)^[ \t]*::: \{')).Count -ne 39 -or
    ([regex]::Matches($unit027Text, '(?m)^[ \t]*:::\s*$')).Count -ne 39) { throw 'Unit 027 fenced-div census or balance mismatch.' }
if (([regex]::Matches($unit027Text, '\$\$')).Count % 2 -ne 0) { throw 'Unit 027 has unbalanced display math.' }
$unit027Ids = [regex]::Matches($unit027Text, '#(o012-[a-z0-9-]+)(?=[}\s])') | ForEach-Object { $_.Groups[1].Value }
if ($unit027Ids.Count -ne 46 -or @($unit027Ids | Sort-Object -Unique).Count -ne 46) { throw 'Unit 027 stable-ID census mismatch.' }
...
$counts = @{}
foreach ($match in [regex]::Matches($unit027Text, "(?m)^[ \t]*::: \{\.(?<kind>$blockKinds)\s+#o012-")) {
...
if ($signature -ne 'boundary=1,definition=2,equation=2,example=3,exercise=6,figure=1,hint=6,lemma=1,proof=3,proposition=1,solution=6,source-audit=5,source-margin=1,theorem=1') {
    throw "Unit 027 semantic-block mismatch: $signature"
}
if (-not $unit027Text.Contains('data-source-label="thm:mayer-vietoris"') -or
    -not $unit027Text.Contains('Baris 5924')) {
    throw 'Unit 027 source alias or terminal cursor marker is missing.'
}
...
$assembled = Join-Path $checkDir 'reader-units-001-027.md'
$header = @'
---
title: "Topologi Aljabar - Unit 1-27"
subtitle: "Homotopi, Ruang Penutup, Grup Fundamental, Kohomologi Simpleksial dan Singular, Kohomologi Relatif dan Tereduksi, Lema Lima, Mayer--Vietoris, serta Kohomologi Sfera"
author:
...
$htmlAssembled = Join-Path $checkDir 'reader-units-001-027-html.md'
...
$common = @(
    ...
    '--metadata=lang:id-ID', '--metadata=pagetitle:Topologi Aljabar - Unit 1-27',
...
$htmlA = Join-Path $checkDir 'units-001-027-a.html'
$htmlB = Join-Path $checkDir 'units-001-027-b.html'
...
$missingUnit027Ids = @($unit027Ids | Where-Object { -not $domIdSet.Contains($_) })
if ($missingUnit027Ids.Count -ne 0) { throw "Unit 027 IDs missing from HTML: $($missingUnit027Ids -join ', ')" }
...
$pdfAssembled = Join-Path $checkDir 'reader-units-001-027-pdf.md'
$inDisplayMath = $false
...
$pdfA = Join-Path $checkDir 'units-001-027-a.pdf'
$pdfB = Join-Path $checkDir 'units-001-027-b.pdf'
$pdfHeader = Join-Path $checkDir 'reader-units-001-027-header.tex'
