...
\begin{center}
\includegraphics[scale=0.50]{assets/ordinals.svg.png}
\end{center}
...
\newpage
  ...
  \begin{definition}[Bell's numbers]
...
\begin{itemize}
  \item A \emph{singleton} is a set containing only one element.
\item $\powerset(A) := \set{B \colon B \subseteq A}$.
\item $A \triangle B := \set{A \cup B} \setminus \set{A \cap B}$.
\item $|\R| = c = \beth_1 = \aleph$.
\item $A^c = \set{b \colon b \notin A}$.
\item $\prod_{i\in I} X_i = 
  \set{f \colon I \to \bigcup_{i\in I} X_i \mid f(i)\in X_i \text{ for all } i \in I}$.
\item A pairing function is a bijection $\pi \colon \N \times \N \to \N$.
\item The indicator function of $A \subseteq X$ is defined as
  $1_A(x) = I_A(x) = \chi_A(x) = 1 \iff x$ is in $A$ and equals $0$ otherwise.
\end{itemize}
