...
\end{proposition}
\begin{proof}
  Suppose first that $F$ is not closed.
  Then there exists $\set{x_n}_{n \geq 1}$
  ...
  Suppose next that $X \setminus F$ is not open.
...
\begin{definition}[Continuity]
  ...
  We say that $f$ is continuous at $x \in X$ if for every
  $\{x_n\}_{n \geq 1} \subset X$ we have
  \[
...
\begin{proposition}
  ...
  at $x$ if and only if for any $\epsilon > 0$ there exists $\delta > 0$
  so that $f(B(x,\delta)) \subset B(f(x),\epsilon)$.
\end{proposition}
\begin{proof}
  First suppose that for any $\epsilon > 0$ there exists $\delta > 0$
  so that $f(B(x,\delta)) \subset B(f(x),\epsilon)$.
  Let $\set{x_n}_{n \geq 1} \subset X$ be a sequence that converges to
  $x \in X$.
  Let $\epsilon > 0$, then there exists $\delta > 0$ so that
  $f(B(x,\delta)) \subset B(f(x),\epsilon)$.
  We know that exists $N \geq 1$ so that for all $n > N$ we have
  $x_n \in B(x,\delta)$ and so $f(x_n) \in B(f(x),\epsilon)$
  which implies that $\set{f(x_n)}_{n \geq 1}$ converges to $f(x)$
  which completes the proof.

  Next suppose that $f$ is continuous.
  Let $\epsilon > 0$.
  Assume, by way of contradiction, that there does not exist $\delta > 0$
  so that $f(B(x,\delta)) \subset B(f(x),\epsilon)$.
  This means that for any $\delta > 0$ there exists $x' \in B(x,\delta))$
  so that $x \notin B(f(x),\epsilon)$.
  In particular, for any $1/n$ there exists $x_n \in B(x_n,1/n))$
  so that $f(x_n) \notin B(f(x),\epsilon)$.
  We now have a sequence $\set{x_n}_{n \geq 1}$ that converges to $x$
  but $\set{f(x_n)}_{n \geq 1}$ does not converge to $f(x)$
  which is a contradiction.
  Thus, our assumption must be false which completes the proof.
\end{proof}
...
\end{proposition}
\begin{proof}
  First let $f$ be continuous.
  Let $U \subset Y$ be open, and let $x \in f(U)^{-1}$.
  Since $f(x) \in U$ and $U$ is open there exists an neighbourhood
  $y \subset B(y,\epsilon) \subset U$ for some $\epsilon > 0$.
  Since $f$ is continuous there exists $\delta > 0$ so that
  for any $f(B(x,\delta)) \subset B(f(x),\epsilon)$ which implies
  that $B(x,\delta) \subset f(U)^{-1}$ so $f(U)^{-1}$ is open.

  Next suppose that $f^{-1}(U)$ is open for every open $U \subset Y$.
  Let $x \in X$.
  The set $B(f(x),\delta) \subset Y$ is open in $Y$ for any $\delta$.
  Thus, for any $\delta > 0$ the set $f(B(f(x),\delta))^{-1} \subset X$
  is open.
  In particular, since $f(x) \in B(f(x),\delta)$
  there exists $\epsilon > 0$ so that
  $f(B(x,\delta)) \subset B(f(x),\epsilon)$,
  which shows that $f$ is continuous at $x$ for all $x \in X$
...
\begin{definition}[Isolated point]
  Let $A \subset X$. We say that $x \in A$ is an isolated point of
  $A$ if exists $U$ open in $X$ such that $A \cap U = \{x\}$.
\end{definition}
...
\begin{definition}[Limit point]
  Let $A \subset X$. We say that $x \in X$ is a limit point of $A$ if for every
...
\begin{example}
  Consider the set $A = \set{1/n \mid n \in \Z_{+}} \cup \{0\}$ as
...
\begin{remark}
  ...
  is equivalent. If we denote the metric space by $(X,d)$ and the 
  countable dense set by $A$, then
  \[
...
\begin{example}
  ...
  second countable is the Sorgenfrey line, also known as $\R$ with the lower
  limit topology, which we will discuss later.
\end{example}

\begin{theorem}[Lindel\"of’s theorem]
  \label{thm:lindelof}
  Suppose that $X$ is second countable. Let
  $\{U_i\}_{i \in I}$ be a family of open subsets of $X$. Then there
  exists a countable $I_0 \subset I$ so that 
  $\bigcup_{i \in I_{0}}{U_i} = \bigcup_{i \in I}{U_i}$.
\end{theorem}
\begin{proof}
  It is clear that
  $\bigcup_{i \in I_{0}}{U_i} \subseteq \bigcup_{i \in I}{U_i}$
  for any countable $I_0 \subset I$,
  so we will only show that there exists a countable $I_0 \subset I$
  so that $\bigcup_{i \in I_{0}}{U_i} \supseteq \bigcup_{i \in I}{U_i}$.
  Let $\mathcal{B}$ be a countable open base for $X$. Set,
  \[
    \mathcal{B}_0 = \set{B \in \mathcal{B} \mid B \subset U_i
    \text{ for some } i \in I},
  \]
  and for each $B \in \mathcal{B}_0$ choose an arbitrary $i_B \in I$ such
  that $B \subset U_{i_B}$. Set $I_0 = \set{i_B \mid B \in \mathcal{B}_0}$.
  Since $\mathcal{B}$ is countable, $I_0$ is also countable. It remains
  to show that $\bigcup_{i \in I_{0}}{U_i} = \bigcup_{i \in I}{U_i}$. Let
  $x \in \bigcup_{i \in I}{U_i}$ then there exists some $j$
  such that $x \in U_j$.
  Since $\mathcal{B}$ is an open base there exists $B \subset U_j$ such that
  $x \in B$. By definition we see that $B \in \mathcal{B}_0$,
  so there exists $i_B \in I_0$ so that $x \in B \subset U_{i_B}$, and then
  \[
    x \in B \subset U_{i_B} \subset \bigcup_{i \in I_{0}}{U_i}
  \]
  which completes the proof.
\end{proof}

\begin{definition}[Lindel\"of space]
  A topological space $X$ is said to be a Lindel\"of space if for every
  family of open subsets of $X$ denoted $\set{U_i}_{i \in I}$,
  there exists a countable $I_0 \subset I$ so that 
  $\bigcup_{i \in I_{0}}{U_i} = \bigcup_{i \in I}{U_i}$.
\end{definition}
\begin{remark}
  A metric space $X$ is a Lindel\"of space if and only if it is seperable,
  if and only if it is second countable.
  However, not every topolgical space $X$ that is a Lindel\"of space
  is second countable.
  This means that the converse to \Cref{thm:lindelof} is false.
\end{remark}

\begin{corollary}
  Let $X$ be second countable topological space
  and let $\mathcal{B}$ be an open base for $X$.
  Then there exists a countable $\mathcal{B}_0 \subset \mathcal{B}$ 
...
\begin{proof}
  ...
  $E = \bigcup_{B \in \mathcal{B}_E} B$. From Lindel\"of's theorem
  we get that exists a countable $\mathcal{B}^0_E \subset \mathcal{B}_E$
  such that $U_{B \in \mathcal{B}^0_E} = \bigcup_{i \in \mathcal{B}_E}{U_i}$.
  Now set $\mathcal{B}_0 = \bigcup_{E \in \mathcal{E}}{\mathcal{B}^0_E}$.
...
\begin{proposition}
  Let $X$ and $Y$ be topological spaces and let $\mathcal{S}$ be an open
  subbase for $Y$.
  Suppose that $f^{-1}(S)$ is open for each $S \in \mathcal{S}$.
  Then $f$ is continuous.
\end{proposition}
\begin{proof}
...
\begin{proposition}
  ...
  \[
    \mathcal{B} :=
    \set{\bigcap_{i=1}^{n}{S_i} \mid n \geq 0 \textnormal{ and }
...
\begin{remark}
  We denote the set of all topologies on a set $X$ by $\mathcal{T}(X)$.
\end{remark}
...
\begin{remark}
  ...
  topology generated by $\bigcup_{\tau \in \mathcal T_0} \tau$.
\end{remark}
...
\[
  S = \set{f_i^{-1}(U) \middle\vert i \in I \text{ and $U$ is open in $Y_i$}}
\]
\end{remark}
...
\begin{remark}
  ...
  \[
    \mathcal{S} = \set{\prod_{i \in I}{U_i} \middle\vert \exists j \in I \st 
  ...
  {\small
  \[
    \mathcal{B} = \set{\prod_{i \in I}U_i \middle\vert
    \text{There exists a finite
...
\begin{proposition}[Universal property of the product topology]
...
\begin{example}
  ...
  \begin{align*}
    \abs{f(x) - f_n(x)} &\le
    \limsup_{m \to \infty}\del{\abs{f(x) - f_m(x)} + \abs{f_m(x) - f_n(x)}} \\
...
\begin{theorem}
  ...
  metric space such that there exists an isometry $g \colon X \to Y$ such that
...
\begin{remark}
  We will now present the intuition for the definition for a nowhere dense set.
  Let $X$ be a topological space.
  A subset $A \subset X$ is said to be dense in another subset $S \subset X$
  if the intersection $A \cap S$ is dense in $S$.
  We could now define that $A \subset X$ is nowhere dense if it is not
  dense in any $\emptyset \neq S \subset X$.
  This definition might make more sense,
  but it is equivalent to definition we gave, and less practical,
  which is why we don't use it.
  Another equivalent condition for a set to be nowhere dense,
  is that its exterior ($X \setminus \overline A$) is dense in $X$.
\end{remark}

\begin{remark}
  Note that a closed subset $A \subset X$ is nowhere dense if and only if 
  its interior is the empty set, that is, $\Int(A) = \emptyset$.
\end{remark}
...
\begin{example}
  ...
  \[
    W = (W^{\perp})^{\perp} = 
    \bigcap_{u \in W^{\perp}}{\set{x \in \R^d \mid \ip{x}{u} = 0}},
  \]
...
\begin{definition}[First category set]
  A subset $E$ of $X$ is said to be of the first category (or meagre)
  if there exist nowhere dense subsets $A_1,A_2,\dots \subset X$ so that 
  $E = \cup_{n \geq 1}{A_n}$. A subset of $X$ which is not of the first 
  category is said to be of the second category (or nonmeagre).
\end{definition}

\begin{theorem}[Baire category theorem]
  Let $X$ be a complete metric space and let $E \subset X$ be of the first
...
\begin{proofof}{\Cref{thm:alexander-first}}
...
\begin{proofof}{\Cref{thm:alexander-second}}
...
\begin{proof}
  ...
  \[
    \set{(-\infty, c) \mid c \in \R} \cup 
    \set{(d, \infty) \mid d \in \R}
  \]
  is an open subbase to $\R$. From the lemma we have that the set:
  \[
    S = \set{[a, c) \mid a < c \le b} \cup 
    \set{(d, b] \mid a < d \le b}
  \]
  ...
  \[
    \mathcal{U} = 
    \set{[a,c_i) \mid i \in I} \cup \set{(d_j,b] \mid j \in J}.
  \]
...
\begin{proofof}{\Cref{thm:heine-borel}}
...
\end{proposition}
\begin{proof}
  Assume by contradiction that $X$ is not totally bounded, then there exists
...
\begin{definition}[$T_1$-spaces]
  ...
  $x_1, x_2 \in X$ there exist neighbourhoods $U_1$ of $x_1$ and $U_2$ of
  $x_2$ such that $x_1 \notin U_2$ and $x_2 \notin U_1$.
\end{definition}
...
\begin{proposition}
  The space $X$ is a $T_1$-space if and only if $\{x\}$ is 
  closed in $X$ for every $x \in X$.
\end{proposition}
\begin{proof}
  Suppose that $X$ is a $T_1$-space. Let $x \in X$. For every 
  $y \in X \setminus \{x\}$ there exists a neighbourhood 
...
\begin{definition}[Hausdorff space]
  ...
  $x_1, x_2 \in X$ there exist open sets $U_1, U_2 \subset X$ with 
...
\begin{corollary}
  ...
  Then every compact subset of $X$ is closed.
\end{corollary}
\begin{proof}
...
\begin{proofof}{\Cref{lem:urysohn}}
  To be added.
\end{proofof}
