...
Derajat, dan Kompleks Seluler §1.1–1.13*, versi 0.30.7. Komponen Roberts lengkap
30/30 sampai `Notes.tex:6368`. Komponen Fomberg mencakup
`algebraic_topology.tex:31–4185` (Bagian 1.1–1.13,
`O012-FOM-001–007`); bentang sumber Fomberg terpilih telah selesai, jalur
komposit masih parsial, dan kursor sumber berikutnya adalah baris 4186.
...
- `backend/` memuat backend append-only dengan skema `curriculum.interop
  0.1.0`. Boundary ini mempunyai 6.742 rekaman, 8.213.649 byte, dan hash bundel
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build-roberts-001-030-fomberg-001-007.ps1
```
